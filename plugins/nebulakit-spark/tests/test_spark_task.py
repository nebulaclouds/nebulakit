...
from nebulakitplugins.spark.task import Databricks, new_spark_session
from pyspark.sql import SparkSession

import nebulakit
from nebulakit import StructuredDataset, StructuredDatasetTransformerEngine, task
from nebulakit.configuration import Image, ImageConfig, SerializationSettings
from nebulakit.core.context_manager import ExecutionParameters, NebulaContextManager
...
def test_spark_task(reset_spark_session):
    ...
    def my_spark(a: str) -> int:
        session = nebulakit.current_context().spark_session
        assert session.sparkContext.appName == "NebulaSpark: ex:local:local:local"
        return 10
    ...
    assert ("spark.app.name", "NebulaSpark: ex:local:local:local") in configs
...
def test_to_html():
    ...
    output = tf.to_html(NebulaContextManager.current_context(), sd, pyspark.sql.DataFrame)
