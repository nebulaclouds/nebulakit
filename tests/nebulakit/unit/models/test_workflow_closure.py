...
from nebulakit.models import workflow_closure as _workflow_closure
...
def test_workflow_closure():
    ...
    task_metadata = _task.TaskMetadata(
        True,
        _task.RuntimeMetadata(_task.RuntimeMetadata.RuntimeType.NEBULA_SDK, "1.0.0", "python"),
        timedelta(days=1),
    ...
    task = _task.TaskTemplate(
        ...
        extended_resources=tasks_pb2.ExtendedResources(gpu_accelerator=T4.to_nebula_idl()),
    )
    ...
    obj2 = _workflow_closure.WorkflowClosure.from_nebula_idl(obj.to_nebula_idl())
    assert obj == obj2
    ...
    assert WorkflowSpec.from_nebula_idl(workflow_spec.to_nebula_idl()) == workflow_spec
