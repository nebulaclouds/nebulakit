...
import pytest
from nebulaidl.core.tasks_pb2 import ExtendedResources, TaskMetadata
from google.protobuf import text_format

import nebulakit.models.interface as interface_models
import nebulakit.models.literals as literal_models
from nebulakit import Description, Documentation, SourceCode
from nebulakit.extras.accelerators import T4
from nebulakit.models import literals, task, types
from nebulakit.models.core import identifier
...
def test_resource_entry():
    ...
    assert task.Resources.ResourceEntry.from_nebula_idl(obj.to_nebula_idl()) == obj
...
def test_resources(resource_list):
    ...
    assert obj == task.Resources.from_nebula_idl(obj.to_nebula_idl())


def test_runtime_metadata():
    obj = task.RuntimeMetadata(task.RuntimeMetadata.RuntimeType.NEBULA_SDK, "1.0.0", "python")
    assert obj.type == task.RuntimeMetadata.RuntimeType.NEBULA_SDK
    assert obj.version == "1.0.0"
    assert obj.flavor == "python"
    assert obj == task.RuntimeMetadata.from_nebula_idl(obj.to_nebula_idl())
    assert obj != task.RuntimeMetadata(task.RuntimeMetadata.RuntimeType.NEBULA_SDK, "1.0.1", "python")
    assert obj != task.RuntimeMetadata(task.RuntimeMetadata.RuntimeType.OTHER, "1.0.0", "python")
    assert obj != task.RuntimeMetadata(task.RuntimeMetadata.RuntimeType.NEBULA_SDK, "1.0.0", "golang")


def test_task_metadata_interruptible_from_nebula_idl():
    # Interruptible not set
    idl = TaskMetadata()
    obj = task.TaskMetadata.from_nebula_idl(idl)
...
def test_task_metadata():
    ...
    assert obj.runtime.type == task.RuntimeMetadata.RuntimeType.NEBULA_SDK
    ...
    assert obj == task.TaskMetadata.from_nebula_idl(obj.to_nebula_idl())
...
def test_task_template(in_tuple):
    ...
    obj = task.TaskTemplate(
        ...
        config={"a": "b"},
        extended_resources=ExtendedResources(gpu_accelerator=T4.to_nebula_idl()),
    )
    ...
    assert text_format.MessageToString(obj.to_nebula_idl()) == text_format.MessageToString(
        task.TaskTemplate.from_nebula_idl(obj.to_nebula_idl()).to_nebula_idl()
    )
...
def test_task_spec():
    ...
    source_code = SourceCode(link="https://github.com/nebulaclouds/nebulakit")
    docs = Documentation(
    ...
    assert task.TaskSpec.from_nebula_idl(obj.to_nebula_idl()) == obj
...
def test_task_template_security_context(sec_ctx):
    ...
    if sec_ctx:
        if sec_ctx.run_as is None and sec_ctx.secrets is None and sec_ctx.tokens is None:
            expected = None
    assert task.TaskTemplate.from_nebula_idl(obj.to_nebula_idl()).security_context == expected
...
def test_task_template_extended_resources(extended_resources):
    ...
    assert task.TaskTemplate.from_nebula_idl(obj.to_nebula_idl()).extended_resources == extended_resources
...
def test_task(task_closure):
    ...
    assert obj == task.Task.from_nebula_idl(obj.to_nebula_idl())
...
def test_container(resources):
    ...
    obj.env == {"a": "b"}
    obj.config == {"d": "e"}
    assert obj == task.Container.from_nebula_idl(obj.to_nebula_idl())
...
def test_dataloadingconfig():
    ...
    dlc2 = task.DataLoadingConfig.from_nebula_idl(dlc.to_nebula_idl())
    assert dlc2 == dlc
...
def test_ioconfig():
    ...
    assert io == task.IOStrategy.from_nebula_idl(io.to_nebula_idl())
...
def test_k8s_metadata():
    ...
    assert obj == task.K8sObjectMetadata.from_nebula_idl(obj.to_nebula_idl())
...
def test_k8s_pod():
    ...
    assert obj == task.K8sPod.from_nebula_idl(obj.to_nebula_idl())
