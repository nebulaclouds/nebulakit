...
from nebulaidl.plugins import dask_pb2 as dask_task

from nebulakit.models import common as common
from nebulakit.models import task as task


class Scheduler(common.NebulaIdlEntity):
    """
    ...
    def to_nebula_idl(self) -> dask_task.DaskScheduler:
        """
        :return: The scheduler spec serialized to protobuf
        """
        return dask_task.DaskScheduler(
            image=self.image,
            resources=self.resources.to_nebula_idl() if self.resources else None,
        )


class WorkerGroup(common.NebulaIdlEntity):
    """
    ...
    def to_nebula_idl(self) -> dask_task.DaskWorkerGroup:
        """
...
class DaskJob(common.NebulaIdlEntity):
    """
    ...
    def to_nebula_idl(self) -> dask_task.DaskJob:
        """
        :return: The dask job serialized to protobuf
        """
        return dask_task.DaskJob(
            scheduler=self.scheduler.to_nebula_idl(),
            workers=self.workers.to_nebula_idl(),
        )
