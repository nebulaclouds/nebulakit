...
class MyProxyConfiguration(DataClassJsonMixin):
    # File and directory paths kept as 'str' so Nebula doesn't manage these static resources
    splat_data_dir: str
    apriori_file: str
...
class MyAprioriConfiguration(DataClassJsonMixin):
    static_data_dir: NebulaDirectory
    external_data_dir: NebulaDirectory


@dataclass
class MyInput(DataClassJsonMixin):
    main_product: NebulaFile
...
def two_sample_inputs(folders_and_files_setup):
    ...
    main_product = NebulaFile(file_path)
    apriori = MyAprioriConfiguration(
        static_data_dir=NebulaDirectory(static_data_path),
        external_data_dir=NebulaDirectory(external_data_path),
    )
...
def test_two(two_sample_inputs):
    ...
    @dynamic
    def dt1(a: List[MyInput]) -> List[NebulaFile]:
        x = []
        for aa in a:
...
def test_str_input(folders_and_files_setup):
    ...
    my_input = MyInput(
        ...
        apriori_config=MyAprioriConfiguration(
            static_data_dir=NebulaDirectory("gs://my-bucket/one"),
            external_data_dir=NebulaDirectory("gs://my-bucket/two"),
        ),
        proxy_config=proxy_c,
        proxy_params=proxy_p,
    )
...
def test_dc_dyn_directory(folders_and_files_setup):
    ...
    my_input_gcs = MyInput(
        main_product=NebulaFile(folders_and_files_setup[0]),
        apriori_config=MyAprioriConfiguration(
            static_data_dir=NebulaDirectory("gs://my-bucket/one"),
            external_data_dir=NebulaDirectory("gs://my-bucket/two"),
        ),
    ...
    my_input_gcs_2 = MyInput(
        main_product=NebulaFile(folders_and_files_setup[0]),
        apriori_config=MyAprioriConfiguration(
            static_data_dir=NebulaDirectory("gs://my-bucket/three"),
            external_data_dir=NebulaDirectory("gs://my-bucket/four"),
        ),
        proxy_config=proxy_c,
        proxy_params=proxy_p,
    )
    ...
    @dynamic
    def dt1(a: List[MyInput]) -> List[NebulaDirectory]:
        x = []
        for aa in a:
