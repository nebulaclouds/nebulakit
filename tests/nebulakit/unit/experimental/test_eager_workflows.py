...
from nebulakit.core.type_engine import TypeTransformerFailedError
from nebulakit.experimental import EagerException, eager
...
@task
def create_file() -> NebulaFile:
    fname = "/tmp/nebulakit_test_file"
    with open(fname, "w") as fh:
        fh.write("some data\n")
    return NebulaFile(path=fname)


@task
def create_directory() -> NebulaDirectory:
    dirname = "/tmp/nebulakit_test_dir"
    Path(dirname).mkdir(exist_ok=True, parents=True)
    with open(os.path.join(dirname, "file"), "w") as tmp:
        tmp.write("some data\n")
    return NebulaDirectory(path=dirname)
...
def test_eager_workflow_with_offloaded_types():
    ...
    @eager
    async def eager_wf_nebula_file() -> str:
    ...
    @eager
    async def eager_wf_nebula_directory() -> str:
    ...
    result = asyncio.run(eager_wf_nebula_file())
    assert result == "some data"

    result = asyncio.run(eager_wf_nebula_directory())
    assert result == "some data"
