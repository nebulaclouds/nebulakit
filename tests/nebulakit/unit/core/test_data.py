...
from nebulakit.configuration import Config, DataConfig, S3Config
from nebulakit.core.context_manager import NebulaContextManager
from nebulakit.core.data_persistence import FileAccessProvider, get_fsspec_storage_options, s3_setup_args
...
@mock.patch("nebulakit.core.data_persistence.UUID")
def test_path_getting(mock_uuid_class, mock_gcs):
    ...
    assert local_raw_fp.get_random_local_path() == os.path.join(root, "tmp", "unittest", "local_nebulakit", "abcdef123")
    assert local_raw_fp.get_random_local_path("xjiosa/blah.txt") == os.path.join(
        root, "tmp", "unittest", "local_nebulakit", "abcdef123", "blah.txt"
    )
    assert local_raw_fp.get_random_local_directory() == os.path.join(
        root, "tmp", "unittest", "local_nebulakit", "abcdef123"
    )
...
@mock.patch("nebulakit.configuration.get_config_file")
...
def test_s3_setup_args_env_both(mock_os, mock_get_config_file):
    ...
    ee = {
        ...
        "NEBULA_AWS_ACCESS_KEY_ID": "nebula",
        "NEBULA_AWS_SECRET_ACCESS_KEY": "nebula-secret",
    }
    mock_os.get.side_effect = lambda x, y: ee.get(x)
    kwargs = s3_setup_args(S3Config.auto())
    assert kwargs == {"key": "nebula", "secret": "nebula-secret", "cache_regions": True}


@mock.patch("nebulakit.configuration.get_config_file")
@mock.patch("os.environ")
def test_s3_setup_args_env_nebula(mock_os, mock_get_config_file):
    mock_get_config_file.return_value = None
    ee = {
        "NEBULA_AWS_ACCESS_KEY_ID": "nebula",
        "NEBULA_AWS_SECRET_ACCESS_KEY": "nebula-secret",
...
def test_get_fsspec_storage_options_gcs(mock_os, mock_get_config_file):
    mock_get_config_file.return_value = None
    ee = {
        "NEBULA_GCP_GSUTIL_PARALLELISM": "False",
    }
    mock_os.get.side_effect = lambda x, y: ee.get(x)
...
def test_get_fsspec_storage_options_azure(mock_os, mock_get_config_file):
    mock_get_config_file.return_value = None
    ee = {
        "NEBULA_AZURE_STORAGE_ACCOUNT_NAME": "accountname",
        "NEBULA_AZURE_STORAGE_ACCOUNT_KEY": "accountkey",
        "NEBULA_AZURE_TENANT_ID": "tenantid",
        "NEBULA_AZURE_CLIENT_ID": "clientid",
        "NEBULA_AZURE_CLIENT_SECRET": "clientsecret",
    }
    mock_os.get.side_effect = lambda x, y: ee.get(x)
...
def test_crawl_local_nt(source_folder):
    ...
    fd = NebulaDirectory(path=source_folder)
    res = fd.crawl()
...
def test_crawl_local_non_nt(source_folder):
    ...
    fd = NebulaDirectory(path=os.path.join(source_folder, "original.txt"))
    res = fd.crawl()
    files = [os.path.join(x, y) for x, y in res]
    assert len(files) == 0
...
def test_crawl_s3(source_folder):
    ...
    with NebulaContextManager.with_context(ctx.with_file_access(provider)):
        fd = NebulaDirectory(path=s3_random_target)
        res = fd.crawl()
        ...
        fd_file = NebulaDirectory(path=f"{s3_random_target}/original.txt")
        res = fd_file.crawl()
...
def test_walk_local_copy_to_s3(source_folder):
    ...
    ctx = NebulaContextManager.current_context()
    local_fd = NebulaDirectory(path=source_folder)
