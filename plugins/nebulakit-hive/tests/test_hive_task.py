...
import pandas
import pytest
from nebulakitplugins.hive.task import HiveConfig, HiveSelectTask, HiveTask
...
from nebulakit.testing import task_mock
from nebulakit.types.schema import NebulaSchema


def test_serialization():
    hive_task = HiveTask(
        name="nebulakit.demo.hive_task.hivequery1",
        inputs=kwtypes(my_schema=NebulaSchema, ds=str),
        config=HiveConfig(cluster_label="nebula"),
        query_template="""
            set engine=tez;
    ...
    @workflow
    def my_wf(in_schema: NebulaSchema, ds: str) -> NebulaSchema:
