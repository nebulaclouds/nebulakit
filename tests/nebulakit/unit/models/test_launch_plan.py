from nebulaidl.admin import launch_plan_pb2 as _launch_plan_idl

from nebulakit.models import common, interface, launch_plan, literals, schedule, types
from nebulakit.models.core import identifier


def test_metadata():
    obj = launch_plan.LaunchPlanMetadata(schedule=None, notifications=[])
    obj2 = launch_plan.LaunchPlanMetadata.from_nebula_idl(obj.to_nebula_idl())
    assert obj == obj2
...
def test_lp_closure():
    ...
    obj2 = launch_plan.LaunchPlanClosure.from_nebula_idl(obj.to_nebula_idl())
    assert obj == obj2
...
def test_launch_plan_spec():
    ...
    obj2 = launch_plan.LaunchPlanSpec.from_nebula_idl(lp_spec_raw_output_prefixed.to_nebula_idl())
    ...
    obj2 = launch_plan.LaunchPlanSpec.from_nebula_idl(lp_spec_no_prefix.to_nebula_idl())
...
def test_old_style_role():
    ...
    old_style_spec = _launch_plan_idl.LaunchPlanSpec(
        workflow_id=identifier_model.to_nebula_idl(),
        entity_metadata=launch_plan_metadata_model.to_nebula_idl(),
        default_inputs=parameter_map.to_nebula_idl(),
        fixed_inputs=fixed_inputs.to_nebula_idl(),
        labels=labels_model.to_nebula_idl(),
        annotations=annotations_model.to_nebula_idl(),
        raw_output_data_config=raw_data_output_config.to_nebula_idl(),
        auth=old_role,
    )

    lp_spec = launch_plan.LaunchPlanSpec.from_nebula_idl(old_style_spec)
