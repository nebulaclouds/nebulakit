...
from nebulakit.models.core import errors as _error_model


class NebulaScopedException(Exception):
    def __init__(self, context, exc_type, exc_value, exc_tb, top_trim=0, bottom_trim=0, kind=None):
        ...
        self._kind = kind
        super(NebulaScopedException, self).__init__(str(self.value))
    ...
    @property
    def value(self):
        if isinstance(self._exc_value, NebulaScopedException):
    ...
    def kind(self) -> int:
        ...
        if self._kind is not None:
            ...
            return self._kind
        elif isinstance(self._exc_value, NebulaScopedException):
            # Otherwise, go lower in the scope to find the kind of exception.
            return self._exc_value.kind
        elif isinstance(self._exc_value, _base_exceptions.NebulaRecoverableException):
...
class NebulaScopedSystemException(NebulaScopedException):
    def __init__(self, exc_type, exc_value, exc_tb, **kwargs):
        super(NebulaScopedSystemException, self).__init__("SYSTEM", exc_type, exc_value, exc_tb, **kwargs)
    ...
    def verbose_message(self):
        ...
        :rtype: Text
        """
        base_msg = super(NebulaScopedSystemException, self).verbose_message
        base_msg += "\n\nSYSTEM ERROR! Contact platform administrators."
        return base_msg


class NebulaScopedUserException(NebulaScopedException):
    def __init__(self, exc_type, exc_value, exc_tb, **kwargs):
        super(NebulaScopedUserException, self).__init__("USER", exc_type, exc_value, exc_tb, **kwargs)
    ...
    def verbose_message(self):
        ...
        :rtype: Text
        """
        base_msg = super(NebulaScopedUserException, self).verbose_message
        base_msg += "\n\nUser error."
        return base_msg
...
def system_entry_point(wrapped, args, kwargs):
    """
    The reason these two (see the user one below) decorators exist is to categorize non-Nebula exceptions at arbitrary
    locations. For example, while there is a separate ecosystem of Nebula-defined user and system exceptions
    (see the NebulaException hierarchy), and we can easily understand and categorize those, if nebulakit comes upon
    a random ``ValueError`` or other non-nebulakit defined error, how would we know if it was a bug in nebulakit versus an
    ...
    try:
        ...
        if _is_base_context():
            ...
            except NebulaScopedException as ex:
                raise ex.value
        else:
            try:
                return wrapped(*args, **kwargs)
            except NebulaScopedException as scoped:
                raise scoped
            except _user_exceptions.NebulaUserException:
                # Re-raise from here.
                raise NebulaScopedUserException(*_exc_info())
            except Exception:
                ...
                raise NebulaScopedSystemException(*_exc_info(), kind=_error_model.ContainerError.Kind.RECOVERABLE)
    finally:
        _CONTEXT_STACK.pop()
...
def user_entry_point(wrapped, args, kwargs):
    ...
    try:
        ...
        if _is_base_context():
            ...
            try:
                return wrapped(*args, **kwargs)
            except NebulaScopedException as exc:
            ...
            try:
                ...
                raise NebulaScopedUserException(*_exc_info())
            except _system_exceptions.NebulaSystemException:
                raise NebulaScopedSystemException(*_exc_info())
            except Exception:
                # This is why this function exists - arbitrary exceptions that we don't know what to do with are
                # interpreted as user exceptions.
                # This will also catch NebulaUserException re-raised by the system_entry_point handler
                raise NebulaScopedUserException(*_exc_info())
    finally:
        _CONTEXT_STACK.pop()
