...
from nebulakit.extras.accelerators import A100, T4
...
class TestAccelerators:
    def test_gpu_accelerator(self):
        ...
        ts = get_serializable(OrderedDict(), serialization_settings, needs_t4).to_nebula_idl()
    ...
    def test_mig(self):
        ...
        ts = get_serializable(OrderedDict(), serialization_settings, needs_a100).to_nebula_idl()
        gpu_accelerator = ts.template.extended_resources.gpu_accelerator
        assert gpu_accelerator is not None
    ...
    def test_mig_unpartitioned(self):
        ...
        ts = get_serializable(OrderedDict(), serialization_settings, needs_unpartitioned_a100).to_nebula_idl()
        gpu_accelerator = ts.template.extended_resources.gpu_accelerator
        assert gpu_accelerator is not None
        assert gpu_accelerator.device == "nvidia-tesla-a100"
    ...
    def test_mig_partitioned(self):
        ...
        ts = get_serializable(OrderedDict(), serialization_settings, needs_partitioned_a100).to_nebula_idl()
        gpu_accelerator = ts.template.extended_resources.gpu_accelerator
        assert gpu_accelerator is not None
        assert gpu_accelerator.device == "nvidia-tesla-a100"
