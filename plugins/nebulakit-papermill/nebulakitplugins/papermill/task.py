...
from nebulaidl.core.literals_pb2 import Literal as _pb2_Literal
from nebulaidl.core.literals_pb2 import LiteralMap as _pb2_LiteralMap
from google.protobuf import text_format as _text_format
from nbconvert import HTMLExporter

from nebulakit import NebulaContext, PythonInstanceTask, StructuredDataset
from nebulakit.configuration import SerializationSettings
from nebulakit.core import utils
from nebulakit.core.context_manager import ExecutionParameters
from nebulakit.core.tracker import extract_task_module
from nebulakit.deck.deck import Deck
from nebulakit.extend import Interface, TaskPlugins, TypeEngine
...
from nebulakit.types.file import NebulaFile, HTMLPage, PythonNotebook
...
SAVE_AS_LITERAL = (NebulaFile, NebulaDirectory, StructuredDataset)
...
class NotebookTask(PythonInstanceTask[T]):
    ...
    cell. The task will inject the outputs observed from Nebula
    ...
    #. It captures the executed notebook in its entirety and is available from Nebula with the name ``out_nb``.
    #. It also converts the captured notebook into an ``html`` page, which the NebulaConsole will render called -
    ...
    .. note:
        ...
        str, int, float, bool
        Most output types are supported as long as NebulaFile etc is used.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # tasks should not be serialized at all, but we don't currently have a mechanism for skipping Nebula entities
        ...
        # injecting a logger, so we cannot redirect logs to the nebula child loggers (e.g., the userspace logger)
                ...
                if "outputs" in meta["tags"]:
                    ...
                    for record in p["outputs"]:
                        if "data" in record:
                            ...
                            return LiteralMap.from_nebula_idl(m)
        return None
    ...
    def execute(self, **kwargs) -> Any:
        """
        TODO: Figure out how to share NebulaContext ExecutionParameters with the notebook kernel (as notebook kernel
        ...
        for k, type_v in self.python_interface.outputs.items():
            if k == self._IMPLICIT_OP_NOTEBOOK:
                ...
                v = TypeEngine.to_python_value(ctx=NebulaContext.current_context(), lv=m[k], expected_python_type=type_v)
...
def record_outputs(**kwargs) -> str:
    ...
    It will convert all outputs to a Nebula understandable format. For Files, Directories, please use NebulaFile or
    NebulaDirectory, or wrap up your paths in these decorators.
    """
    if kwargs is None:
        return ""
    ...
    for k, v in kwargs.items():
        ...
        m[k] = lit
    return LiteralMap(literals=m).to_nebula_idl()


def save_python_val_to_file(input: Any) -> str:
    """Save a python value to a local file as a Nebula literal.
    ...
    utils.write_proto_to_file(lit.to_nebula_idl(), tmp_file)
    return tmp_file


def load_python_val_from_file(path: str, dtype: T) -> T:
    """Loads a python value from a Nebula literal saved to a local file.
    ...
    lit = Literal.from_nebula_idl(proto)
    ctx = NebulaContext.current_context()
    python_value = TypeEngine.to_python_value(ctx, lit, dtype)
    return python_value


def load_nebulafile(path: str) -> T:
    """Loads a NebulaFile from a file.
    ...
    Returns:
        T: the python value of the literal
    """
    return load_python_val_from_file(path=path, dtype=NebulaFile)


def load_nebuladirectory(path: str) -> T:
    """Loads a NebulaDirectory from a file.
    ...
    Returns:
        T: the python value of the literal
    """
    return load_python_val_from_file(path=path, dtype=NebulaDirectory)
