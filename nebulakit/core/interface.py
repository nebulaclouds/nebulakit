...
from nebulakit.exceptions.user import NebulaValidationException
from nebulakit.loggers import logger
from nebulakit.models import interface as _interface_models
from nebulakit.models.literals import Void
...
class Interface(object):
    ...
    def remove_inputs(self, vars: Optional[List[str]]) -> Interface:
        """
        This method is useful in removing some variables from the Nebula backend inputs specification, as these are
...
def transform_inputs_to_parameters(
    ctx: context_manager.NebulaContext, interface: Interface
    ...
    for k, v in inputs_vars.items():
        ...
        if _default is None and get_origin(val) is typing.Union and type(None) in get_args(val):
            from nebulakit import Literal, Scalar
...
def transform_interface_to_typed_interface(
    ...
    """
    Transform the given simple python native interface to NebulaIDL's interface
    """
...
def transform_variable_map(
    ...
    Nebula Variable object with that type.
    """
    res = OrderedDict()
    if variable_map:
...
def extract_return_annotation(return_annotation: Union[Type, Tuple, None]) -> Dict[str, Type]:
    ...
    if hasattr(return_annotation, "__origin__") and return_annotation.__origin__ is tuple:  # type: ignore
        ...
        if len(return_annotation) == 1:
            raise NebulaValidationException("Please don't use a tuple if you're just returning one thing.")
