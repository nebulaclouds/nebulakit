...
import pytest
from nebulakitplugins.awssagemaker.models import parameter_ranges
...
def test_continuous_parameter_range():
    ...
    pr2 = parameter_ranges.ContinuousParameterRange.from_nebula_idl(pr.to_nebula_idl())
    assert pr == pr2
...
def test_integer_parameter_range():
    ...
    pr2 = parameter_ranges.IntegerParameterRange.from_nebula_idl(pr.to_nebula_idl())
    assert pr == pr2
...
def test_categorical_parameter_range():
    ...
    pr2 = parameter_ranges.CategoricalParameterRange.from_nebula_idl(pr.to_nebula_idl())
    assert pr == pr2
...
def test_parameter_ranges():
    ...
    pr2 = parameter_ranges.ParameterRanges.from_nebula_idl(pr.to_nebula_idl())
    assert pr == pr2
...
def test_parameter_ranges_oneof(param_tuple):
    ...
    oneof2 = parameter_ranges.ParameterRangeOneOf.from_nebula_idl(oneof.to_nebula_idl())
    assert oneof2 == oneof
    assertion(oneof)
    assertion(oneof2)
