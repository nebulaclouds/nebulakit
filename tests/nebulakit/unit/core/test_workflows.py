...
from nebulakit import NebulaContextManager, StructuredDataset, kwtypes
...
from nebulakit.core.workflow import WorkflowFailurePolicy, WorkflowMetadata, WorkflowMetadataDefaults, workflow
...
def test_metadata_values():
    with pytest.raises(NebulaValidationException):
...
def test_unexpected_outputs():
    ...
    with pytest.raises(NebulaValueException):
        no_outputs_wf()
...
def test_wf_nested_comp():
    ...
    assert sub_wf.nodes[0].task_node.reference_id.name == f"{prefix}tests.nebulakit.unit.core.test_workflows.t1"
...
def test_all_node_types():
    ...
    assert len(sub_wf.nodes) == 1
    assert sub_wf.nodes[0].id == "n0"
    assert sub_wf.nodes[0].task_node.reference_id.name == "tests.nebulakit.unit.core.test_workflows.add_5"
...
@task
def t3(df: NebulaSchema[superset_cols]) -> NebulaSchema[superset_cols]:
    return df


@task
def t4() -> NebulaSchema[superset_cols]:
    return superset_df
