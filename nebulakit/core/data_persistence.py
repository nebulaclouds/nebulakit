"""
======================================
:mod:`nebulakit.core.data_persistence`
======================================

.. currentmodule:: nebulakit.core.data_persistence

The Data persistence module is used by core nebulakit and most of the core TypeTransformers to manage data fetch & store,
...
from nebulakit.configuration import DataConfig
from nebulakit.core.local_fsspec import NebulaLocalFileSystem
from nebulakit.core.utils import timeit
from nebulakit.exceptions.user import NebulaAssertion
from nebulakit.interfaces.random import random
...
class FileAccessProvider(object):
    """
    This is the class that is available through the NebulaContext and can be used for persisting data to the remote
    durable store.
    """
    ...
    def __init__(
        ...
        local_sandbox_dir_appended = os.path.join(local_sandbox_dir, "local_nebulakit")
        ...
        if os.name == "nt" and raw_output_prefix.startswith("file://"):
            raise NebulaAssertion("Cannot use the file:// prefix on Windows.")
    ...
    def get_filesystem(
        ...
        if protocol == "file":
            ...
            return NebulaLocalFileSystem(**kwargs)
        elif protocol == "s3":
    ...
    def put(self, from_path: str, to_path: str, recursive: bool = False, **kwargs):
        ...
        if recursive:
            ...
            if file_system.protocol == "file" and not file_system.isdir(from_path):
                raise NebulaAssertion(f"Source path {from_path} is not a directory")
    ...
    def put_raw_data(
        ...
        **kwargs,
        ...
        if isinstance(lpath, str) or isinstance(lpath, os.PathLike) or isinstance(lpath, pathlib.Path):
            ...
            from_path = str(lpath)
            if not p.exists():
                raise NebulaAssertion(f"File {from_path} does not exist")
            elif p.is_symlink():
                raise NebulaAssertion(f"File {from_path} is a symlink, can't upload")
            if p.is_dir():
        ...
        if isinstance(lpath, io.BufferedReader) or isinstance(lpath, io.BytesIO):
            if not lpath.readable():
                raise NebulaAssertion("Buffered reader must be readable")
            fs = self.get_filesystem_for_path(to_path)
            lpath.seek(0)
            with fs.open(to_path, "wb", **kwargs) as s:
        ...
        raise NebulaAssertion(f"Unsupported lpath type {type(lpath)}")
    ...
    def join(
        ...
        *args: Unpack[str],  # type: ignore
        ...
        # todo add a check here for nebula fs
    ...
    def put_data(
        ...
        try:
            ...
            with timeit(f"Upload data to {remote_path}"):
                ...
                # Callers of this put_data function in nebulakit have been changed to assign the remote path to the
                # output
                # of this function, so we want to make sure we don't change it unless we need to.
                if remote_path.startswith("nebula://"):
                    return put_result
                return remote_path
        except Exception as ex:
            raise NebulaAssertion(
...
nebula_tmp_dir = tempfile.mkdtemp(prefix="nebula-")
default_local_file_access_provider = FileAccessProvider(
    local_sandbox_dir=os.path.join(nebula_tmp_dir, "sandbox"),
    raw_output_prefix=os.path.join(nebula_tmp_dir, "raw"),
