...
def create_recoverable_error_file() -> None:
    ...
    the string `NebulaRecoverableException` - which would not detect exceptions inheriting from
    `NebulaRecoverableException` - we create a file in the worker process to signal to the agent process
