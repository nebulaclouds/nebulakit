...
from nebulakit import configuration as _config
from nebulakit.clis.sdk_in_container import constants as _constants
from nebulakit.clis.sdk_in_container import pynebula as _pynebula
from nebulakit.tools import module_loader as _module_loader
...
def mock_ctx(request):
    with _config.TemporaryConfiguration(request.param):
        ...
        try:
            with _mock.patch("nebulakit.tools.module_loader.iterate_modules") as mock_module_load:
...
def mock_clirunner(monkeypatch):
    def f(*args, **kwargs):
        ...
        result = runner.invoke(_pynebula.main, base_args + list(args), **kwargs)
