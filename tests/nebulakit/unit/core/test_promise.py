...
from nebulakit import LaunchPlan, task, workflow
from nebulakit.core import context_manager
from nebulakit.core.context_manager import CompilationState, NebulaContextManager
...
from nebulakit.exceptions.user import NebulaAssertion, NebulaPromiseAttributeResolveException
...
def test_create_and_link_node():
    ...
    with pytest.raises(NebulaAssertion, match="Cannot create node when not compiling..."):
        ctx = context_manager.NebulaContext.current_context()
        create_and_link_node(ctx, t1, a=3)

    ctx = context_manager.NebulaContext.current_context().with_compilation_state(CompilationState(prefix=""))
...
def test_create_and_link_node_from_remote_ignore():
    ...
    # which is incorrect
    with pytest.raises(NebulaAssertion, match="Missing input `i` type `<NebulaLiteral simple: INTEGER>`"):
    ...
    with pytest.raises(
        NebulaAssertion, match="ixed inputs cannot be specified. Please remove the following inputs - {'i'}"
    ):
...
def test_resolve_attr_path_in_promise():
    ...
    src_lit = TypeEngine.to_literal(
        NebulaContextManager.current_context(),
        src,
        Dict[str, List[Foo]],
    ...
    assert "foo" == TypeEngine.to_python_value(NebulaContextManager.current_context(), tgt_promise.val, str)

    # exception
    with pytest.raises(NebulaPromiseAttributeResolveException):
