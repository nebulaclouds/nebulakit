...
from nebulakit.clis.helpers import display_help_with_error
from nebulakit.clis.sdk_in_container import constants
from nebulakit.clis.sdk_in_container.helpers import get_and_save_remote_with_click_context, patch_image_config
from nebulakit.clis.sdk_in_container.utils import domain_option_dec, project_option_dec
...
from nebulakit.tools import repo

_register_help = """
This command is similar to ``package`` but instead of producing a zip file, all your Nebula entities are compiled,
and then sent to the backend specified by your config file. Think of this as combining the ``pynebula package``
and the ``nebulactl register`` steps in one command. This is why you see switches you'd normally use with nebulactl
...
``pynebula run``, tasks registered from this command will download and unzip that code package before running.
...
def register(
    ...
    click.secho(
        f"Running pynebula register from {os.getcwd()} "
    ...
    # Create and save NebulaRemote,
    remote = get_and_save_remote_with_click_context(ctx, project, domain, data_upload_location="nebula://data")
