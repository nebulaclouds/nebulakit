...
from nebulakit.clis.sdk_in_container.constants import CTX_VERBOSE
from nebulakit.exceptions.base import NebulaException
from nebulakit.exceptions.user import NebulaInvalidInputException
...
project_option = click.Option(
    ...
    required=False,
    type=str,
    default=os.getenv("NEBULA_DEFAULT_PROJECT", "nebulasnacks"),
    show_default=True,
    help="Project to register and run this workflow in. Can also be set through envvar " "``NEBULA_DEFAULT_PROJECT``",
)
...
domain_option = click.Option(
    ...
    required=False,
    type=str,
    default=os.getenv("NEBULA_DEFAULT_DOMAIN", "development"),
    show_default=True,
    help="Domain to register and run this workflow in, can also be set through envvar " "``NEBULA_DEFAULT_DOMAIN``",
)
...
project_option_dec = click.option(
    ...
    default=os.getenv("NEBULA_DEFAULT_PROJECT", "nebulasnacks"),
    show_default=True,
    help="Project for workflow/launchplan. Can also be set through envvar " "``NEBULA_DEFAULT_PROJECT``",
)
...
domain_option_dec = click.option(
    ...
    default=os.getenv("NEBULA_DEFAULT_DOMAIN", "development"),
    show_default=True,
    help="Domain for workflow/launchplan, can also be set through envvar " "``NEBULA_DEFAULT_DOMAIN``",
)
...
def pretty_print_exception(e: Exception):
    ...
    if isinstance(e, NebulaException):
        click.secho(f"Failed with Exception Code: {e._ERROR_CODE}", fg="red")  # noqa
        if isinstance(e, NebulaInvalidInputException):
...
@dataclass
class PyNebulaParams:
    config_file: typing.Optional[str] = None
    verbose: bool = False
    pkgs: typing.List[str] = field(default_factory=list)

    @classmethod
    def from_dict(cls, d: typing.Dict[str, typing.Any]) -> "PyNebulaParams":
        return cls(**d)
