...
class AWSBatchFunctionTask(PythonFunctionTask):
    ...
    def get_command(self, settings: SerializationSettings) -> List[str]:
        container_args = [
            ...
            "{{.input}}",
            "--output-prefix",
            # As of NebulaPropeller v0.16.28, aws array batch plugin support to run single job.
            # This task will call aws batch plugin to execute the task on aws batch service.
            # For single job, NebulaPropeller will always read the output from this directory (outputPrefix/0)
            # More detail, see https://github.com/nebulaclouds/nebulaplugins/blob/0dd93c23ed2edeca65d58e89b0edb613f88120e0/go/tasks/plugins/array/catalog.go#L501.
            "{{.outputPrefix}}/0",
...
# Inject the AWS batch plugin into nebulakits dynamic plugin loading system
