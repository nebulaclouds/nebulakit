...
See ``plugins/nebulakit-kf-pytorch/nebulakitplugins/kfpytorch/models.py`` as an example.
"""

from nebulaidl.plugins import qubole_pb2 as _qubole

from nebulakit.models import common as _common


class HiveQuery(_common.NebulaIdlEntity):
...
class HiveQueryCollection(_common.NebulaIdlEntity):
    ...
    def to_nebula_idl(self):
        ...
        return _qubole.HiveQueryCollection(
            queries=[query.to_nebula_idl() for query in self.queries] if self.queries else None
        )
    ...
    def from_nebula_idl(cls, pb2_object):
        ...
        """
        return cls(queries=[HiveQuery.from_nebula_idl(query) for query in pb2_object.queries])


class QuboleHiveJob(_common.NebulaIdlEntity):
    ...
    def to_nebula_idl(self):
        ...
        return _qubole.QuboleHiveJob(
            query_collection=self._query_collection.to_nebula_idl() if self._query_collection else None,
            query=self._query.to_nebula_idl() if self._query else None,
    ...
    def from_nebula_idl(cls, p):
        ...
        """
        return cls(
            query_collection=HiveQueryCollection.from_nebula_idl(p.query_collection)
            if p.HasField("query_collection")
            else None,
            query=HiveQuery.from_nebula_idl(p.query) if p.HasField("query") else None,
