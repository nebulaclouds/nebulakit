"""
=========================================
:mod:`nebulakit.core.python_function_task`
=========================================

.. currentmodule:: nebulakit.core.python_function_task
...
from nebulakit.core.interface import transform_function_to_interface
from nebulakit.core.promise import VoidPromise, translate_inputs_to_literals
from nebulakit.core.python_auto_container import PythonAutoContainerTask, default_task_resolver
from nebulakit.core.tracker import extract_task_module, is_functools_wrapped_module_level, isnested, istestfunction
from nebulakit.core.workflow import (
...
from nebulakit.exceptions.user import NebulaValueException
...
class PythonFunctionTask(PythonAutoContainerTask[T]):  # type: ignore
    ...
    automatically detect interface of the python function and when serialized on the hosted Nebula platform handles the
        ...
        if self._task_resolver is default_task_resolver:
            ...
            if (
                ...
                raise ValueError(
                    ...
                    "It should be accessible at a module level for Nebula to execute it. Test modules with "
    ...
    def execute(self, **kwargs) -> Any:
        ...
        if self.execution_mode == self.ExecutionBehavior.DEFAULT:
            ...
            # has access to the NebulaContext.
            kwargs["async_ctx"] = NebulaContextManager.current_context()
            return exception_scopes.user_entry_point(self._task_function)(**kwargs)
    ...
    def compile_into_workflow(
        self, ctx: NebulaContext, task_function: Callable, **kwargs
        ...
        with NebulaContextManager.with_context(updated_ctx):
            ...
            # This is the only circular dependency between the translator.py module and the rest of the nebulakit
            ...
            for entity, model in model_entities.items():
                ...
                # require a network call to nebulaadmin to populate the TaskTemplate
                # model
    ...
    def dynamic_execute(self, task_function: Callable, **kwargs) -> Any:
        """
        By the time this function is invoked, the local_execute function should have unwrapped the Promises and Nebula
