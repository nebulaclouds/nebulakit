...
from nebulakit.core.utils import _get_container_definition, load_proto_from_file
from nebulakit.loggers import logger
from nebulakit.models import task as _task_model
from nebulakit.models.core import identifier as identifier_models
...
class PythonCustomizedContainerTask(ExecutableTemplateShimTask, PythonTask[TC]):  # type: ignore
    """
    Please take a look at the comments for :py:class`nebulakit.extend.ExecutableTemplateShimTask` as well. This class
    should be subclassed and a custom Executor provided as a default to this parent class constructor
    when building a new external-container nebulakit-only plugin.
    ...
    * Also pass the executor you created as the ``executor_type`` argument of this class's constructor.
    ...
    frequently by the Nebula engine.
    """
    ...
    SERIALIZE_SETTINGS = SerializationSettings(
        ...
        image_config=ImageConfig(
            default_image=Image(name="custom_container_task", fqn="nebulaclouds.io/placeholder", tag="image")
        ),
    )
    ...
    def serialize_to_model(self, settings: SerializationSettings) -> _task_model.TaskTemplate:
        ...
        # TODO: After new control plane classes are in, promote the template to a NebulaTask, so that authors of
...
class TaskTemplateResolver(TrackedInstance, TaskResolverMixin):
    ...
    * all tasks will result in the same list of strings for a given subclass of the ``ShimTaskExecutor``
    ...
    def load_task(self, loader_args: List[str]) -> ExecutableTemplateShimTask:  # type: ignore
        ...
        task_template_model = _task_model.TaskTemplate.from_nebula_idl(task_template_proto)
