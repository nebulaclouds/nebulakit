...
from nebulakit.core.base_task import TaskMetadata, TaskResolverMixin
...
from nebulakit.core.reference_entity import ReferenceEntity, TaskReference
from nebulakit.core.resources import Resources
from nebulakit.extras.accelerators import BaseAccelerator
...
class TaskPlugins(object):
    ...
    :py:class:`nebulakitplugins.athena.task.AthenaTask` and :py:class:`nebulakitplugins.hive.task.HiveTask`, ML tools like
    :py:class:`plugins.awssagemaker.nebulakitplugins.awssagemaker.training.SagemakerBuiltinAlgorithmsTask`, kubeflow
    operators like :py:class:`plugins.kfpytorch.nebulakitplugins.kfpytorch.task.PyTorchFunctionTask` and
    :py:class:`plugins.kftensorflow.nebulakitplugins.kftensorflow.task.TensorflowFunctionTask`, and generic plugins like
    :py:class:`nebulakitplugins.pod.task.PodFunctionTask` which doesn't integrate with third party tools or services.
    ...
    def register_pythontask_plugin(cls, plugin_config_type: type, plugin: Type[PythonFunctionTask]):
        """
        Use this method to register a new plugin into Nebulakit. Usage ::
...
def task(
    ...
    """
    This is the core decorator to use for any task type in nebulakit.

    Tasks are the building blocks of Nebula. They represent users code. Tasks have the following properties
    ...
    * Unit testable
    ...
    :param interruptible: [Optional] Boolean that indicates that this task can be interrupted and/or scheduled on nodes
                          ...
                          Nebula platform level configuration. If no value is provided, the task will inherit this
    ...
    :param container_image: By default the configured NEBULA_INTERNAL_IMAGE is used for every task. This directive can be
                ...
                .. code-block:: python
                    ...
                    # nebulacookbook:tag-gitsha is published alongwith the default of nebulacookbook:gitsha
    ...
    :param limits: Compute limits. Specify compute resource limits for your task. For Pod-plugin tasks, these values
      will apply only to the primary container. For more information, please see :py:class:`nebulakit.Resources`.
...
class ReferenceTask(ReferenceEntity, PythonFunctionTask):  # type: ignore
    ...
    as stored by Nebula Admin, if not, workflows using this will break upon compilation.
    """
...
def reference_task(
    ...
    """
    A reference task is a pointer to a task that already exists on your Nebula installation. This
