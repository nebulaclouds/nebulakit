...
.. currentmodule:: nebulakit.configuration

Nebulakit Configuration Sources
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

There are multiple ways to configure nebulakit settings:

**Command Line Arguments**: This is the recommended way of setting configuration values for many cases.
For example, see `pynebula package <pynebula.html#pynebula-package>`_ command.

**Python Config Object**: A :py:class:`~nebulakit.configuration.Config` object can by used directly, e.g. when
initializing a :py:class:`~nebulafit.remote.remote.NebulaRemote` object. See :doc:`here <design/control_plane>` for examples on
how to specify a ``Config`` object.

**Environment Variables**: Users can specify these at compile time, but when your task is run, Nebula Propeller will
also set configuration to ensure correct interaction with the platform. The environment variables must be specified
with the format ``NEBULA_{SECTION}_{OPTION}``, all in upper case. For example, to specify the
:py:class:`PlatformConfig.endpoint <nebulakit.configuration.PlatformConfig>` setting, the environment variable would
be ``NEBULA_PLATFORM_URL``.

.. note::

   Environment variables won't work for image configuration, which need to be specified with the
   `pynebula package --image ... <pynebula.html#cmdoption-pynebula-package-i>`_ option or in a configuration
   file.

**YAML Format Configuration File**: A configuration file that contains settings for both
`nebulactl <https://docs.nebula.org/projects/nebulactl/>`__ and ``nebulakit``. This is the recommended configuration
file format. Invoke the :ref:`nebulactl config init <nebulactl_config_init>` command to create a boilerplate
``~/.nebula/config.yaml`` file, and  ``nebulactl --help`` to learn about all of the configuration yaml options.

.. dropdown:: See example ``config.yaml`` file
   :title: text-muted
   :animate: fade-in-slide-down

   .. literalinclude:: ../../tests/nebulakit/unit/configuration/configs/sample.yaml
      :language: yaml
      :caption: config.yaml

**INI Format Configuration File**: A configuration file for ``nebulakit``. By default, ``nebulakit`` will look for a
file in two places:

1. First, a file named ``nebulakit.config`` in the Python interpreter's working directory.
2. A file in ``~/.nebula/config`` in the home directory as detected by Python.

.. dropdown:: See example ``nebulakit.config`` file
   :title: text-muted
   :animate: fade-in-slide-down

   .. literalinclude:: ../../tests/nebulakit/unit/configuration/configs/images.config
      :language: ini
      :caption: nebulakit.config
...
- **Compile-time settings**: these are settings like the default image and named images, where to look for Nebula code, etc.
- **Platform settings**: Where to find the Nebula backend (Admin DNS, whether to use SSL)
...
- **Other settings** - Statsd configuration, which is a run-time applicable setting but is not necessarily relevant to the Nebula platform.
...
`pynebula package <pynebula.html#pynebula-package>`_ or `pynebula register <pynebula.html#pynebula-register>`_. These
configuration settings are typically passed in as flags to the above CLI commands.

The image configurations are typically either passed in via an `--image <pynebula.html#cmdoption-pynebula-package-i>`_ flag,
...
Users typically shouldn't be concerned with these configurations, as they are typically set by NebulaPropeller or
NebulaAdmin. The configurations below are useful for authenticating to a Nebula backend, configuring data access
...
from nebulakit.configuration.file import ConfigEntry, ConfigFile, get_config_file, read_file_if_exists, set_if_exists
...
DEFAULT_NEBULAKIT_ENTRYPOINT_FILELOC = "bin/entrypoint.py"
...
class Image(DataClassJsonMixin):
    ...
    def look_up_image_info(name: str, tag: str, optional_tag: bool = False) -> Image:
        ...
        Looks up the image tag from environment variable (should be set from the Dockerfile).
            NEBULA_INTERNAL_IMAGE should be the environment variable.
...
class ImageConfig(DataClassJsonMixin):
    ...
    For example, ImageConfig.auto(img_name=""ghcr.io/nebulaclouds/nebulacookbook:v1.0.0"") will create an ImageConfig.
    ...
    def validate_image(_: typing.Any, param: str, values: tuple) -> ImageConfig:
        ...
        if default_image is None:
            default_image_str = os.environ.get("NEBULA_INTERNAL_IMAGE", DefaultImages.default_image())
    ...
    def auto(
        ...
        Note that this function does not take into account the nebulakit default images (see the Dockerfiles at the
        ...
        """
            ...
            .. code:: python
              ...
              ImageConfig.from_dict(
                  "ghcr.io/nebulaclouds/nebulacookbook:v1.0.0",
                   {
                        "spark": "ghcr.io/nebulaclouds/myspark:...",
                        "other": "...",
                   }
              )
...
class AuthType(enum.Enum):
    ...
    # The following values are copied from nebulaidl's admin client to align the two code bases on the same enum values.
...
class PlatformConfig(object):
    """
    This object contains the settings to talk to a Nebula backend (the DNS location of your Admin server basically).

    :param endpoint: DNS for Nebula backend
    :param insecure: Whether or not to use SSL
    :param insecure_skip_verify: Whether to skip SSL certificate verification
    :param console_endpoint: endpoint for console if different from Nebula backend
    ...
    :param client_id: This is the public identifier for the app which handles authorization for a Nebula deployment.
      More details here: https://www.oauth.com/oauth2-servers/client-registration/client-id-secret/.
    :param client_credentials_secret: Used for service auth, which is automatically called during pynebula. This will
      allow the Nebula engine to read the password directly from the environment variable. Note that this is
...
class DataConfig(object):
    ...
    Nebula sandbox environment we store the access key id and secret.
...
class Config(object):
    ...
    1. Interactive session with Nebula backend
    2. Some parts are required for Serialization, for example Platform Config is not required
    3. Runtime of a task

    Args:
        entrypoint_settings: EntrypointSettings object for use with Spark tasks. If supplied, this will be
          used when serializing Spark tasks, which need to know the path to the nebulakit entrypoint.py file,
    ...
    local_sandbox_path: str = tempfile.mkdtemp(prefix="nebula")
    ...
    def auto(cls, config_file: typing.Union[str, ConfigFile, None] = None) -> Config:
        ...
        Automatically constructs the Config Object. The order of precedence is as follows
          1. first try to find any env vars that match the config vars specified in the NEBULA_CONFIG format.
    ...
    def for_endpoint(
        ...
        :param endpoint: -> Endpoint where Nebula admin is available
        ...
        :param config_file: -> Optional config file in the nebulakit config format.
        :return: Config
        """
...
class EntrypointSettings(DataClassJsonMixin):
    ...
    This is where `pynebula-execute` code can be found. This is useful for cases like pyspark execution.
    """
...
class SerializationSettings(DataClassJsonMixin):
    ...
    Attributes:
        ...
        nebulakit_virtualenv_root (Optional[str]):  During out of container serialize the absolute path of the nebulakit
    ...
    nebulakit_virtualenv_root: Optional[str] = None
    fast_serialization_settings: Optional[FastSerializationSettings] = None
    source_root: Optional[str] = None

    def __post_init__(self):
        if self.nebulakit_virtualenv_root is None:
            self.nebulakit_virtualenv_root = self.venv_root_from_interpreter(self.python_interpreter)
    ...
    def entrypoint_settings(self) -> EntrypointSettings:
        return EntrypointSettings(
            path=os.path.join(
                ...
                DEFAULT_NEBULAKIT_ENTRYPOINT_FILELOC,
            )
        )
    ...
    def for_image(
        ...
        return SerializationSettings(
            ...
            nebulakit_virtualenv_root=cls.venv_root_from_interpreter(python_interpreter_path),
        )
    ...
    def default_entrypoint_settings(interpreter_path: str) -> EntrypointSettings:
        ...
        return EntrypointSettings(
            path=os.path.join(
                SerializationSettings.venv_root_from_interpreter(interpreter_path), DEFAULT_NEBULAKIT_ENTRYPOINT_FILELOC
            )
        )
    ...
    def new_builder(self) -> Builder:
        ...
        return SerializationSettings.Builder(
            ...
            nebulakit_virtualenv_root=self.nebulakit_virtualenv_root,
