...
class Pod(object):
    ...
    :param str primary_container_name: the primary container name. If provided the pod-spec can contain a container whose name matches the primary_container_name. This will force Nebula to give up control of the primary
        container and will expect users to control setting up the container. If you expect your python function to run as is, simply create containers that do not match the default primary-container-name and Nebula will auto-inject a
    ...
    def __post_init__(self):
        if not self.pod_spec:
            raise _user_exceptions.NebulaValidationException("A pod spec cannot be undefined")
        if not self.primary_container_name:
            raise _user_exceptions.NebulaValidationException("A primary container name cannot be undefined")
...
class PodFunctionTask(PythonFunctionTask[Pod]):
    ...
    def local_execute(self, ctx: NebulaContext, **kwargs) -> Union[Tuple[Promise], Promise, None]:
        logger.warning(
