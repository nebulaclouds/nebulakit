...
from nebulakit import PythonFunctionTask
from nebulakit.core.tracker import get_full_module_path
from nebulakit.core.workflow import ImperativeWorkflow, WorkflowBase
...
def compress_scripts(source_path: str, destination: str, module_name: str):
    ...
    nebula.workflows.example and that will produce a tar file that contains only that file alongside
...
def copy_module_to_destination(
    ...
    """
    Copy the module (file) to the destination directory. If the module relative imports other modules, nebulakit will
    ...
    for nebula_entity_name in mod.__dict__:
        nebula_entity = mod.__dict__[nebula_entity_name]
        if (
            isinstance(nebula_entity, (PythonFunctionTask, WorkflowBase))
            and not isinstance(nebula_entity, ImperativeWorkflow)
            and nebula_entity.instantiated_in
        ):
            copy_module_to_destination(
                original_source_path, original_destination_path, nebula_entity.instantiated_in, visited
            )
