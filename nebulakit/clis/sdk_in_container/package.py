...
from nebulakit.tools.repo import NoSerializableEntitiesError, serialize_and_package
...
@click.option(
    ...
    default=False,
    required=False,
    help="This flag enables fast packaging, that allows `no container build` deploys of nebula workflows and tasks. "
...
@click.option(
    ...
    "Nebula to load your program. This is usually where you install nebulakit within the container.",
)
@click.option(
    "-d",
...
def package(
    ...
    """
    This command produces a Nebula backend registrable package of all entities in Nebula.
    ...
    if not pkgs:
        display_help_with_error(ctx, "No packages to scan for nebula entities. Aborting!")

    try:
        serialize_and_package(pkgs, serialization_settings, source, output, fast, deref_symlinks)
    except NoSerializableEntitiesError:
        click.secho(f"No nebula objects found in packages {pkgs}", fg="yellow")
