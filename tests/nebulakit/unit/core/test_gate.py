...
from nebulakit.remote.entities import NebulaWorkflow
...
def test_dyn_signal():
    ...
    with NebulaContextManager.with_context(
        NebulaContextManager.current_context().with_serialization_settings(serialization_settings)
    ) as ctx:
        with NebulaContextManager.with_context(
...
def test_promote():
    ...
    fwf = NebulaWorkflow.promote_from_model(wf_spec.template, tasks=tts)
