...
from nebulakit import lazy_module
...
from nebulakit.models.literals import Literal, Scalar, StructuredDatasetMetadata
from nebulakit.models.types import LiteralType, SchemaType, StructuredDatasetType
...
class StructuredDataset(DataClassJSONMixin):
    ...
    def all(self) -> DF:  # type: ignore
        ...
        return nebula_dataset_transformer.open_as(ctx, self.literal, self._dataframe_type, self.metadata)

    def iter(self) -> Generator[DF, None, None]:
        if self._dataframe_type is None:
            raise ValueError("No dataframe type set. Use open() to set the local dataframe type you want to use.")
        ctx = NebulaContextManager.current_context()
        return nebula_dataset_transformer.iter_as(
...
class StructuredDatasetEncoder(ABC):
    def __init__(self, python_type: Type[T], protocol: Optional[str] = None, supported_format: Optional[str] = None):
        ...
        StructuredDatasetTransformerEngine class in order for the core nebulakit type engine to handle
        dataframe libraries. This is the encoding interface, meaning it is used when there is a Python value that the
        nebulakit type engine is trying to convert into a Nebula Literal. For the other way, see
        ...
        :param protocol: A prefix representing the storage driver (e.g. 's3, 'gs', 'bq', etc.). You can use either
          ...
          If None, this encoder will be registered with all protocols that nebulakit's data persistence layer
...
class StructuredDatasetDecoder(ABC):
    def __init__(self, python_type: Type[DF], protocol: Optional[str] = None, supported_format: Optional[str] = None):
        ...
        StructuredDatasetTransformerEngine class in order for the core nebulakit type engine to handle
        dataframe libraries. This is the decoder interface, meaning it is used when there is a Nebula Literal value,
        ...
        :param protocol: A prefix representing the storage driver (e.g. 's3, 'gs', 'bq', etc.). You can use either
          "s3" or "s3://". They are the same since the "://" will just be stripped by the constructor.
          If None, this decoder will be registered with all protocols that nebulakit's data persistence layer
          is capable of handling.
        :param supported_format: Arbitrary string representing the format. If not supplied then an empty string
    ...
    def decode(
        ...
        """
        This is code that will be called by the dataset transformer engine to ultimately translate from a Nebula Literal
        value into a Python instance.

        :param ctx: A NebulaContext, useful in accessing the filesystem and other attributes
        :param nebula_value: This will be a Nebula IDL StructuredDataset Literal - do not confuse this with the
        ...
        :param current_task_metadata: Metadata object containing the type (and columns if any) for the currently
         executing task. This type may have more or less information than the type information bundled inside the incoming nebula_value.
...
class StructuredDatasetTransformerEngine(TypeTransformer[StructuredDataset]):
    ...
    If you are bringing a custom data frame type, or any data frame type, to nebulakit, instead of
    ...
    def register(
        ...
        """
        Call this with any Encoder or Decoder to register it with the nebulakit type system. If your handler does not
        ...
        :param default_for_type: If set, when a user returns from a task an instance of the dataframe the handler
          ...
          is capable of handling all the different storage protocols that nebulakit's data persistence layer is aware of.
    ...
    def register_for_protocol(
        ...
        # long as the older Pandas/NebulaSchema transformer do not also specify the override
    ...
    def to_literal(
        ...
        # Check first to see if it's even an SD type. For backwards compatibility, we may be getting a NebulaSchema
        python_type, *attrs = extract_cols_and_format(python_type)
        # In case it's a NebulaSchema
    ...
    def _protocol_from_type_or_prefix(self, ctx: NebulaContext, df_type: Type, uri: Optional[str] = None) -> str:
        """
    ...
    def to_python_value(
        self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[T] | StructuredDataset
    ) -> T | StructuredDataset:
        """
        ...
        # The literal that we get in might be an old NebulaSchema.
        ...
        if lv.scalar.schema is not None:
            ...
            if column_dict is None or len(column_dict) == 0:
                ...
                # Dataframe will always be serialized to parquet file by NebulaSchema transformer
    ...
    def to_html(self, ctx: NebulaContext, python_val: typing.Any, expected_python_type: Type[T]) -> str:
        if isinstance(python_val, StructuredDataset):
            if python_val.dataframe is not None:
...
nebula_dataset_transformer = StructuredDatasetTransformerEngine()
TypeEngine.register(nebula_dataset_transformer)
