...
from nebulakitplugins.awssagemaker.distributed_training import DistributedTrainingContext
from google.protobuf.json_format import MessageToDict
from typing_extensions import Annotated

import nebulakit
from nebulakit import ExecutionParameters, NebulaContextManager, PythonFunctionTask, kwtypes
from nebulakit.configuration import SerializationSettings
from nebulakit.extend import ExecutionState, IgnoreOutputs, Interface, PythonTask, TaskPlugins
...
class SagemakerBuiltinAlgorithmsTask(PythonTask[SagemakerTrainingJobConfig]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        interface = Interface(
            # TODO change train and validation to be NebulaDirectory when available
            inputs=kwtypes(
                static_hyperparameters=dict, train=NebulaDirectory[input_type], validation=NebulaDirectory[input_type]
            ),
            outputs=kwtypes(model=NebulaFile[self.OUTPUT_TYPE]),
        )
        super().__init__(
    ...
    def get_custom(self, settings: SerializationSettings) -> Dict[str, Any]:
        ...
        return MessageToDict(training_job.to_nebula_idl())
...
class SagemakerCustomTrainingTask(PythonFunctionTask[SagemakerTrainingJobConfig]):
    ...
    is built according to Nebula plugin documentation (TODO point the link here)
    """
    ...
    def pre_execute(self, user_params: ExecutionParameters) -> ExecutionParameters:
        ...
        if self._is_distributed():
            ...
            exec_state = NebulaContextManager.current_context().execution_state
    ...
    def post_execute(self, user_params: ExecutionParameters, rval: Any) -> Any:
        ...
        if self._is_distributed():
            logger.info("Distributed context detected!")
            dctx = nebulakit.current_context().distributed_training_context
            if not self.task_config.should_persist_output(dctx):
                logger.info("output persistence predicate not met, Nebulakit will ignore outputs")
                raise IgnoreOutputs(f"Distributed context - Persistence predicate not met. Ignoring outputs - {dctx}")
        return rval


# Register the Tensorflow Plugin into the nebulakit core plugin system
