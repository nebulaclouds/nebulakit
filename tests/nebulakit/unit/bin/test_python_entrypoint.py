...
import mock
import pytest
from nebulaidl.core.errors_pb2 import ErrorDocument

from nebulakit.bin.entrypoint import _dispatch_execute, normalize_inputs, setup_execution
...
from nebulakit.models.core import errors as error_models
from nebulakit.models.core import execution as execution_models


@mock.patch("nebulakit.core.utils.load_proto_from_file")
@mock.patch("nebulakit.core.data_persistence.FileAccessProvider.get_data")
@mock.patch("nebulakit.core.data_persistence.FileAccessProvider.put_data")
@mock.patch("nebulakit.core.utils.write_proto_to_file")
def test_dispatch_execute_void(mock_write_to_file, mock_upload_dir, mock_get_data, mock_load_proto):
    ...
    with context_manager.NebulaContextManager.with_context(
        ...
        empty_literal_map = _literal_models.LiteralMap({}).to_nebula_idl()
...
@mock.patch.dict(os.environ, {"NEBULA_FAIL_ON_ERROR": "True"})
@mock.patch("nebulakit.core.utils.load_proto_from_file")
@mock.patch("nebulakit.core.data_persistence.FileAccessProvider.get_data")
@mock.patch("nebulakit.core.data_persistence.FileAccessProvider.put_data")
@mock.patch("nebulakit.core.utils.write_proto_to_file")
...
# See nebulakit.core.utils.write_proto_to_file for the original
...
def test_dispatch_execute_normal(mock_write_to_file, mock_upload_dir, mock_get_data, mock_load_proto):
    ...
    with context_manager.NebulaContextManager.with_context(
        ...
        mock_load_proto.return_value = input_literal_map.to_nebula_idl()
        ...
        lm = _literal_models.LiteralMap.from_nebula_idl(v)
...
def test_dispatch_execute_user_error_non_recov(mock_write_to_file, mock_upload_dir, mock_get_data, mock_load_proto):
    ...
    with context_manager.NebulaContextManager.with_context(
        ...
        v = list(files.values())[0]
        ed = error_models.ErrorDocument.from_nebula_idl(v)
...
def test_dispatch_execute_user_error_recoverable(mock_write_to_file, mock_upload_dir, mock_get_data, mock_load_proto):
    ...
    def my_subwf(a: int) -> typing.List[str]:
        ...
        raise user_exceptions.NebulaRecoverableException(f"recoverable {a}")
...
@mock.patch("nebulakit.bin.entrypoint.os")
def test_env_reading(mock_os):
    mock_env = {
        "NEBULA_INTERNAL_EXECUTION_PROJECT": "exec_proj",
        "NEBULA_INTERNAL_EXECUTION_DOMAIN": "exec_dom",
        "NEBULA_INTERNAL_EXECUTION_ID": "exec_name",
        "NEBULA_INTERNAL_TASK_PROJECT": "task_proj",
        "NEBULA_INTERNAL_TASK_DOMAIN": "task_dom",
        "NEBULA_INTERNAL_TASK_NAME": "task_name",
        "NEBULA_INTERNAL_TASK_VERSION": "task_ver",
    }
