...
from nebulakit.clients.auth.auth_client import AuthorizationClient
from nebulakit.clients.auth.authenticator import Authenticator
from nebulakit.clients.auth.exceptions import AccessTokenNotFoundError
from nebulakit.clients.auth.keyring import Credentials, KeyringStore
...
WEBAPP_CLIENT_ID_HELP = (
    ...
    "Created when activating IAP for the Nebula deployment. "
...
def get_service_account_id_token(audience: str, service_account_email: str) -> str:
    ...
    Uses nebulakit's KeyringStore to cache the ID token.
    ...
    """
    # Nebulakit's KeyringStore, by default, uses the endpoint as the key to store the credentials
