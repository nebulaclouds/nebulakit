...
from nebulakit.configuration import (
...
from nebulakit.core.array_node_map_task import ArrayNodeMapTaskResolver
from nebulakit.core.base_task import IgnoreOutputs, PythonTask
from nebulakit.core.checkpointer import SyncCheckpoint
from nebulakit.core.context_manager import ExecutionParameters, ExecutionState, NebulaContext, NebulaContextManager
from nebulakit.core.data_persistence import FileAccessProvider
from nebulakit.core.map_task import MapTaskResolver
from nebulakit.core.promise import VoidPromise
from nebulakit.deck.deck import _output_deck
from nebulakit.exceptions import scopes as _scoped_exceptions
from nebulakit.exceptions import scopes as _scopes
from nebulakit.interfaces.stats.taggable import get_stats as _get_stats
from nebulakit.loggers import entrypoint_logger as logger
from nebulakit.loggers import user_space_logger
from nebulakit.models import dynamic_job as _dynamic_job
from nebulakit.models import literals as _literal_models
from nebulakit.models.core import errors as _error_models
from nebulakit.models.core import execution as _execution_models
from nebulakit.models.core import identifier as _identifier
from nebulakit.tools.fast_registration import download_distribution as _download_distribution
from nebulakit.tools.module_loader import load_object_from_module


def get_version_message():
    import nebulakit

    return f"Welcome to Nebula! Version: {nebulakit.__version__}"
...
def _dispatch_execute(
    ...
    try:
        ...
        idl_input_literals = _literal_models.LiteralMap.from_nebula_idl(input_proto)

        # Step2
        # Decorate the dispatch execute function before calling it, this wraps all exceptions into one
        # of the NebulaScopedExceptions
    ...
    # Handle user-scoped errors
    except _scoped_exceptions.NebulaScopedUserException as e:
        ...
        logger.error("!! Begin User Error Captured by Nebula !!")
        logger.error(e.verbose_message)
        logger.error("!! End Error Captured by Nebula !!")

    # Handle system-scoped errors
    except _scoped_exceptions.NebulaScopedSystemException as e:
        if isinstance(e.value, IgnoreOutputs):
        ...
        logger.error("!! Begin System Error Captured by Nebula !!")
        logger.error(e.verbose_message)
        logger.error("!! End Error Captured by Nebula !!")
    ...
    # dispatch_execute) as recoverable system exceptions.
        ...
        logger.error("!! Begin Unknown System Error Captured by Nebula !!")
        logger.error(exc_str)
        logger.error("!! End Error Captured by Nebula !!")

    for k, v in output_file_dict.items():
        utils.write_proto_to_file(v.to_nebula_idl(), os.path.join(ctx.execution_state.engine_dir, k))
    ...
    if os.environ.get("NEBULA_FAIL_ON_ERROR", "").lower() == "true" and _constants.ERROR_FILE_NAME in output_file_dict:
        # This env is set by the nebulapropeller
...
def setup_execution(
    ...
    :return:
    """
    exe_project = get_one_of("NEBULA_INTERNAL_EXECUTION_PROJECT", "_F_PRJ")
    exe_domain = get_one_of("NEBULA_INTERNAL_EXECUTION_DOMAIN", "_F_DM")
    exe_name = get_one_of("NEBULA_INTERNAL_EXECUTION_ID", "_F_NM")
    exe_wf = get_one_of("NEBULA_INTERNAL_EXECUTION_WORKFLOW", "_F_WF")
    exe_lp = get_one_of("NEBULA_INTERNAL_EXECUTION_LAUNCHPLAN", "_F_LP")

    tk_project = get_one_of("NEBULA_INTERNAL_TASK_PROJECT", "_F_TK_PRJ")
    tk_domain = get_one_of("NEBULA_INTERNAL_TASK_DOMAIN", "_F_TK_DM")
    tk_name = get_one_of("NEBULA_INTERNAL_TASK_NAME", "_F_TK_NM")
    tk_version = get_one_of("NEBULA_INTERNAL_TASK_VERSION", "_F_TK_V")
    ...
    from nebulakit import __version__ as _api_version
    ...
    try:
        file_access = FileAccessProvider(
            local_sandbox_dir=tempfile.mkdtemp(prefix="nebula"),
            raw_output_prefix=raw_output_data_prefix,
        )
    ...
    with NebulaContextManager.with_context(cb) as ctx:
        yield ctx
...
def _execute_task(
    ...
    :param dynamic_dest_dir: In the case of parent tasks executed using the 'fast' mode this captures where compressed
        code archives should be installed in the nebula task container.
    :return:
    """
...
@_pass_through.command("pynebula-execute")
...
@_pass_through.command("pynebula-fast-execute")
...
@_pass_through.command("pynebula-map-execute")
@_click.option("--inputs", required=True)
@_click.option("--output-prefix", required=True)
@_click.option("--raw-output-data-prefix", required=False)
