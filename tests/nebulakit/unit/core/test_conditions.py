...
from nebulakit.models.core.workflow import Node
...
def test_condition_sub_workflows():
    ...
    def math_ops(a: int, b: int) -> typing.Tuple[int, int]:
        # Nebula will only make `sum` and `sub` available as outputs because they are common between all branches
        sum, sub = (
...
def test_condition_tuple_branches():
    ...
    assert (
        ...
        == f"{prefix}tests.nebulakit.unit.core.test_conditions.sum_sub"
    )
...
def test_subworkflow_condition_serialization():
    ...
    fmt = "tests.nebulakit.unit.core.test_conditions.{}"
