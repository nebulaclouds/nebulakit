...
from nebulakit import LaunchPlan, kwtypes
from nebulakit.configuration import Config, ImageConfig, SerializationSettings
from nebulakit.exceptions.user import NebulaAssertion, NebulaEntityNotExistException
...
CONFIG = os.environ.get("NEBULACTL_CONFIG", str(pathlib.Path.home() / ".nebula" / "config-sandbox.yaml"))
...
def test_fetch_execute_launch_plan(register):
    ...
    nebula_launch_plan = remote.fetch_launch_plan(name="basic.hello_world.my_wf", version=VERSION)
    execution = remote.execute(nebula_launch_plan, inputs={}, wait=True)
    assert execution.outputs["o0"] == "hello world"


def fetch_execute_launch_plan_with_args(register):
    remote = NebulaRemote(Config.auto(config_file=CONFIG), PROJECT, DOMAIN)
    nebula_launch_plan = remote.fetch_launch_plan(name="basic.basic_workflow.my_wf", version=VERSION)
    execution = remote.execute(nebula_launch_plan, inputs={"a": 10, "b": "foobar"}, wait=True)
...
def test_monitor_workflow_execution(register):
    ...
    while datetime.datetime.utcnow() < time_to_give_up:
        ...
        with pytest.raises(
            NebulaAssertion, match="Please wait until the execution has completed before requesting the outputs."
        ):
            execution.outputs
...
def test_fetch_execute_launch_plan_with_subworkflows(register):
    ...
    nebula_launch_plan = remote.fetch_launch_plan(name="basic.subworkflows.parent_wf", version=VERSION)
    execution = remote.execute(nebula_launch_plan, inputs={"a": 101}, wait=True)
...
def test_fetch_execute_launch_plan_with_child_workflows(register):
    ...
    nebula_launch_plan = remote.fetch_launch_plan(name="basic.child_workflow.parent_wf", version=VERSION)
    execution = remote.execute(nebula_launch_plan, inputs={"a": 3}, wait=True)
...
def test_fetch_execute_workflow(register):
    remote = NebulaRemote(Config.auto(config_file=CONFIG), PROJECT, DOMAIN)
    nebula_workflow = remote.fetch_workflow(name="basic.hello_world.my_wf", version=VERSION)
    execution = remote.execute(nebula_workflow, inputs={}, wait=True)
    assert execution.outputs["o0"] == "hello world"
    ...
    execution_to_terminate = remote.execute(nebula_workflow, {})
    remote.terminate(execution_to_terminate, cause="just because")


def test_fetch_execute_task(register):
    remote = NebulaRemote(Config.auto(config_file=CONFIG), PROJECT, DOMAIN)
    nebula_task = remote.fetch_task(name="basic.basic_workflow.t1", version=VERSION)
    execution = remote.execute(nebula_task, inputs={"a": 10}, wait=True)
...
def test_execute_python_task(register):
    ...
    assert execution.spec.tags == ["nebula"]
...
def test_execute_python_workflow_and_launch_plan(register):
    ...
    nebula_workflow_execution = remote.fetch_execution(name=execution.id.name)
    assert execution.inputs == nebula_workflow_execution.inputs
    assert execution.outputs == nebula_workflow_execution.outputs


def test_fetch_execute_launch_plan_list_of_floats(register):
    remote = NebulaRemote(Config.auto(config_file=CONFIG), PROJECT, DOMAIN)
    nebula_launch_plan = remote.fetch_launch_plan(name="basic.list_float_wf.my_wf", version=VERSION)
    xs: typing.List[float] = [42.24, 999.1, 0.0001]
    execution = remote.execute(nebula_launch_plan, inputs={"xs": xs}, wait=True)
    assert execution.outputs["o0"] == "[42.24, 999.1, 0.0001]"


def test_fetch_execute_task_list_of_floats(register):
    remote = NebulaRemote(Config.auto(config_file=CONFIG), PROJECT, DOMAIN)
    nebula_task = remote.fetch_task(name="basic.list_float_wf.concat_list", version=VERSION)
    xs: typing.List[float] = [0.1, 0.2, 0.3, 0.4, -99999.7]
    execution = remote.execute(nebula_task, inputs={"xs": xs}, wait=True)
    assert execution.outputs["o0"] == "[0.1, 0.2, 0.3, 0.4, -99999.7]"


def test_fetch_execute_task_convert_dict(register):
    remote = NebulaRemote(Config.auto(config_file=CONFIG), PROJECT, DOMAIN)
    nebula_task = remote.fetch_task(name="basic.dict_str_wf.convert_to_string", version=VERSION)
    d: typing.Dict[str, str] = {"key1": "value1", "key2": "value2"}
    execution = remote.execute(nebula_task, inputs={"d": d}, wait=True)
...
def test_execute_joblib_workflow(register):
    remote = NebulaRemote(Config.auto(config_file=CONFIG), PROJECT, DOMAIN)
    nebula_workflow = remote.fetch_workflow(name="basic.joblib.joblib_workflow", version=VERSION)
    input_obj = [1, 2, 3]
    execution = remote.execute(nebula_workflow, inputs={"obj": input_obj}, wait=True)
...
def test_fetch_not_exist_launch_plan(register):
    remote = NebulaRemote(Config.auto(config_file=CONFIG), PROJECT, DOMAIN)
    with pytest.raises(NebulaEntityNotExistException):
