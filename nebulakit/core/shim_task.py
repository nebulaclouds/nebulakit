...
class ExecutableTemplateShimTask(object):
    ...
    locally or on a Nebula cluster), the function runs.

    This class, along with the ``ShimTaskExecutor`` class below, represents another execution pattern. This pattern,
    has two components:

    * The ``TaskTemplate``, or something like it like a ``NebulaTask``.
    * An executor, which can use information from the task template (including the ``custom`` field)

    Basically at execution time (both locally and on a Nebula cluster), the task template is given to the executor,
    which is responsible for computing and returning the results.

    .. note::

       The interface at execution time will have to derived from the Nebula IDL interface, which means it may be lossy.
       This is because when a task is serialized from Python into the ``TaskTemplate`` some information is lost because
       Nebula IDL can't keep track of every single Python type (or Java type if writing in the Java nebulakit).
    ...
    def dispatch_execute(
        ...
        with NebulaContextManager.with_context(
            ...
            # Added: Have to reverse the Python interface from the task template Nebula interface
            ...
            logger.info(f"Invoking NebulaTask executor {self.task_template.id.name} with inputs: {native_inputs}")
            try:
