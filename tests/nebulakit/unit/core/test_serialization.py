...
from nebulakit import ContainerTask, kwtypes
...
from nebulakit.core.python_auto_container import get_registerable_container_image
...
def test_serialization():
    ...
    sum = ContainerTask(
        name="sum",
        input_data_dir="/var/nebula/inputs",
        output_data_dir="/var/nebula/outputs",
        inputs=kwtypes(x=int, y=int),
        outputs=kwtypes(out=int),
        image="alpine",
        command=["sh", "-c", "echo $(( {{.Inputs.x}} + {{.Inputs.y}} )) | tee /var/nebula/outputs/out"],
    )
...
def test_serialization_images():
    ...
    os.environ["NEBULA_INTERNAL_IMAGE"] = "docker.io/default:version"
    imgs = ImageConfig.auto(
        config_file=os.path.join(os.path.dirname(os.path.realpath(__file__)), "configs/images.config")
    )
    rs = nebulakit.configuration.SerializationSettings(
        project="project",
        domain="domain",
        version="version",
        env=None,
...
def test_serialization_command1():
    ...
    assert task_spec.template.container.args[-7:] == [
        ...
        "--",
        "task-module",
        "tests.nebulakit.unit.core.test_serialization",
...
def test_serialization_set_command():
    ...
    t1.reset_command_fn()
    custom_command = t1.get_command(serialization_settings)
    assert custom_command[0] == "pynebula-execute"
...
def test_serialization_nested_subwf():
    ...
    assert subwf.keys() == {
        "tests.nebulakit.unit.core.test_serialization.leaf_subwf",
        "tests.nebulakit.unit.core.test_serialization.middle_subwf",
    }
    midwf = subwf["tests.nebulakit.unit.core.test_serialization.middle_subwf"]
    assert len(midwf.nodes) == 1
    assert midwf.nodes[0].workflow_node is not None
    assert (
        midwf.nodes[0].workflow_node.sub_workflow_ref.name == "tests.nebulakit.unit.core.test_serialization.leaf_subwf"
    )
