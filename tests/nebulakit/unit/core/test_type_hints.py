...
from nebulakit import Secret, SQLTask, dynamic, kwtypes, map_task
from nebulakit.configuration import FastSerializationSettings, Image, ImageConfig
from nebulakit.core import context_manager, launch_plan, promise
...
from nebulakit.core.promise import NodeOutput, Promise, VoidPromise
from nebulakit.core.resources import Resources
from nebulakit.core.task import TaskMetadata, task
from nebulakit.core.testing import patch, task_mock
from nebulakit.core.type_engine import RestrictedTypeError, SimpleTransformer, TypeEngine
from nebulakit.core.workflow import workflow
from nebulakit.models import literals as _literal_models
from nebulakit.models.core import types as _core_types
from nebulakit.models.interface import Parameter
from nebulakit.models.task import Resources as _resource_models
from nebulakit.models.types import LiteralType, SimpleType
from nebulakit.tools.translator import get_serializable
from nebulakit.types.directory import NebulaDirectory, TensorboardLogs
from nebulakit.types.file import NebulaFile
from nebulakit.types.schema import NebulaSchema, SchemaOpenMode
...
def test_default_wf_params_works():
    @task
    def my_task(a: int):
        wf_params = nebulakit.current_context()
        assert str(wf_params.execution_id) == "ex:local:local:local"
        assert nebula_tmp_dir in wf_params.raw_output_prefix

    my_task(a=3)
    assert context_manager.NebulaContextManager.size() == 1
...
def test_simple_input_no_output():
    ...
    with context_manager.NebulaContextManager.with_context(ctx.with_new_compilation_state()) as ctx:
        outputs = my_task(a=3)
...
def test_engine_file_output():
    ...
    fs = FileAccessProvider(local_sandbox_dir="/tmp/nebulatesting", raw_output_prefix="/tmp/nebularaw")
    ctx = context_manager.NebulaContextManager.current_context()
...
def test_promise_return():
    ...
    are returned wrapping Nebula literals instead of the unpacked dict.
    """
...
def test_nebula_file_in_dataclass():
    ...
    def t1(path: str) -> FileStruct:
        file = NebulaFile(path)
        fs = FileStruct(a=file, b=InnerFileStruct(a=file, b=NebulaFile(path)))
        return fs
    ...
    def t2(fs: FileStruct) -> os.PathLike:
        ...
        assert nebula_tmp_dir in fs.a.path
        assert nebula_tmp_dir in fs.b.a.path
        assert nebula_tmp_dir in fs.b.b.path

        return fs.a.path

    @task
    def t3(fs: FileStruct) -> NebulaFile:
        return fs.a

    @workflow
    def wf(path: str) -> (os.PathLike, NebulaFile):
        n1 = t1(path=path)
        dyn(fs=n1)
        return t2(fs=n1), t3(fs=n1)

    assert nebula_tmp_dir in wf(path="s3://somewhere")[0].path
    assert nebula_tmp_dir in wf(path="s3://somewhere")[1].path
...
def test_wf1_with_dynamic():
    ...
    with context_manager.NebulaContextManager.with_context(
        context_manager.NebulaContextManager.current_context().with_serialization_settings(
        ...
        with context_manager.NebulaContextManager.with_context(ctx.with_execution_state(new_exc_state)) as ctx:
    ...
    with context_manager.NebulaContextManager.with_context(
        context_manager.NebulaContextManager.current_context().with_serialization_settings(
            nebulakit.configuration.SerializationSettings(
                ...
                fast_serialization_settings=FastSerializationSettings(
                    enabled=True,
                    destination_dir="/User/nebula/workflows",
        ...
        with context_manager.NebulaContextManager.with_context(
            ...
            assert args.startswith(
                "pynebula-fast-execute --additional-distribution s3://my-s3-bucket/fast/123 "
                "--dest-dir /User/nebula/workflows"
            )
...
def test_comparison_refs():
    def dummy_node(node_id) -> Node:
        n = Node(
            ...
            bindings=[],
            upstream_nodes=[],
            nebula_entity=SQLTask(name="x", query_template="x", inputs={}),
        )
...
def test_lp_serialize():
    ...
    parameter_a = Parameter.from_nebula_idl(parameter_a.to_nebula_idl())
...
def test_wf_typed_schema():
    schema1 = NebulaSchema[kwtypes(x=int, y=str)]
    ...
    @task
    def t2(s: NebulaSchema[kwtypes(x=int, y=str)]) -> NebulaSchema[kwtypes(x=int)]:
        df = s.open().all()
        return df[s.column_names()[:-1]]

    @workflow
    def wf() -> NebulaSchema[kwtypes(x=int)]:
        return t2(s=t1())
...
def test_nebula_schema_dataclass():
    TestSchema = NebulaSchema[kwtypes(some_str=str)]
...
def test_environment():
    ...
    with context_manager.NebulaContextManager.with_context(
        context_manager.NebulaContextManager.current_context().with_new_compilation_state()
    ):
...
def test_secrets():
    ...
    def foo() -> str:
        return nebulakit.current_context().secrets.get("my_group", "")
    ...
    def foo2() -> str:
        return nebulakit.current_context().secrets.get("group", "key")

    os.environ[nebulakit.current_context().secrets.get_secrets_env_var("group", "key")] = "super-secret-value2"
...
def test_nested_dynamic():
    ...
    settings = nebulakit.configuration.SerializationSettings(
        project="test_proj",
        domain="test_domain",
        version="abc",
        image_config=ImageConfig(Image(name="name", fqn="image", tag="name")),
    ...
    ctx = context_manager.NebulaContextManager.current_context().with_serialization_settings(settings)
    with context_manager.NebulaContextManager.with_context(ctx) as ctx:
        es = ctx.new_execution_state().with_params(mode=ExecutionState.Mode.TASK_EXECUTION)
        with context_manager.NebulaContextManager.with_context(ctx.with_execution_state(es)) as ctx:
...
def test_error_messages():
    ...
    with pytest.raises(
        TypeError,
        match=(
            f"Failed to convert inputs of task '{prefix}tests.nebulakit.unit.core.test_type_hints.foo':\n"
    ...
    with pytest.raises(
        TypeError,
        match=(
            f"Failed to convert outputs of task '{prefix}tests.nebulakit.unit.core.test_type_hints.foo2' "
            "at position 0:\n"
    ...
    with pytest.raises(
        TypeError,
        match=f"Failed to convert inputs of task '{prefix}tests.nebulakit.unit.core.test_type_hints.foo3':\n  "
...
def test_union_type():
    ut = typing.Union[int, str, float, NebulaFile, NebulaSchema, typing.List[int], typing.Dict[str, int]]
    ...
    assert isinstance(wf(a=NebulaFile(file.name)), NebulaFile)
    nebulaSchema = NebulaSchema()
    nebulaSchema.open().write(pd.DataFrame({"Name": ["Tom", "Joseph"], "Age": [1, 22]}))
    assert isinstance(wf(a=nebulaSchema), NebulaSchema)
    ...
    with pytest.raises(
        ...
        match=re.escape(
            ...
            f"  Failed to convert inputs of task '{prefix}tests.nebulakit.unit.core.test_type_hints.t2':\n"
            '  Cannot convert from <NebulaLiteral scalar { union { value { scalar { primitive { string_value: "2" } } } '
...
def test_ref_as_key_name():
    class MyOutput(typing.NamedTuple):
        # to make sure nebulakit itself doesn't use this string
        ref: str
