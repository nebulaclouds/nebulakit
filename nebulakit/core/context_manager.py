...
.. autoclass:: nebulakit.core.context_manager::ExecutionState.Mode
   :noindex:
.. autoclass:: nebulakit.core.context_manager::ExecutionState.Mode.TASK_EXECUTION
   :noindex:
.. autoclass:: nebulakit.core.context_manager::ExecutionState.Mode.LOCAL_WORKFLOW_EXECUTION
   :noindex:
.. autoclass:: nebulakit.core.context_manager::ExecutionState.Mode.LOCAL_TASK_EXECUTION
   :noindex:
...
from nebulakit.configuration import Config, SecretsConfig, SerializationSettings
from nebulakit.core import mock_stats, utils
from nebulakit.core.checkpointer import Checkpoint, SyncCheckpoint
from nebulakit.core.data_persistence import FileAccessProvider, default_local_file_access_provider
from nebulakit.core.node import Node
from nebulakit.interfaces.cli_identifiers import WorkflowExecutionIdentifier
from nebulakit.interfaces.stats import taggable
from nebulakit.loggers import logger, user_space_logger
from nebulakit.models.core import identifier as _identifier

if typing.TYPE_CHECKING:
    from nebulakit import Deck
    from nebulakit.clients import friendly as friendly_client  # noqa

# TODO: resolve circular import from nebulakit.core.python_auto_container import TaskResolverMixin

# Enables static type checking https://docs.python.org/3/library/typing.html#typing.TYPE_CHECKING

nebula_context_Var: ContextVar[typing.List[NebulaContext]] = ContextVar("", default=[])

if typing.TYPE_CHECKING:
    from nebulakit.core.base_task import Task, TaskResolverMixin
...
# During out of container serialize the absolute path of the nebulakit virtualenv at serialization time won't match the
# in-container value at execution time. The following default value is used to provide the in-container virtualenv path
# but can be optionally overridden at serialization time based on the installation of your nebulakit virtualenv.
...
class ExecutionParameters(object):
    ...
    .. code-block:: python
        ...
        nebulakit.current_context()

    This object provides the following
    * a statsd handler
    * a logging handler
    * the execution ID as an :py:class:`nebulakit.models.core.identifier.WorkflowExecutionIdentifier` object
    * a working directory for the user to write arbitrary files to

    Please do not confuse this object with the :py:class:`nebulakit.NebulaContext` object.
    """
    ...
    def timeline_deck(self) -> "TimeLineDeck":  # type: ignore
        from nebulakit.deck.deck import TimeLineDeck
    ...
    def __getattr__(self, attr_name: str) -> typing.Any:
        ...
        raise AssertionError(f"{attr_name} not available as a parameter in Nebula context - are you in right task-type?")
...
class CompilationState(object):
    ...
    Attributes:
        ...
        mode (int): refer to :py:class:`nebulakit.extend.ExecutionState.Mode`
        task_resolver (Optional[TaskResolverMixin]): Please see :py:class:`nebulakit.extend.TaskResolverMixin`
...
class ExecutionState(object):
    ...
    class Mode(Enum):
        ...
        # This represents when nebulakit is locally running a workflow. The behavior of tasks differs in this case
...
@dataclass(frozen=True)
class NebulaContext(object):
    """
    This is an internal-facing context object, that most users will not have to deal with. It's essentially a globally
    available grab bag of settings and objects that allows nebulakit to do things like convert complex types, run and
    compile workflows, serialize Nebula entities, etc.

    Even though this object as a ``current_context`` function on it, it should not be called directly. Please use the
    :py:class:`nebulakit.NebulaContextManager` object instead.

    Please do not confuse this object with the :py:class:`nebulakit.ExecutionParameters` object.
    """

    file_access: FileAccessProvider
    level: int = 0
    nebula_client: Optional["friendly_client.SynchronousNebulaClient"] = None
    ...
    @staticmethod
    def current_context() -> NebulaContext:
        """
        This method exists only to maintain backwards compatibility. Please use
        ``NebulaContextManager.current_context()`` instead.

        Users of nebulakit should be wary not to confuse the object returned from this function
        with :py:func:`nebulakit.current_context`
        """
        return NebulaContextManager.current_context()
    ...
    def get_deck(self) -> typing.Union[str, "IPython.core.display.HTML"]:  # type:ignore
        ...
        .. code-block:: python
            ...
            with nebulakit.new_context() as ctx:
                my_task(...)
            ctx.get_deck()
        ...
        """
        from nebulakit.deck.deck import _get_deck
    ...
    class Builder(object):
        ...
        def enter_conditional_section(self) -> NebulaContext.Builder:
            """
        ...
        def with_execution_state(self, es: ExecutionState) -> NebulaContext.Builder:
            self.execution_state = es
            return self

        def with_compilation_state(self, c: CompilationState) -> NebulaContext.Builder:
            self.compilation_state = c
            return self

        def with_new_compilation_state(self) -> NebulaContext.Builder:
            return self.with_compilation_state(self.new_compilation_state())

        def with_file_access(self, fa: FileAccessProvider) -> NebulaContext.Builder:
            self.file_access = fa
            return self

        def with_serialization_settings(self, ss: SerializationSettings) -> NebulaContext.Builder:
...
class NebulaContextManager(object):
    """
    NebulaContextManager manages the execution context within Nebulakit. It holds global state of either compilation
    or Execution. It is not thread-safe and can only be run as a single threaded application currently.
    Context's within Nebulakit is useful to manage compilation state and execution state. Refer to ``CompilationState``
    and ``ExecutionState`` for more information. NebulaContextManager provides a singleton stack to manage these contexts.

    Typical usage is

    .. code-block:: python

        NebulaContextManager.initialize()
        with NebulaContextManager.with_context(o) as ctx:
          pass

        # If required - not recommended you can use
        NebulaContextManager.push_context()
        # but correspondingly a pop_context should be called
        NebulaContextManager.pop_context()
    """
    ...
    @staticmethod
    def current_context() -> NebulaContext:
        if not nebula_context_Var.get():
            # we will lost the default nebula context in the new thread. Therefore, reinitialize the context when running in the new thread.
            NebulaContextManager.initialize()
        return nebula_context_Var.get()[-1]

    @staticmethod
    def push_context(ctx: NebulaContext, f: Optional[traceback.FrameSummary] = None) -> NebulaContext:
        if not f:
            f = NebulaContextManager.get_origin_stackframe(limit=2)
        ctx.set_stackframe(f)
        context_list = nebula_context_Var.get()
        context_list.append(ctx)
        nebula_context_Var.set(context_list)
        t = "\t"
        logger.debug(
            f"{t * ctx.level}[{len(nebula_context_Var.get())}] Pushing context - {'compile' if ctx.compilation_state else 'execute'}, branch[{ctx.in_a_condition}], {ctx.get_origin_stackframe_repr()}"
        )
        return ctx

    @staticmethod
    def pop_context() -> NebulaContext:
        context_list = nebula_context_Var.get()
        ctx = context_list.pop()
        nebula_context_Var.set(context_list)
        t = "\t"
        logger.debug(
            f"{t * ctx.level}[{len(nebula_context_Var.get()) + 1}] Popping context - {'compile' if ctx.compilation_state else 'execute'}, branch[{ctx.in_a_condition}], {ctx.get_origin_stackframe_repr()}"
        )
        if len(nebula_context_Var.get()) == 0:
            raise AssertionError(f"Illegal Context state! Popped, {ctx}")
        return ctx

    @staticmethod
    @contextmanager
    def with_context(b: NebulaContext.Builder) -> Generator[NebulaContext, None, None]:
        ctx = NebulaContextManager.push_context(b.build(), NebulaContextManager.get_origin_stackframe(limit=3))
        l = NebulaContextManager.size()
        try:
            yield ctx
        finally:
            ...
            while NebulaContextManager.size() >= l:
                NebulaContextManager.pop_context()

    @staticmethod
    def size() -> int:
        return len(nebula_context_Var.get())
    ...
    def initialize():
        ...
        """
        # This is supplied so that tasks that rely on Nebula provided param functionality do not fail when run locally
        ...
        default_context = NebulaContext(file_access=default_local_file_access_provider)
        ...
        default_context.set_stackframe(s=NebulaContextManager.get_origin_stackframe())
        nebula_context_Var.set([default_context])


class NebulaEntities(object):
    """
