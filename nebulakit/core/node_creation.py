...
from nebulakit.core.context_manager import BranchEvalMode, NebulaContext
...
def create_node(
    ...
    if (
        ...
        raise AssertionError(f"Should be a callable Nebula entity (either local or fetched) but is {type(entity)}")
    ...
    if ctx.compilation_state is not None and ctx.compilation_state.mode == 1:
        ...
        #  before the model Template classes in NebulaTask/Workflow/LaunchPlan
        if entity.interface.outputs:  # noqa
            if isinstance(outputs, tuple):
                ...
                if len(output_names) != 1:
                    raise _user_exceptions.NebulaAssertion(f"Output of length 1 expected but {len(output_names)} found")
    ...
    # https://github.com/nebulaclouds/nebulakit/blob/0815345faf0fae5dc26746a43d4bda4cc2cdf830/nebulakit/core/python_function_task.py#L262
        ...
        raise Exception(f"Cannot use explicit run to call Nebula entities {entity.name}")
