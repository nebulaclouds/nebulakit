...
from nebulaidl.core import tasks_pb2 as _core_tasks_pb2
from nebulaidl.core import workflow_pb2 as _core_workflow_pb2
from google.protobuf.json_format import MessageToJson
from google.protobuf.pyext.cpp_message import GeneratedProtocolMessageType as _GeneratedProtocolMessageType

from nebulakit import __version__, configuration
from nebulakit.clients import friendly as _friendly_client
from nebulakit.clis.helpers import hydrate_registration_parameters
from nebulakit.core import utils
from nebulakit.core.context_manager import NebulaContextManager
from nebulakit.exceptions import user as _user_exceptions
from nebulakit.interfaces import cli_identifiers
from nebulakit.models import common as _common_models
...
from nebulakit.models import named_entity as _named_entity
from nebulakit.models.admin import common as _admin_common
from nebulakit.models.common import AuthRole as _AuthRole
from nebulakit.models.common import RawOutputDataConfig as _RawOutputDataConfig
from nebulakit.models.core import execution as _core_execution_models
from nebulakit.models.core import identifier as _core_identifier
from nebulakit.models.matchable_resource import ClusterResourceAttributes as _ClusterResourceAttributes
from nebulakit.models.matchable_resource import ExecutionClusterLabel as _ExecutionClusterLabel
from nebulakit.models.matchable_resource import ExecutionQueueAttributes as _ExecutionQueueAttributes
from nebulakit.models.matchable_resource import MatchableResource as _MatchableResource
from nebulakit.models.matchable_resource import MatchingAttributes as _MatchingAttributes
from nebulakit.models.matchable_resource import PluginOverride as _PluginOverride
from nebulakit.models.matchable_resource import PluginOverrides as _PluginOverrides
from nebulakit.models.project import Project as _Project
from nebulakit.models.schedule import Schedule as _Schedule
from nebulakit.tools.fast_registration import get_additional_distribution_loc as _get_additional_distribution_loc
...
# Similar to how kubectl has a config file in the users home directory, this Nebula CLI will also look for one.
...
_default_config_file_dir = ".nebula"
...
def _welcome_message():
    ...
    _click.secho(
        "# nebula-cli is being deprecated in favor of nebulactl. More details about nebulactl in https://docs.nebula.org/projects/nebulactl/ #",
        bold=True,
    )
    _click.secho(
        "################################################################################################################################\n",
    ...
    _click.secho("Welcome to Nebula CLI! Version: {}\n".format(_tt(__version__)), bold=True)
    ...
    if _get_user_filepath_home() and _os.path.exists(config_file):
        ...
        _click.secho(
            """Config file not found at default location, relying on environment variables instead.
                        To setup your config file run 'nebula-cli setup-config'""",
            fg="blue",
        )
    return None
...
def _get_io_string(literal_map, verbose=False):
    """
    :param nebulakit.models.literals.LiteralMap literal_map:
    :param bool verbose:
    :rtype: Text
    """
...
def _fetch_and_stringify_literal_map(path, verbose=False):
    ...
    with utils.AutoDeletingTempDir("nebulacli") as tmp:
        try:
            fname = tmp.get_named_tempfile("literalmap.pb")
            ctx.file_access.get_data(path, fname)
            literal_map = _literals.LiteralMap.from_nebula_idl(
...
def _update_one_launch_plan(client: _friendly_client.SynchronousNebulaClient, urn, state):
    if state == "active":
...
def _get_client(host: str, insecure: bool) -> _friendly_client.SynchronousNebulaClient:
    ...
    return _friendly_client.SynchronousNebulaClient(cfg, **kwargs)
...
_host_option = _click.option(
    *_HOST_FLAGS,
    required=False,
    help="The URL for the Nebula Admin Service. If you intend for this to be consistent, set the NEBULA_PLATFORM_URL "
...
_filter_option = _click.option(
    ...
    syntax can be found here: https://docs.nebula.org/en/latest/dive_deep/admin_service.html#adding-request-filters""",
)
...
_idl_class_option = _click.option(
    ...
    "--proto_class",
    required=True,
    help="Dot (.) separated path to Python IDL class. (e.g. nebulaidl.core.workflow_closure_pb2.WorkflowClosure)",
)
...
class _NebulaSubCommand(_click.Command):
    _PASSABLE_ARGS = {
    ...
    def make_context(self, cmd_name, args, parent=None):
        ...
        # nebula-cli -h localhost:30081 -i setup-config
        # nebula-cli setup-config -h localhost:30081 -i
        ctx = super(_NebulaSubCommand, self).make_context(cmd_name, prefix_args + args, parent=parent)
        ctx.obj = ctx.obj or {}
        ...
        # These two flags are special in that they are specifiable in both the user's default ~/.nebula/config file,
        # and in the nebula-cli command itself, both in the parent-command position (nebula-cli) , and in the
...
@_insecure_option
@_click.group("nebula-cli", deprecated=True)
@_click.pass_context
def _nebula_cli(ctx, host, config, project, domain, name, cacert, insecure):
    """
    Command line tool for interacting with all entities on the Nebula Platform.
    """
    if cacert and insecure:
        raise _user_exceptions.NebulaValidationException("Should not pass both certificate and insecure options!")
...
@_nebula_cli.command("parse-proto", cls=_click.Command)
@_filename_option
@_idl_class_option
...
@_nebula_cli.command("list-task-names", cls=_NebulaSubCommand)
@_project_option
@_domain_option
@_host_option
@_insecure_option
...
@_nebula_cli.command("list-task-versions", cls=_NebulaSubCommand)
@_project_option
@_domain_option
...
@_nebula_cli.command("get-task", cls=_NebulaSubCommand)
@_urn_option
@_host_option
@_insecure_option
...
@_nebula_cli.command("list-workflow-names", cls=_NebulaSubCommand)
@_project_option
@_domain_option
@_host_option
@_insecure_option
...
@_nebula_cli.command("list-workflow-versions", cls=_NebulaSubCommand)
@_project_option
@_domain_option
@_optional_name_option
@_host_option
...
@_nebula_cli.command("get-workflow", cls=_NebulaSubCommand)
@_urn_option
@_host_option
@_insecure_option
...
@_nebula_cli.command("list-launch-plan-names", cls=_NebulaSubCommand)
@_project_option
@_domain_option
@_host_option
@_insecure_option
...
@_nebula_cli.command("list-active-launch-plans", cls=_NebulaSubCommand)
@_project_option
@_domain_option
@_host_option
@_insecure_option
...
@_nebula_cli.command("list-launch-plan-versions", cls=_NebulaSubCommand)
@_project_option
@_domain_option
@_optional_name_option
@_host_option
...
@_nebula_cli.command("get-launch-plan", cls=_NebulaSubCommand)
@_urn_option
@_host_option
@_insecure_option
...
@_nebula_cli.command("get-active-launch-plan", cls=_NebulaSubCommand)
@_project_option
@_domain_option
@_name_option
@_host_option
...
@_nebula_cli.command("update-launch-plan", cls=_NebulaSubCommand)
@_state_choice
@_host_option
@_insecure_option
...
@_nebula_cli.command("recover-execution", cls=_NebulaSubCommand)
@_urn_option
@_optional_name_option
@_host_option
@_insecure_option
def recover_execution(urn, name, host, insecure):
    ...
    Usage:
        $ nebula-cli recover-execution -u ex:nebulaexamples:development:some-workflow:abc123 -n my_retry_name
...
@_nebula_cli.command("terminate-execution", cls=_NebulaSubCommand)
@_host_option
@_insecure_option
@_cause_option
@_optional_urn_option
def terminate_execution(host, insecure, cause, urn=None):
    ...
    NebulaCLI only sends a termination request to NebulaAdmin. The actual termination
    of the execution(s) depends on many other factors such as the status of the
    machine serving the execution, etc.

    e.g.,
        $ nebula-cli -h localhost:30081 -p nebulaexamples -d development terminate-execution \
            -u lp:nebulaexamples:development:some-execution:abc123
    """
    _welcome_message()
    client = _get_client(host, insecure)
...
@_nebula_cli.command("list-executions", cls=_NebulaSubCommand)
@_project_option
@_domain_option
@_host_option
@_insecure_option
            ...
            for te in sorted(task_executions, key=lambda x: x.id.retry_attempt):
                ...
                if te.is_parent:
                    _click.echo(
                        "\t\t\t\t\t{:15} {:60} ".format(
                            "Subtasks:",
                            "nebula-cli get-child-executions -h {host}{insecure} -u {urn}".format(
                                host=host,
...
@_nebula_cli.command("get-execution", cls=_NebulaSubCommand)
@_urn_option
@_host_option
@_insecure_option
@_show_io_option
...
@_nebula_cli.command("get-child-executions", cls=_NebulaSubCommand)
@_urn_option
@_host_option
@_insecure_option
@_show_io_option
...
@_nebula_cli.command("register-project", cls=_NebulaSubCommand)
...
@_nebula_cli.command("list-projects", cls=_NebulaSubCommand)
@_host_option
@_insecure_option
@_token_option
@_limit_option
...
@_nebula_cli.command("archive-project", cls=_NebulaSubCommand)
@_project_identifier_option
@_host_option
@_insecure_option
...
@_nebula_cli.command("activate-project", cls=_NebulaSubCommand)
@_project_identifier_option
@_host_option
@_insecure_option
...
def _extract_pair(
    ...
    :rtype: (nebulaidl.core.identifier_pb2.Identifier, T)
    """
...
def _extract_files(
    ...
    """
    :param file_paths:
    :rtype: List[(nebulaidl.core.identifier_pb2.Identifier, T)]
    """
...
def _get_patch_launch_plan_fn(
    ...
    def patch_launch_plan(entity: _GeneratedProtocolMessageType) -> _GeneratedProtocolMessageType:
        ...
        the nebula config and/or a custom output_location_prefix.
        """
        # entity is of type nebulaidl.admin.launch_plan_pb2.LaunchPlanSpec
        ...
        if output_location_prefix is not None:
            entity.spec.raw_output_data_config.CopyFrom(
                _RawOutputDataConfig(output_location_prefix=output_location_prefix).to_nebula_idl()
            )
...
def _extract_and_register(
    client: _friendly_client.SynchronousNebulaClient,
    project: str,
    domain: str,
    version: str,
    file_paths: List[str],
    ...
    nebula_entities_list = _extract_files(project, domain, version, file_paths, patches)
    for id, nebula_entity in nebula_entities_list:
        _click.secho(f"Registering {id}", fg="yellow")
        try:
            if id.resource_type == _identifier_pb2.LAUNCH_PLAN:
                client.raw.create_launch_plan(_launch_plan_pb2.LaunchPlanCreateRequest(id=id, spec=nebula_entity.spec))
            elif id.resource_type == _identifier_pb2.TASK:
                client.raw.create_task(_task_pb2.TaskCreateRequest(id=id, spec=nebula_entity))
            elif id.resource_type == _identifier_pb2.WORKFLOW:
                client.raw.create_workflow(_workflow_pb2.WorkflowCreateRequest(id=id, spec=nebula_entity))
            else:
                raise _user_exceptions.NebulaAssertion(
                    f"Only tasks, launch plans, and workflows can be called with this function, "
                    f"resource type {id.resource_type} was passed"
                )
        except _user_exceptions.NebulaEntityAlreadyExistsException:
            _click.secho(f"Skipping because already registered {id}", fg="cyan")

    _click.echo(f"Finished scanning {len(nebula_entities_list)} files")


@_nebula_cli.command("register-files", cls=_NebulaSubCommand)
...
def register_files(
    ...
    """
    Given a list of files, this will (after sorting the input list), attempt to register them against Nebula Admin.
    This command expects the files to be the output of the pynebula serialize command.  See the code there for more
    information. Valid files need to be:\n
        * Ordered in the order that you want registration to happen. pynebula should have done the topological sort
          for you and produced file that have a prefix that sets the correct order.\n
        * Of the correct type. That is, they should be the serialized form of one of these Nebula IDL objects
          (or an identifier object).\n
          - nebulaidl.admin.launch_plan_pb2.LaunchPlan for launch plans\n
          - nebulaidl.admin.workflow_pb2.WorkflowSpec for workflows\n
          - nebulaidl.admin.task_pb2.TaskSpec for tasks\n
...
@_nebula_cli.command("fast-register-files", cls=_NebulaSubCommand)
@_click.option(*_PROJECT_FLAGS, required=True, help="The project namespace to register with.")
@_click.option(*_DOMAIN_FLAGS, required=True, help="The domain namespace to register with.")
@_click.option(
    *_VERSION_FLAGS,
...
def fast_register_files(
    ...
    information. Valid files need to be:\n
        ...
        * Of the correct type. That is, they should be the serialized form of one of these Nebula IDL objects
          (or an identifier object).\n
          - nebulaidl.admin.launch_plan_pb2.LaunchPlanSpec for launch plans\n
          - nebulaidl.admin.workflow_pb2.WorkflowSpec for workflows\n
          - nebulaidl.admin.task_pb2.TaskSpec for tasks\n
    ...
    if compressed_source is None:
        raise _click.UsageError(
            "Could not discover compressed source, did you remember to run `pynebula serialize fast ...`?"
        )
    ...
    def fast_register_task(entity: _GeneratedProtocolMessageType) -> _GeneratedProtocolMessageType:
        """
        Updates task definitions during fast-registration in order to use the compatible pynebula fast execute command at
        task execution.
        """
        # entity is of type nebulaidl.admin.task_pb2.TaskSpec
...
@_nebula_cli.command("update-workflow-meta", cls=_NebulaSubCommand)
...
@_nebula_cli.command("update-task-meta", cls=_NebulaSubCommand)
@_named_entity_description_option
@_host_option
@_insecure_option
@_project_option
...
@_nebula_cli.command("update-launch-plan-meta", cls=_NebulaSubCommand)
@_named_entity_description_option
@_host_option
@_insecure_option
@_project_option
...
@_nebula_cli.command("update-cluster-resource-attributes", cls=_NebulaSubCommand)
@_host_option
@_insecure_option
@_project_option
@_domain_option
...
def update_cluster_resource_attributes(host, insecure, project, domain, name, attributes):
    ...
    attributes in your nebulaadmin deployment.

    e.g.
        $ nebula-cli -h localhost:30081 -p nebulaexamples -d development update-cluster-resource-attributes \
...
@_nebula_cli.command("update-execution-queue-attributes", cls=_NebulaSubCommand)
@_host_option
@_insecure_option
@_project_option
@_domain_option
...
def update_execution_queue_attributes(host, insecure, project, domain, name, tags):
    ...
    e.g.
        $ nebula-cli -h localhost:30081 -p nebulaexamples -d development update-execution-queue-attributes \
...
@_nebula_cli.command("update-execution-cluster-label", cls=_NebulaSubCommand)
@_host_option
@_insecure_option
@_project_option
@_domain_option
...
def update_execution_cluster_label(host, insecure, project, domain, name, value):
    ...
    e.g.
        $ nebula-cli -h localhost:30081 -p nebulaexamples -d development update-execution-cluster-label --value foo
    """
    _welcome_message()
    client = _get_client(host, insecure)
...
@_nebula_cli.command("update-plugin-override", cls=_NebulaSubCommand)
@_host_option
@_insecure_option
@_project_option
@_domain_option
...
def update_plugin_override(host, insecure, project, domain, name, task_type, plugin_id, missing_plugin_behavior):
    ...
    e.g.
        $ nebula-cli -h localhost:30081 -p nebulaexamples -d development update-plugin-override --task-type python \
...
@_nebula_cli.command("get-matching-attributes", cls=_NebulaSubCommand)
@_host_option
@_insecure_option
@_project_option
@_domain_option
...
@_nebula_cli.command("list-matching-attributes", cls=_NebulaSubCommand)
@_host_option
@_insecure_option
@_click.option(
    "--resource-type",
...
@_nebula_cli.command("setup-config", cls=_NebulaSubCommand)
@_host_option
@_insecure_option
def setup_config(host, insecure):
    ...
    if not _os.path.isdir(config_dir):
        _click.secho(
            "Creating default Nebula configuration directory at {}".format(_tt(config_dir)),
            fg="blue",
        )
        _os.mkdir(config_dir)

    full_host = "http://{}".format(host) if insecure else "https://{}".format(host)
    config_url = _urlparse.urljoin(full_host, "config/v1/nebula_client")
