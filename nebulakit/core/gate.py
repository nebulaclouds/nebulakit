...
from nebulakit.core import constants
...
from nebulakit.core.promise import Promise, VoidPromise, nebula_entity_call_handler
from nebulakit.core.type_engine import TypeEngine
from nebulakit.exceptions.user import NebulaDisapprovalException
from nebulakit.interaction.parse_stdin import parse_stdin_to_literal
from nebulakit.interaction.string_literals import scalar_to_string
from nebulakit.models.core import workflow as _workflow_model
from nebulakit.models.literals import Scalar
...
class Gate(object):
    ...
    def __init__(
        ...
        if self._sleep_duration:
            ...
            self._python_interface = nebula_interface.Interface()
        elif input_type:
    ...
    def local_execute(self, ctx: NebulaContext, **kwargs) -> Union[Tuple[Promise], Promise, VoidPromise]:
        ...
        if proceed:
            ...
            raise NebulaDisapprovalException(f"User did not approve the transaction for gate node {self.name}")
...
def wait_for_input(name: str, timeout: datetime.timedelta, expected_type: typing.Type):
    ...
    :param timeout: How long to wait for before Nebula fails the workflow.
    ...
    return nebula_entity_call_handler(g)
...
def approve(upstream_item: Union[Tuple[Promise], Promise, VoidPromise], name: str, timeout: datetime.timedelta):
    ...
    :param name: The name of the gate node.
    :param timeout: How long to wait before Nebula fails the workflow.
    :return:
    """
    ...
    if ctx.compilation_state is not None and ctx.compilation_state.mode == 1:
        ...
        if not upstream_item.ref.node.nebula_entity.python_interface:
            raise ValueError(
                f"Upstream node doesn't have a Python interface. Node entity is: "
                f"{upstream_item.ref.node.nebula_entity}"
            )
        ...
        io_type = upstream_item.ref.node.nebula_entity.python_interface.outputs[upstream_item.var]
    ...
    g._python_interface = nebula_interface.Interface(
        inputs={
            io_var_name: io_type,
        },
        outputs={
    ...
    return nebula_entity_call_handler(g, **kwargs)
