...
from nebulakit.remote import NebulaTask, NebulaWorkflow
...
def test_wf_promote_subwf_lps():
    ...
    fwf = NebulaWorkflow.promote_from_model(
        ...
        tasks={k: NebulaTask.promote_from_model(t) for k, t in task_templates.items()},
    )
    assert len(fwf.outputs) == 1
    assert list(fwf.interface.inputs.keys()) == ["b"]
    assert len(fwf.nodes) == 1
    assert len(fwf.nebula_nodes) == 1
...
def test_upstream():
    ...
    assert len(fwf.nebula_nodes[0].upstream_nodes) == 0
    assert len(fwf.nebula_nodes[1].upstream_nodes) == 1
    ...
    fwf = NebulaWorkflow.promote_from_model(
        ...
        sub_workflows=sub_wf_dict,
        node_launch_plans={},
        tasks={k: NebulaTask.promote_from_model(v) for k, v in task_templates.items()},
    )
    # Test upstream nodes don't get confused by subworkflows
    assert len(fwf.nebula_nodes[0].upstream_nodes) == 0
    assert len(fwf.nebula_nodes[1].upstream_nodes) == 0
