...
import pytest
from nebulakitplugins.pod.task import Pod, PodFunctionTask
from kubernetes.client import ApiClient
from kubernetes.client.models import V1Container, V1EnvVar, V1PodSpec, V1ResourceRequirements, V1VolumeMount

from nebulakit import Resources, TaskMetadata, dynamic, map_task, task
...
from nebulakit.extend import ExecutionState
...
def test_dynamic_pod_task():
    ...
    with context_manager.NebulaContextManager.with_context(
        context_manager.NebulaContext.current_context().with_serialization_settings(
            SerializationSettings(
                project="test_proj",
                domain="test_domain",
                version="abc",
...
def test_fast():
    ...
    def get_minimal_pod_task_config() -> Pod:
        primary_container = V1Container(name="nebulatask")
        pod_spec = V1PodSpec(containers=[primary_container])
        return Pod(pod_spec=pod_spec, primary_container_name="nebulatask")
    ...
    with context_manager.NebulaContextManager.with_context(
        context_manager.NebulaContextManager.current_context().with_serialization_settings(serialization_settings)
    ) as ctx:
        with context_manager.NebulaContextManager.with_context(
...
def test_pod_config():
    with pytest.raises(user.NebulaValidationException):
        Pod(pod_spec=None)
