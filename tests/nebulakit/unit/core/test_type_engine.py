...
from nebulaidl.core import errors_pb2
...
from nebulakit.core.data_persistence import nebula_tmp_dir
...
from nebulakit.core.type_engine import (
...
from nebulakit.models import types as model_types
from nebulakit.models.annotation import TypeAnnotation
from nebulakit.models.core.types import BlobType
from nebulakit.models.literals import Blob, BlobMetadata, Literal, LiteralCollection, LiteralMap, Primitive, Scalar, Void
from nebulakit.models.types import LiteralType, SimpleType, TypeStructure, UnionType
from nebulakit.types.directory import TensorboardLogs
from nebulakit.types.directory.types import NebulaDirectory
from nebulakit.types.file import FileExt, JPEGImageFile
from nebulakit.types.file.file import NebulaFile, NebulaFilePathTransformer, noop
from nebulakit.types.pickle import NebulaPickle
from nebulakit.types.pickle.pickle import BatchSize, NebulaPickleTransformer
...
def test_type_resolution():
    ...
    assert type(TypeEngine.get_transformer(os.PathLike)) == NebulaFilePathTransformer
    assert type(TypeEngine.get_transformer(NebulaPickle)) == NebulaPickleTransformer
    assert type(TypeEngine.get_transformer(typing.Any)) == NebulaPickleTransformer
...
def test_file_formats_getting_literal_type():
    ...
    lt = transformer.get_literal_type(NebulaFile[typing.TypeVar("jpg")])
    ...
    lt = transformer.get_literal_type(NebulaFile[typing.TypeVar(".png")])
...
def test_file_format_getting_python_value():
    ...
    pv = transformer.to_python_value(ctx, lv, expected_python_type=NebulaFile["txt"])
    assert isinstance(pv, NebulaFile)
...
def test_annotated_type():
    class JsonTypeTransformer(TypeTransformer[T]):
        ...
        def to_literal(
            self, ctx: NebulaContext, python_val: T, python_type: typing.Type[T], expected: LiteralType
        ) -> Literal:
    ...
    assert (
        TypeEngine.to_literal(NebulaContext.current_context(), test_dict, MyJsonDict, JsonTypeTransformer.LiteralType)
        == test_literal
    )
...
def test_file_no_downloader_default():
    # The idea of this test is to assert that if a NebulaFile is created with no download specified,
    ...
    pv = transformer.to_python_value(ctx, lv, expected_python_type=NebulaFile)
    assert isinstance(pv, NebulaFile)
...
def test_dir_no_downloader_default():
    ...
    pv = transformer.to_python_value(ctx, lv, expected_python_type=NebulaDirectory)
    assert isinstance(pv, NebulaDirectory)
    assert pv.download() == local_dir


def test_dir_with_batch_size():
    nebula_dir = Annotated[NebulaDirectory, BatchSize(100)]
    val = nebula_dir("s3://bucket/key")
    transformer = TypeEngine.get_transformer(nebula_dir)
    ctx = NebulaContext.current_context()
    lt = transformer.get_literal_type(nebula_dir)
    lv = transformer.to_literal(ctx, val, nebula_dir, lt)
    assert val.path == transformer.to_python_value(ctx, lv, nebula_dir).remote_source
...
def test_protos():
    ...
    assert lt.metadata["pb_type"] == "nebulaidl.core.errors_pb2.ContainerError"
...
def test_guessing_basic():
    ...
    lt = model_types.LiteralType(
        blob=BlobType(
            format=NebulaPickleTransformer.PYTHON_PICKLE_FORMAT, dimensionality=BlobType.BlobDimensionality.SINGLE
        )
    )
    pt = TypeEngine.guess_python_type(lt)
    assert pt is NebulaPickle
...
@mock.patch("nebulakit.core.data_persistence.FileAccessProvider.put_data")
def test_optional_nebulafile_in_dataclass(mock_upload_dir):
    ...
    class TestFileStruct(DataClassJsonMixin):
        a: NebulaFile
        b: typing.Optional[NebulaFile]
        b_prime: typing.Optional[NebulaFile]
        c: typing.Union[NebulaFile, None]
        d: typing.List[NebulaFile]
        e: typing.List[typing.Optional[NebulaFile]]
        e_prime: typing.List[typing.Optional[NebulaFile]]
        f: typing.Dict[str, NebulaFile]
        g: typing.Dict[str, typing.Optional[NebulaFile]]
        g_prime: typing.Dict[str, typing.Optional[NebulaFile]]
        h: typing.Optional[NebulaFile] = None
        h_prime: typing.Optional[NebulaFile] = None
    ...
    with tempfile.TemporaryFile() as f:
        f.write(b"abc")
        f1 = NebulaFile("f1", remote_path=remote_path)
        o = TestFileStruct(
            a=f1,
            b=f1,
            b_prime=None,
...
@dataclass
class A_optional_nebulafile(DataClassJSONMixin):
    a: int


@dataclass
class TestFileStruct_optional_nebulafile(DataClassJSONMixin):
    a: NebulaFile
    b: typing.Optional[NebulaFile]
    b_prime: typing.Optional[NebulaFile]
    c: typing.Union[NebulaFile, None]
    ...
    h: typing.Optional[NebulaFile] = None
    h_prime: typing.Optional[NebulaFile] = None
    i: typing.Optional[A_optional_nebulafile] = None
    i_prime: typing.Optional[A_optional_nebulafile] = field(default_factory=lambda: A_optional_nebulafile(a=99))


@mock.patch("nebulakit.core.data_persistence.FileAccessProvider.put_data")
def test_optional_nebulafile_in_dataclassjsonmixin(mock_upload_dir):
    remote_path = "s3://tmp/file"
    mock_upload_dir.return_value = remote_path
    ...
    with tempfile.TemporaryFile() as f:
        f.write(b"abc")
        f1 = NebulaFile("f1", remote_path=remote_path)
        o = TestFileStruct_optional_nebulafile(
            a=f1,
            b=f1,
            b_prime=None,
            c=f1,
            ...
            g_prime={"a": None},
            h=f1,
            i=A_optional_nebulafile(a=42),
        )

        ctx = NebulaContext.current_context()
        tf = DataclassTransformer()
        lt = tf.get_literal_type(TestFileStruct_optional_nebulafile)
        lv = tf.to_literal(ctx, o, TestFileStruct_optional_nebulafile, lt)
        ...
        ot = tf.to_python_value(ctx, lv=lv, expected_python_type=TestFileStruct_optional_nebulafile)
        ...
        assert ot.h_prime is None
        assert o.i == ot.i
        assert o.i_prime == A_optional_nebulafile(a=99)


def test_nebula_file_in_dataclass():
    @dataclass
    class TestInnerFileStruct(DataClassJsonMixin):
        a: JPEGImageFile
        b: typing.List[NebulaFile]
        c: typing.Dict[str, NebulaFile]
        d: typing.List[NebulaFile]
        e: typing.Dict[str, NebulaFile]
    ...
    remote_path = "s3://tmp/file"
    f1 = NebulaFile(remote_path)
    f2 = NebulaFile("/tmp/file")
...
@dataclass
class TestInnerFileStruct_nebula_file(DataClassJSONMixin):
    a: JPEGImageFile
    b: typing.List[NebulaFile]
    c: typing.Dict[str, NebulaFile]
    d: typing.List[NebulaFile]
...
@dataclass
class TestFileStruct_nebula_file(DataClassJSONMixin):
    a: NebulaFile
    b: TestInnerFileStruct_nebula_file


def test_nebula_file_in_dataclassjsonmixin():
    remote_path = "s3://tmp/file"
    f1 = NebulaFile(remote_path)
    f2 = NebulaFile("/tmp/file")
    f2._remote_source = remote_path
    o = TestFileStruct_nebula_file(
        a=f1,
        b=TestInnerFileStruct_nebula_file(
    ...
    ctx = NebulaContext.current_context()
    tf = DataclassTransformer()
    lt = tf.get_literal_type(TestFileStruct_nebula_file)
    lv = tf.to_literal(ctx, o, TestFileStruct_nebula_file, lt)
    ot = tf.to_python_value(ctx, lv=lv, expected_python_type=TestFileStruct_nebula_file)
    assert ot.a._downloader is not noop
    assert ot.b.a._downloader is not noop
    assert ot.b.b[0]._downloader is not noop
    assert ot.b.c["hello"]._downloader is not noop
...
def test_nebula_directory_in_dataclass():
    @dataclass
    class TestInnerFileStruct(DataClassJsonMixin):
        a: TensorboardLogs
        b: typing.List[NebulaDirectory]
        c: typing.Dict[str, NebulaDirectory]
        d: typing.List[NebulaDirectory]
        e: typing.Dict[str, NebulaDirectory]
    ...
    remote_path = "s3://tmp/file"
    tempdir = tempfile.mkdtemp(prefix="nebula-")
    f1 = NebulaDirectory(tempdir)
    f1._remote_source = remote_path
    f2 = NebulaDirectory(remote_path)
    o = TestFileStruct(
        a=f1,
...
@dataclass
class TestInnerFileStruct_nebula_directory(DataClassJSONMixin):
    a: TensorboardLogs
    b: typing.List[NebulaDirectory]
    c: typing.Dict[str, NebulaDirectory]
    d: typing.List[NebulaDirectory]
...
@dataclass
class TestFileStruct_nebula_directory(DataClassJSONMixin):
    a: NebulaDirectory
    b: TestInnerFileStruct_nebula_directory


def test_nebula_directory_in_dataclassjsonmixin():
    remote_path = "s3://tmp/file"
    tempdir = tempfile.mkdtemp(prefix="nebula-")
    f1 = NebulaDirectory(tempdir)
    f1._remote_source = remote_path
    f2 = NebulaDirectory(remote_path)
    o = TestFileStruct_nebula_directory(
        a=f1,
        b=TestInnerFileStruct_nebula_directory(
    ...
    ctx = NebulaContext.current_context()
    tf = DataclassTransformer()
    lt = tf.get_literal_type(TestFileStruct_nebula_directory)
    lv = tf.to_literal(ctx, o, TestFileStruct_nebula_directory, lt)
    ot = tf.to_python_value(ctx, lv=lv, expected_python_type=TestFileStruct_nebula_directory)
...
def test_structured_dataset_type():
    ...
    assert nebula_tmp_dir in lv.scalar.structured_dataset.uri
    ...
    assert nebula_tmp_dir in subset_lv.scalar.structured_dataset.uri
...
def test_union_type_with_annotated():
    pt = typing.Union[
        Annotated[str, NebulaAnnotation({"hello": "world"})], Annotated[int, NebulaAnnotation({"test": 123})]
    ]
    lt = TypeEngine.to_literal_type(pt)
    assert lt.union_type.variants == [
        LiteralType(
...
def test_annotated_union_type():
    pt = Annotated[typing.Union[str, int], NebulaAnnotation({"hello": "world"})]
    lt = TypeEngine.to_literal_type(pt)
    assert lt.union_type.variants == [
        LiteralType(simple=SimpleType.STRING, structure=TypeStructure(tag="str")),
        LiteralType(simple=SimpleType.INTEGER, structure=TypeStructure(tag="int")),
...
def test_union_custom_transformer_sanity_check():
    ...
    class UnsignedIntTransformer(TypeTransformer[UnsignedInt]):
        ...
        def to_python_value(self, ctx: NebulaContext, lv: Literal, expected_python_type: typing.Type[T]) -> Literal:
...
def test_pickle_type():
    ...
    lt = TypeEngine.to_literal_type(NebulaPickle)
    assert lt.blob.format == NebulaPickleTransformer.PYTHON_PICKLE_FORMAT
    assert lt.blob.dimensionality == BlobType.BlobDimensionality.SINGLE

    ctx = NebulaContextManager.current_context()
    lv = TypeEngine.to_literal(ctx, Foo(1), NebulaPickle, lt)
    assert nebula_tmp_dir in lv.scalar.blob.uri

    transformer = NebulaPickleTransformer()
    ...
    with pytest.raises(AssertionError, match="Cannot pickle None Value"):
        ...
        TypeEngine.to_literal(ctx, None, NebulaPickle, lt)
...
def test_dict_to_literal_map_with_wrong_input_type():
    ...
    with pytest.raises(user_exceptions.NebulaTypeException):
...
def test_annotated_simple_types():
    ...
    _check_annotation(typing_extensions.Annotated[int, NebulaAnnotation({"foo": "bar"})], {"foo": "bar"})
    _check_annotation(typing_extensions.Annotated[int, NebulaAnnotation(["foo", "bar"])], ["foo", "bar"])
    _check_annotation(
        typing_extensions.Annotated[int, NebulaAnnotation({"d": {"test": "data"}, "l": ["nested", ["list"]]})],
        {"d": {"test": "data"}, "l": ["nested", ["list"]]},
    )
    _check_annotation(
        typing_extensions.Annotated[int, NebulaAnnotation(InnerStruct(a=1, b="fizz", c=[1]))],
        InnerStruct(a=1, b="fizz", c=[1]),
    )


def test_annotated_list():
    t = typing_extensions.Annotated[typing.List[int], NebulaAnnotation({"foo": "bar"})]
    lt = TypeEngine.to_literal_type(t)
    assert isinstance(lt.annotation, TypeAnnotation)
    assert lt.annotation.annotations == {"foo": "bar"}

    t = typing.List[typing_extensions.Annotated[int, NebulaAnnotation({"foo": "bar"})]]
    lt = TypeEngine.to_literal_type(t)
...
def test_type_alias():
    inner_t = typing_extensions.Annotated[int, NebulaAnnotation("foo")]
    t = typing_extensions.Annotated[inner_t, NebulaAnnotation("bar")]
    with pytest.raises(ValueError):
        TypeEngine.to_literal_type(t)


def test_unsupported_complex_literals():
    t = typing_extensions.Annotated[typing.Dict[int, str], NebulaAnnotation({"foo": "bar"})]
    with pytest.raises(ValueError):
        TypeEngine.to_literal_type(t)

    # Enum.
    t = typing_extensions.Annotated[Color, NebulaAnnotation({"foo": "bar"})]
    with pytest.raises(ValueError):
        TypeEngine.to_literal_type(t)

    # Dataclass.
    t = typing_extensions.Annotated[Result, NebulaAnnotation({"foo": "bar"})]
    with pytest.raises(ValueError):
        TypeEngine.to_literal_type(t)


def test_multiple_annotations():
    t = typing_extensions.Annotated[int, NebulaAnnotation({"foo": "bar"}), NebulaAnnotation({"anotha": "one"})]
    with pytest.raises(Exception):
        TypeEngine.to_literal_type(t)


TestSchema = NebulaSchema[kwtypes(some_str=str)]  # type: ignore
...
def test_guess_of_dataclass():
    ...
    lv = TypeEngine.to_literal(NebulaContext.current_context(), foo, Foo, lt)
    lit_dict = {"a": lv}
...
def test_nebula_dir_in_union():
    pt = typing.Union[str, NebulaDirectory, NebulaFile]
    lt = TypeEngine.to_literal_type(pt)
    ctx = NebulaContext.current_context()
    tf = UnionTransformer()

    pv = tempfile.mkdtemp(prefix="nebula-")
    lv = tf.to_literal(ctx, NebulaDirectory(pv), pt, lt)
    ot = tf.to_python_value(ctx, lv=lv, expected_python_type=pt)
    assert ot is not None

    pv = "s3://bucket/key"
    lv = tf.to_literal(ctx, NebulaFile(pv), pt, lt)
    ot = tf.to_python_value(ctx, lv=lv, expected_python_type=pt)
    assert ot is not None
...
def test_file_ext_with_nebula_file_existing_file():
...
def test_file_ext_with_nebula_file_new_file():
    TAR_GZ = Annotated[str, FileExt("tar.gz")]
    nebula_file = NebulaFile[TAR_GZ]
    assert nebula_file.extension() == "tar.gz"
...
def test_file_ext_with_nebula_file_wrong_type():
    WRONG_TYPE = Annotated[int, WrongType(2)]
    with pytest.raises(ValueError) as e:
        NebulaFile[WRONG_TYPE]
...
def test_is_batchable():
    ...
    assert ListTransformer.is_batchable(typing.List[typing.Dict[str, NebulaPickle]]) is False
    assert ListTransformer.is_batchable(typing.List[typing.List[NebulaPickle]]) is False

    assert ListTransformer.is_batchable(typing.List[NebulaPickle]) is True
    assert ListTransformer.is_batchable(Annotated[typing.List[NebulaPickle], BatchSize(3)]) is True
    assert (
        ListTransformer.is_batchable(Annotated[typing.List[NebulaPickle], HashMethod(function=str), BatchSize(3)])
        is True
    )
...
@pytest.mark.parametrize(
    ...
    [
        # Case 1: List of NebulaPickle objects with default batch size.
        # (By default, the batch_size is set to the length of the whole list.)
        # After converting to literal, the result will be [batched_NebulaPickle(5 items)].
        # Therefore, the expected list length is [1].
        ([{"foo"}] * 5, typing.List[NebulaPickle], [1]),
        # Case 2: List of NebulaPickle objects with batch size 2.
        # After converting to literal, the result will be
        # [batched_NebulaPickle(2 items), batched_NebulaPickle(2 items), batched_NebulaPickle(1 item)].
        # Therefore, the expected list length is [3].
        (["foo"] * 5, Annotated[typing.List[NebulaPickle], HashMethod(function=str), BatchSize(2)], [3]),
        # Case 3: Nested list of NebulaPickle objects with batch size 2.
        # After converting to literal, the result will be
        # [[batched_NebulaPickle(3 items)], [batched_NebulaPickle(3 items)]]
        # Therefore, the expected list length is [2, 1] (the length of the outer list remains the same, the inner list is batched).
        ([["foo", "foo", "foo"]] * 2, typing.List[Annotated[typing.List[NebulaPickle], BatchSize(3)]], [2, 1]),
        # Case 4: Empty list
        ([[], typing.List[NebulaPickle], []]),
    ],
)
def test_batch_pickle_list(python_val, python_type, expected_list_length):
    ...
    if get_origin(python_type) is Annotated:
        ...
        # @workflow
        # def wf():
        #     data = task0()  # task0() -> Annotated[typing.List[NebulaPickle], BatchSize(2)]
        #     task1(data=data)  # task1(data: typing.List[NebulaPickle])
        assert pv == python_val
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Annotated[typing.Dict[int, str], NebulaAnnotation({"foo": "bar"})], True),
    ],
)
...
def test_DataclassTransformer_to_python_value():
    ...
    result = de.to_python_value(NebulaContext.current_context(), mock_literal, MyDataClass)
    assert isinstance(result, MyDataClass)
    assert result.x == 5

    result = de.to_python_value(NebulaContext.current_context(), mock_literal, MyDataClassMashumaro)
