import pytest
from nebulakitplugins.dask import models
...
def test_create_scheduler_to_nebula_idl_no_optional(image: str, resources: _task.Resources):
    scheduler = models.Scheduler(image=image, resources=resources)
    idl_object = scheduler.to_nebula_idl()
    assert idl_object.image == image
    assert idl_object.resources == resources.to_nebula_idl()


def test_create_scheduler_to_nebula_idl_all_optional(default_resources: _task.Resources):
    scheduler = models.Scheduler(image=None, resources=None)
    idl_object = scheduler.to_nebula_idl()
    assert idl_object.image == ""
    assert idl_object.resources == default_resources.to_nebula_idl()
...
def test_worker_group_to_nebula_idl_no_optional(image: str, resources: _task.Resources):
    n_workers = 1234
    worker_group = models.WorkerGroup(number_of_workers=n_workers, image=image, resources=resources)
    idl_object = worker_group.to_nebula_idl()
    assert idl_object.number_of_workers == n_workers
    assert idl_object.image == image
    assert idl_object.resources == resources.to_nebula_idl()


def test_worker_group_to_nebula_idl_all_optional(default_resources: _task.Resources):
...
def test_dask_job_to_nebula_idl_no_optional(scheduler: models.Scheduler, workers: models.WorkerGroup):
    job = models.DaskJob(scheduler=scheduler, workers=workers)
    idl_object = job.to_nebula_idl()
    assert idl_object.scheduler == scheduler.to_nebula_idl()
    assert idl_object.workers == workers.to_nebula_idl()
