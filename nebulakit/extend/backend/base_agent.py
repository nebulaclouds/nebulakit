...
from nebulaidl.core import literals_pb2
from nebulaidl.core.tasks_pb2 import TaskTemplate
from rich.progress import Progress

import nebulakit
from nebulakit import NebulaContext, PythonFunctionTask, logger
from nebulakit.configuration import SerializationSettings, ImageConfig
...
from nebulakit.exceptions.system import NebulaAgentNotFound
...
class AgentRegistry(object):
    ...
    def get_agent(task_type: str) -> typing.Optional[AgentBase]:
        if task_type not in AgentRegistry._REGISTRY:
            raise NebulaAgentNotFound(f"Cannot find agent for task type: {task_type}.")
        return AgentRegistry._REGISTRY[task_type]


def convert_to_nebula_state(state: str) -> State:
    """
    Convert the state from the agent to the state in nebula.
    """
    state = state.lower()
...
def get_agent_secret(secret_key: str) -> str:
    return nebulakit.current_context().secrets.get(secret_key)
...
class AsyncAgentExecutorMixin:
    ...
    def execute(self, **kwargs) -> typing.Any:
        ...
        if res.resource.state != SUCCEEDED:
            raise NebulaUserException(f"Failed to run the task {self._entity.name}")
        ...
        if task_template.interface.outputs and len(res.resource.outputs.literals) == 0:
            ...
            return LiteralMap.from_nebula_idl(output_proto)

        return LiteralMap.from_nebula_idl(res.resource.outputs)
    ...
    async def _create(
        ...
        if literal_map and isinstance(self, PythonFunctionTask):
            ...
            utils.write_proto_to_file(literal_map.to_nebula_idl(), path)
