...
# This is the env var that the nebulactl sandbox instructions say to set
NEBULACTL_CONFIG_ENV_VAR = "NEBULACTL_CONFIG"
# This is an explicit override only to be used by pynebula and takes precedence in get_config_file over the main
# environment variable.
# This env var should not be set by users
NEBULACTL_CONFIG_ENV_VAR_OVERRIDE = "_NEBULACTL_CONFIG_PYNEBULA_OVERRIDE"
...
class LegacyConfigEntry(object):
    ...
    def get_env_name(self) -> str:
        return f"NEBULA_{self.section.upper()}_{self.option.upper()}"
    ...
    def read_from_env(self, transform: typing.Optional[typing.Callable] = None) -> typing.Optional[typing.Any]:
        ...
        ``NEBULA_{SECTION}_{OPTION}`` all upper cased. We will change this in the future.
        :return:
        """
...
class YamlConfigEntry(object):
    ...
    Args:
        switch: dot-delimited string that should match nebulactl args. Leaving it as dot-delimited instead of a list
          of strings because it's easier to maintain alignment with nebulactl.
...
class ConfigEntry(object):
    ...
    Legacy means the INI style config files. YAML support is for the nebulactl config file, which is there by default
    when nebulactl starts a sandbox
    """
...
class ConfigFile(object):
    ...
    def _get_from_yaml(self, c: YamlConfigEntry) -> typing.Any:
        keys = c.switch.split(".")  # nebulactl switches are dot delimited
        d = self.yaml_config
        try:
            for k in keys:
                d = d[k]
...
def get_config_file(c: typing.Union[str, ConfigFile, None]) -> typing.Optional[ConfigFile]:
    ...
    """
    if c is None:
        # Pynebula override env var takes highest precedence
        # Env var takes second highest precedence
        nebulactl_path_from_env = getenv(NEBULACTL_CONFIG_ENV_VAR_OVERRIDE, getenv(NEBULACTL_CONFIG_ENV_VAR, None))
        if nebulactl_path_from_env:
            nebulactl_path = Path(nebulactl_path_from_env)
            if nebulactl_path.exists():
                logger.info(f"Using nebulactl/YAML config {nebulactl_path.absolute()}")
                return ConfigFile(str(nebulactl_path.absolute()))
            else:
                logger.warning(f"nebulactl config file {nebulactl_path.absolute()} does not exist, ignoring...")

        # See if there's a config file in the current directory where Python is being run from
        current_location_config = Path("nebulakit.config")
        ...
        home_dir_config = Path(Path.home(), ".nebula", "config")  # _default_config_file_name in main.py
        ...
        nebulactl_path = Path(Path.home(), ".nebula", "config.yaml")
        if nebulactl_path.exists():
            logger.info(f"Using nebulactl/YAML config {nebulactl_path.absolute()}")
            return ConfigFile(str(nebulactl_path.absolute()))
