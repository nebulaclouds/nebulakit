...
from nebulakit import Annotations, NebulaContext, NebulaContextManager, Labels, Literal
from nebulakit.clis.sdk_in_container.helpers import get_remote, patch_image_config
from nebulakit.clis.sdk_in_container.utils import (
    PyNebulaParams,
    domain_option,
...
from nebulakit.configuration import DefaultImages, FastSerializationSettings, ImageConfig, SerializationSettings
...
from nebulakit.core.workflow import PythonFunctionWorkflow, WorkflowBase
from nebulakit.exceptions.system import NebulaSystemException
from nebulakit.interaction.click_types import NebulaLiteralConverter, key_value_callback
from nebulakit.models import security
from nebulakit.models.common import RawOutputDataConfig
from nebulakit.models.interface import Parameter, Variable
from nebulakit.models.types import SimpleType
from nebulakit.remote import NebulaLaunchPlan, NebulaRemote, NebulaTask, NebulaWorkflow, remote_fs
from nebulakit.remote.executions import NebulaWorkflowExecution
from nebulakit.tools import module_loader
from nebulakit.tools.script_mode import _find_project_root, compress_scripts
...
@dataclass
class RunLevelParams(PyNebulaParams):
    """
    ...
    dump_snippet: bool = make_click_option_field(
        click.Option(
            ...
            default=False,
            show_default=True,
            help="Whether to dump a code snippet instructing how to load the workflow execution using nebularemote",
        )
    )
    ...
    raw_output_data_prefix: str = make_click_option_field(
        click.Option(
            ...
            + click.style(
                ...
                "used to store Nebulafile, Nebuladirectory, Structuredataset,"
                " dataframes"
            ),
        )
    )
    ...
    remote: bool = make_click_option_field(
        click.Option(
            ...
            is_eager=True,
            show_default=True,
            help="Whether to register and run the workflow on a Nebula deployment",
        )
    )
    ...
    _remote: typing.Optional[NebulaRemote] = None

    def remote_instance(self) -> NebulaRemote:
    ...
    def options(cls) -> typing.List[click.Option]:
        """
        Return the set of base parameters added to every pynebula run workflow subcommand.
        """
...
def load_naive_entity(module_name: str, entity_name: str, project_root: str) -> typing.Union[WorkflowBase, PythonTask]:
    ...
    """
    nebula_ctx_builder = context_manager.NebulaContextManager.current_context().new_builder()
    with context_manager.NebulaContextManager.with_context(nebula_ctx_builder):
...
def dump_nebula_remote_snippet(execution: NebulaWorkflowExecution, project: str, domain: str):
    click.secho(
        f"""
...
remote = NebulaRemote(Config.auto(), default_project="{project}", default_domain="{domain}")
...
def get_entities_in_file(filename: pathlib.Path, should_delete: bool) -> Entities:
    """
    Returns a list of nebula workflow names and list of Nebula tasks in a file.
    """
    nebula_ctx = context_manager.NebulaContextManager.current_context().new_builder()
    module_name = os.path.splitext(os.path.relpath(filename))[0].replace(os.path.sep, ".")
    with context_manager.NebulaContextManager.with_context(nebula_ctx):
...
def run_remote(
    remote: NebulaRemote,
    entity: typing.Union[NebulaWorkflow, NebulaTask, NebulaLaunchPlan],
    project: str,
    domain: str,
    ...
    """
    Helper method that executes the given remote NebulaLaunchplan, NebulaWorkflow or NebulaTask
    """
    ...
    if run_level_params.dump_snippet:
        dump_nebula_remote_snippet(execution, project, domain)


def _update_nebula_context(params: RunLevelParams) -> NebulaContext.Builder:
    # Update the nebula context for the local execution.
    ...
    file_access = FileAccessProvider(
        local_sandbox_dir=tempfile.mkdtemp(prefix="nebula"), raw_output_prefix=output_prefix
    )

    # The task might run on a remote machine if raw_output_prefix is a remote path,
    # so workflow file needs to be uploaded to the remote location to make pynebula-fast-execute work.
...
def run_command(ctx: click.Context, entity: typing.Union[PythonFunctionWorkflow, PythonTask]):
    """
    Returns a function that is used to implement WorkflowCommand and execute a nebula workflow.
    """

    def _run(*args, **kwargs):
        """
        Click command function that is used to execute a nebula workflow from the given entity in the file.
        """
        ...
        try:
            ...
            if not run_level_params.is_remote:
                with NebulaContextManager.with_context(_update_nebula_context(run_level_params)):
            ...
            with context_manager.NebulaContextManager.with_context(remote.context.new_builder()):
...
class DynamicLaunchPlanCommand(click.RichCommand):
    ...
    def _fetch_launch_plan(self, ctx: click.Context) -> NebulaLaunchPlan:
        if self._lp:
            return self._lp
        run_level_params: RunLevelParams = ctx.obj
    ...
    def _get_params(
        ...
        params = []
        nebula_ctx = context_manager.NebulaContextManager.current_context()
        for name, var in inputs.items():
            ...
            if defaults and name in defaults:
                required = False
            params.append(to_click_option(ctx, nebula_ctx, name, var, native_inputs[name], None, required))
        return params
...
class RemoteLaunchPlanGroup(click.RichGroup):
    """
    click multicommand that retrieves launchplans from a remote nebula instance and executes them.
    """
    ...
    def __init__(self):
        super().__init__(
            name="from-server",
            help="Retrieve launchplans from a remote nebula instance and execute them.",
            params=[
                click.Option(
    ...
    def list_commands(self, ctx):
        ...
        with progress:
            ...
            try:
                ...
                return self._lps
            except NebulaSystemException as e:
...
class WorkflowCommand(click.RichGroup):
    ...
    def _create_command(
        ...
        for input_name, input_type_val in loaded_entity.python_interface.inputs_with_defaults.items():
            ...
            params.append(to_click_option(ctx, nebula_ctx, input_name, literal_var, python_type, default_val, required))
    ...
    def get_command(self, ctx, exe_entity):
        ...
        :param exe_entity: string of the nebula entity provided by the user. Should be the name of a workflow, or task
          function.
        :return:
        """
        is_workflow = False
        ...
        if rel_path.startswith(".."):
            raise ValueError(
                f"You must call pynebula from the same or parent dir, {self._filename} not under {os.getcwd()}"
            )
...
class RunCommand(click.RichGroup):
    """
    A click command group for registering and executing nebula workflows & tasks in a file.
    """
