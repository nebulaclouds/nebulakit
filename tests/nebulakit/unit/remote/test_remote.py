...
from nebulaidl.service import dataproxy_pb2
from mock import ANY, MagicMock, patch

import nebulakit.configuration
from nebulakit import CronSchedule, LaunchPlan, WorkflowFailurePolicy, task, workflow
from nebulakit.configuration import Config, DefaultImages, Image, ImageConfig, SerializationSettings
...
from nebulakit.models.admin.workflow import Workflow, WorkflowClosure
from nebulakit.models.core.compiler import CompiledWorkflowClosure
from nebulakit.models.core.identifier import Identifier, ResourceType, WorkflowExecutionIdentifier
from nebulakit.models.execution import Execution
from nebulakit.models.task import Task
from nebulakit.remote import NebulaTask
from nebulakit.remote.lazy_entity import LazyEntity
from nebulakit.remote.remote import NebulaRemote, _get_git_repo_url
from nebulakit.tools.translator import Options, get_serializable, get_serializable_launch_plan
from tests.nebulakit.common.parameterizers import LIST_OF_TASK_CLOSURES
...
def test_remote_fetch_execution(remote):
    ...
    nebula_workflow_execution = remote.fetch_execution(name="n1")
    assert nebula_workflow_execution.id == admin_workflow_execution.id


@pytest.fixture
def mock_wf_exec():
    return patch("nebulakit.remote.executions.NebulaWorkflowExecution.promote_from_model")
...
def test_execute_with_wrong_input_key(remote, mock_wf_exec):
    ...
    with pytest.raises(user_exceptions.NebulaValueException):
        remote._execute(
            mock_entity,
            inputs={"bar": 3},
            project="proj",
...
def test_form_config():
    remote = NebulaRemote(config=Config.auto(), default_project="p1", default_domain="d1")
    assert remote.default_project == "p1"
    assert remote.default_domain == "d1"


@patch("nebulakit.remote.remote.SynchronousNebulaClient")
def test_passing_of_kwargs(mock_client):
    ...
    NebulaRemote(config=Config.auto(), default_project="project", default_domain="domain", **additional_args).client
...
def test_more_stuff(mock_client):
    r = NebulaRemote(config=Config.auto(), default_project="project", default_domain="domain")
...
def test_get_extra_headers_azure_blob_storage():
    native_url = "abfs://nebula@storageaccount/container/path/to/file"
    headers = NebulaRemote.get_extra_headers_for_protocol(native_url)
    assert headers["x-ms-blob-type"] == "BlockBlob"


def test_get_extra_headers_s3():
    native_url = "s3://nebula@storageaccount/container/path/to/file"
    headers = NebulaRemote.get_extra_headers_for_protocol(native_url)
    assert headers == {}
...
def test_generate_console_http_domain_sandbox_rewrite(mock_client):
    ...
    try:
        with open(temp_filename, "w") as f:
            # This string is similar to the relevant configuration emitted by nebulactl in the cases of both demo and sandbox.
            nebulactl_config_file = """admin:
    endpoint: example.com
    authType: Pkce
    insecure: false
"""
            f.write(nebulactl_config_file)
...
def get_compiled_workflow_closure():
    ...
    return CompiledWorkflowClosure.from_nebula_idl(cwc_pb)
...
def test_local_server(mock_client):
    ...
    lr = rr.get("nebula://v1/nebulasnacks/development/f6988c7bdad554a4da7a/n0/o")
    assert lr.get("hello", int) == 55


@mock.patch("nebulakit.remote.remote.uuid")
@mock.patch("nebulakit.remote.remote.NebulaRemote.client")
def test_execution_name(mock_client, mock_uuid):
    ...
    remote = NebulaRemote(config=Config.auto(), default_project="project", default_domain="domain")
    ...
    ft = NebulaTask.promote_from_model(tk_spec.template)
...
@pytest.mark.parametrize(
    "url, host",
    [
        ("https://github.com/nebulakit/nebulakit", "github.com"),
        ("http://github.com/nebulakit/nebulakit", "github.com"),
        ("git@github.com:nebulakit/nebulakit.git", "github.com"),
        ("https://gitlab.com/nebulakit/nebulakit", "gitlab.com"),
        ("git@gitlab.com:nebulakit/nebulakit.git", "gitlab.com"),
    ],
)
def test_get_git_repo_url(url, host, tmp_path):
    ...
    assert returned_url == f"{host}/nebulakit/nebulakit"
