...
from nebulakit.extend import T, TypeEngine, TypeTransformer
from nebulakit.models.literals import Literal, Scalar, Schema
from nebulakit.models.types import LiteralType, SchemaType
from nebulakit.types.schema import SchemaEngine, SchemaFormat, SchemaHandler, SchemaReader, SchemaWriter


class SparkDataFrameSchemaReader(SchemaReader[pyspark.sql.DataFrame]):
    """
    Implements how SparkDataFrame should be read using the ``open`` method of NebulaSchema
    """
    ...
    def all(self, **kwargs) -> pyspark.sql.DataFrame:
        if self._fmt == SchemaFormat.PARQUET:
            ctx = NebulaContext.current_context().user_space_params
...
class SparkDataFrameSchemaWriter(SchemaWriter[pyspark.sql.DataFrame]):
    """
    Implements how SparkDataFrame should be written to using ``open`` method of NebulaSchema
    """
...
class SparkDataFrameTransformer(TypeTransformer[pyspark.sql.DataFrame]):
    ...
    def to_python_value(self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[pyspark.sql.DataFrame]) -> T:
...
# %%
# Registers a handle for Spark DataFrame + Nebula Schema type transition
...
# %%
# This makes pyspark.DataFrame as a supported output/input type with nebulakit.
