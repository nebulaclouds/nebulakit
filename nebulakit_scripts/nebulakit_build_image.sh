...
# NB: This script is bundled with the NebulaKit SDK and is the recommended way for users to build their images.
# This build script ensures that the resulting workflow image that the Nebula ecosystem executes is built
# in a standardized way, complete with all the environment variables that the rest of the system will rely on.

if [ -z "$1" ]; then
    echo usage: ./nebulakit_build_image.sh [/path/to/Dockerfile or /path/to/repo] [optional tag prefix]
    exit
fi
...
if [ -n "$REGISTRY" ]; then
  NEBULA_INTERNAL_IMAGE=${REGISTRY}/${IMAGE_NAME}:${PREFIX}${TAG}
else
  NEBULA_INTERNAL_IMAGE=${IMAGE_NAME}:${PREFIX}${TAG}
fi
...
echo "Building: $NEBULA_INTERNAL_IMAGE for $TARGET_PLATFORM_BUILD architecture"

# This build command is the raison d'etre of this script, it ensures that the version is injected into the image itself
docker build . "${DOCKER_PLATFORM_OPT[@]}" --build-arg tag="${NEBULA_INTERNAL_IMAGE}" -t "${NEBULA_INTERNAL_IMAGE}" -f "${DOCKERFILE_PATH}"

echo "$IMAGE_NAME built locally."

# Create the appropriate tags
echo "${NEBULA_INTERNAL_IMAGE} tagged"
if [ -n "$REGISTRY" ]; then
  ...
  if [[ -z "${NOPUSH}" ]]; then
    ...
    echo "${NEBULA_INTERNAL_IMAGE} pushed to remote"
  fi
fi
