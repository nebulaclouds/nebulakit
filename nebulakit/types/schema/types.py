...
from nebulakit.core.type_engine import TypeEngine, TypeTransformer, TypeTransformerFailedError
...
class SchemaEngine(object):
    ...
    to allow direct support for that type in NebulaSchema.
...
@dataclass
class NebulaSchema(DataClassJSONMixin):
    ...
    def __class_getitem__(
        ...
        logger.warning("NebulaSchema is deprecated, use Structured Dataset instead.")
        if columns is None:
            return NebulaSchema
        ...
        if not isinstance(fmt, SchemaFormat):
            raise AssertionError(
                f"Only NebulaSchemaFormat types are supported, received format is {fmt} of type {type(fmt)}"
            )

        class _TypedSchema(NebulaSchema):
            # Get the type engine to see this as kind of a generic
            __origin__ = NebulaSchema
    ...
    def __init__(
        ...
        if supported_mode == SchemaOpenMode.READ and remote_path is None:
            raise ValueError("To create a NebulaSchema in read mode, remote_path is required")
        if (
            supported_mode == SchemaOpenMode.WRITE
            and local_path is None
            and NebulaContextManager.current_context().file_access is None
        ):
            raise ValueError("To create a NebulaSchema in write mode, local_path is required")

        local_path = local_path or NebulaContextManager.current_context().file_access.get_random_local_directory()
        ...
        fp = NebulaContextManager.current_context().file_access
    ...
    def as_readonly(self) -> NebulaSchema:
        if self._supported_mode == SchemaOpenMode.READ:
            return self
        s = NebulaSchema.__class_getitem__(self.columns(), self.format())(
...
class NebulaSchemaTransformer(TypeTransformer[NebulaSchema]):
    ...
    def __init__(self):
        super().__init__("NebulaSchema Transformer", NebulaSchema)

    def _get_schema_type(self, t: Type[NebulaSchema]) -> SchemaType:
        converted_cols: typing.List[SchemaType.SchemaColumn] = []
        for k, v in t.columns().items():
            if v not in self._SUPPORTED_TYPES:
                raise AssertionError(f"type {v} is currently not supported by NebulaSchema")
    ...
    def assert_type(self, t: Type[NebulaSchema], v: typing.Any):
        if issubclass(t, NebulaSchema) or isinstance(v, NebulaSchema):
            return
        try:
            SchemaEngine.get_handler(type(v))
        except ValueError as e:
            raise TypeError(f"No automatic conversion found from type {type(v)} to NebulaSchema") from e

    def get_literal_type(self, t: Type[NebulaSchema]) -> LiteralType:
        return LiteralType(schema=self._get_schema_type(t))

    def to_literal(
        self, ctx: NebulaContext, python_val: NebulaSchema, python_type: Type[NebulaSchema], expected: LiteralType
    ) -> Literal:
        if isinstance(python_val, NebulaSchema):
    ...
    def to_python_value(self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[NebulaSchema]) -> NebulaSchema:
        def downloader(x, y):
        ...
        if not (lv and lv.scalar and lv.scalar.schema):
            raise AssertionError("Can only convert a literal schema to a NebulaSchema")
    ...
    def guess_python_type(self, literal_type: LiteralType) -> Type[NebulaSchema]:
        ...
        return NebulaSchema.__class_getitem__(columns)


TypeEngine.register(NebulaSchemaTransformer())
