"""
Nebulakit map tasks specify how to run a single task across a list of inputs. Map tasks themselves are constructed with
...
from nebulakit.core import tracker
from nebulakit.core.base_task import PythonTask, Task, TaskResolverMixin
from nebulakit.core.constants import CONTAINER_ARRAY_TASK
from nebulakit.core.context_manager import ExecutionState, NebulaContext, NebulaContextManager
from nebulakit.core.interface import transform_interface_to_list_interface
...
from nebulakit.models.array_job import ArrayJob
from nebulakit.models.interface import Variable
from nebulakit.models.task import Container, K8sPod, Sql
from nebulakit.tools.module_loader import load_object_from_module


class MapPythonTask(PythonTask):
    """
    A MapPythonTask defines a :py:class:`nebulakit.PythonTask` which specifies how to run
    an inner :py:class:`nebulakit.PythonFunctionTask` across a range of inputs in parallel.
    """
    ...
    def get_type_for_output_var(self, k: str, v: Any) -> type:
        """
        We override this method from nebulakit.core.base_task Task because the dispatch_execute method uses this
        interface to construct outputs. Each instance of an container_array task will however produce outputs
        according to the underlying run_task interface and the array plugin handler will actually create a collection
        from these individual outputs as the final output value.
        """
    ...
    def _execute_map_task(self, _: NebulaContext, **kwargs) -> Any:
        """
        This is called during ExecutionState.Mode.TASK_EXECUTION executions, that is executions orchestrated by the
        Nebula platform. Individual instances of the map task, aka array task jobs are passed the full set of inputs but
    ...
    def _raw_execute(self, **kwargs) -> Any:
        """
        This is called during locally run executions. Unlike array task execution on the Nebula platform, _raw_execute
...
def map_task(
    ...
    **kwargs,
    ...
    any individual :py:class:`nebulakit.PythonFunctionTask`.

    Invoke a map task with arguments using the :py:class:`list` version of the expected input.

    Usage:

    .. literalinclude:: ../../../tests/nebulakit/unit/core/test_map_task.py
    ...
    such as :py:class:`nebulakit.TaskMetadata` and ``with_overrides`` are applied to individual instances
    of the mapped task.

    **Map Task Plugins**

    There are two plugins to run maptasks that ship as part of nebulaplugins:

    1. K8s Array
    2. `AWS batch <https://docs.nebula.org/en/latest/deployment/plugin_setup/aws/batch.html>`_

    Enabling a plugin is controlled in the plugin configuration at `values-sandbox.yaml <https://github.com/nebulaclouds/nebula/blob/10cee9f139824512b6c5be1667d321bdbc8835fa/charts/nebula/values-sandbox.yaml#L152-L162>`_.
    ...
    **AWS batch**
    ...
    Learn more about ``AWS batch`` setup configuration `here <https://docs.nebula.org/en/latest/deployment/plugin_setup/aws/batch.html#deployment-plugin-setup-aws-array>`_.
