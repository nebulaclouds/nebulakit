"""
==============================
:mod:`nebulakit.core.base_task`
==============================

.. currentmodule:: nebulakit.core.base_task
...
from nebulakit.core.interface import Interface, transform_interface_to_typed_interface
from nebulakit.core.local_cache import LocalTaskCache
...
from nebulakit.core.type_engine import TypeEngine, TypeTransformerFailedError
...
from nebulakit.models.interface import Variable
from nebulakit.models.security import SecurityContext
    ...
    def to_taskmetadata_model(self) -> _task_model.TaskMetadata:
        ...
        return _task_model.TaskMetadata(
            ...
            runtime=_task_model.RuntimeMetadata(
                _task_model.RuntimeMetadata.RuntimeType.NEBULA_SDK, __version__, "python"
            ),
...
class IgnoreOutputs(Exception):
    ...
    :py:class:`plugins.awssagemaker.nebulakitplugins.awssagemaker.training.SagemakerBuiltinAlgorithmsTask`.
    """
...
class Task(object):
    """
    The base of all Tasks in nebulakit. This task is closest to the NebulaIDL TaskTemplate and captures information in
    NebulaIDL specification and does not have python native interfaces associated. Refer to the derived classes for
    ...
    def local_execute(
        self, ctx: NebulaContext, **kwargs
    ) -> Union[Tuple[Promise], Promise, VoidPromise, Coroutine, None]:
        """
        This function is used only in the local execution path and is responsible for calling dispatch execute.
        Use this function when calling a task with native values (or Promises containing Nebula literals derived from
    ...
    def __call__(self, *args: object, **kwargs: object) -> Union[Tuple[Promise], Promise, VoidPromise, Tuple, None]:
        return nebula_entity_call_handler(self, *args, **kwargs)  # type: ignore
    ...
    def get_container(self, settings: SerializationSettings) -> Optional[_task_model.Container]:
        """
        Returns the container definition (if any) that is used to run the task on hosted Nebula.
        """
        return None

    def get_k8s_pod(self, settings: SerializationSettings) -> Optional[_task_model.K8sPod]:
        """
        Returns the kubernetes pod definition (if any) that is used to run the task on hosted Nebula.
        """
        return None

    def get_sql(self, settings: SerializationSettings) -> Optional[_task_model.Sql]:
        """
        Returns the Sql definition (if any) that is used to run the task on hosted Nebula.
        """
        return None
    ...
    def get_extended_resources(self, settings: SerializationSettings) -> Optional[tasks_pb2.ExtendedResources]:
        """
        Returns the extended resources to allocate to the task on hosted Nebula.
        """
        return None
    ...
    def dispatch_execute(
        ...
        """
        This method translates Nebula's Type system based input values and invokes the actual call to the executor
...
class PythonTask(TrackedInstance, Task, Generic[T]):
    ...
    not have a python function to be executed. Otherwise refer to :py:class:`nebulakit.PythonFunctionTask`.
    """
    ...
    # TODO lets call this interface and the other as nebula_interface?
    @property
    ...
    def compile(self, ctx: NebulaContext, *args, **kwargs) -> Optional[Union[Tuple[Promise], Promise, VoidPromise]]:
        """
    ...
    def _write_decks(self, native_inputs, native_outputs_as_map, ctx, new_user_params):
        if self._disable_deck is False:
            from nebulakit.deck.deck import Deck, _output_deck
            ...
            if ctx.execution_state and ctx.execution_state.is_local_execution():
                # When we run the workflow remotely, nebulakit outputs decks at the end of _dispatch_execute
    ...
    def dispatch_execute(
        self, ctx: NebulaContext, input_literal_map: _literal_models.LiteralMap
...
class TaskResolverMixin(object):
    """
    Nebulakit tasks interact with the Nebula platform very, very broadly in two steps. They need to be uploaded to Admin,
    ...
    might result in a container with arguments like ::
        ...
        pynebula-execute --inputs s3://path/inputs.pb --output-prefix s3://outputs/location \
        --raw-output-data-prefix /tmp/data \
        --resolver nebulakit.core.python_auto_container.default_task_resolver \
        -- \
        task-module repo_root.workflows.example task-name t1

    At serialization time, the container created for the task will start out automatically with the ``pynebula-execute``
