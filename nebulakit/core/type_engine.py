...
from nebulakit.core.type_helpers import load_type_from_tag
from nebulakit.core.utils import timeit
from nebulakit.exceptions import user as user_exceptions
from nebulakit.interaction.string_literals import literal_map_string_repr
from nebulakit.lazy_import.lazy_module import is_imported
...
from nebulakit.models.annotation import TypeAnnotation as TypeAnnotationModel
from nebulakit.models.core import types as _core_types
from nebulakit.models.literals import (
    Blob,
    BlobMetadata,
    Literal,
    LiteralCollection,
...
from nebulakit.models.types import LiteralType, SimpleType, StructuredDatasetType, TypeStructure, UnionType
...
class BatchSize:
    """
    This is used to annotate a NebulaDirectory when we want to download/upload the contents of the directory in batches. For example,
    ...
    In the above example nebulakit will download all files from the input `directory` in chunks of 10, i.e. first it
    downloads 10 files, loads them to memory, then writes those 10 to local disk, then it loads the next 10, so on
    and so forth. Similarly, for outputs, in this case nebulakit is going to upload the resulting directory in chunks of
    100.
    """
...
def modify_literal_uris(lit: Literal):
    ...
    type "nebula://"
    """
    from nebulakit.remote.remote_fs import NebulaPathResolver
    ...
    if lit.collection:
        ...
        if lit.scalar.blob and lit.scalar.blob.uri and lit.scalar.blob.uri.startswith(NebulaPathResolver.protocol):
            lit.scalar.blob._uri = NebulaPathResolver.resolve_remote_path(lit.scalar.blob.uri)
            ...
            and lit.scalar.structured_dataset.uri.startswith(NebulaPathResolver.protocol)
        ):
            lit.scalar.structured_dataset._uri = NebulaPathResolver.resolve_remote_path(
...
class TypeTransformer(typing.Generic[T]):
    """
    Base transformer type that should be implemented for every python native type that can be handled by nebulakit
    """
    ...
    def get_literal_type(self, t: Type[T]) -> LiteralType:
        """
        Converts the python type to a Nebula LiteralType
        """
        raise NotImplementedError("Conversion to LiteralType should be implemented")

    def guess_python_type(self, literal_type: LiteralType) -> Type[T]:
        """
        Converts the Nebula LiteralType to a python object type.
        """
        raise ValueError("By default, transformers do not translate from Nebula types back to Python types")

    @abstractmethod
    def to_literal(self, ctx: NebulaContext, python_val: T, python_type: Type[T], expected: LiteralType) -> Literal:
        """
        Converts a given python_val to a Nebula Literal, assuming the given python_val matches the declared python_type.
        Implementers should refrain from using type(python_val) instead rely on the passed in python_type. If these
        do not match (or are not allowed) the Transformer implementer should raise an AssertionError, clearly stating
        what was the mismatch
        :param ctx: A NebulaContext, useful in accessing the filesystem and other attributes
    ...
    @abstractmethod
    def to_python_value(self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[T]) -> Optional[T]:
        """
        Converts the given Literal to a Python Type. If the conversion cannot be done an AssertionError should be raised
        :param ctx: NebulaContext
    ...
    def to_html(self, ctx: NebulaContext, python_val: T, expected_python_type: Type[T]) -> str:
        """
    ...
    def __repr__(self):
        return f"{self._name} Transforms ({self._t}) to Nebula native"
...
class SimpleTransformer(TypeTransformer[T]):
    ...
    def get_literal_type(self, t: Optional[Type[T]] = None) -> LiteralType:
        return LiteralType.from_nebula_idl(self._lt.to_nebula_idl())
...
class DataclassTransformer(TypeTransformer[object]):
    ...
    def assert_type(self, expected_type: Type[DataClassJsonMixin], v: T):
        ...
        # 2. when using nebula remote to execute the above task, the expected_type is guess_python_type (FooSchema) by default.
        # However, FooSchema is created by nebulakit and it's not equal to the user-defined dataclass (Foo).
    ...
    def get_literal_type(self, t: Type[T]) -> LiteralType:
        ...
        if is_annotated(t):
            raise ValueError(
                "Nebulakit does not currently have support for NebulaAnnotations applied to Dataclass."
    ...
    def to_literal(self, ctx: NebulaContext, python_val: T, python_type: Type[T], expected: LiteralType) -> Literal:
        if not dataclasses.is_dataclass(python_val):
            raise TypeTransformerFailedError(
                ...
                f"user defined datatypes in Nebulakit"
            )
        ...
        self._serialize_nebula_type(python_val, python_type)
    ...
    def _serialize_nebula_type(self, python_val: T, python_type: Type[T]) -> typing.Any:
        """
        If any field inside the dataclass is nebula type, we should use nebula type transformer for that field.
        """
        from nebulakit.types.directory.types import NebulaDirectory
        from nebulakit.types.file import NebulaFile
        from nebulakit.types.schema.types import NebulaSchema
        ...
        if get_origin(python_type) is typing.Union and type(None) in get_args(python_type):
            if python_val is None:
                return None
            return self._serialize_nebula_type(python_val, get_args(python_type)[0])

        if hasattr(python_type, "__origin__") and get_origin(python_type) is list:
            return [self._serialize_nebula_type(v, get_args(python_type)[0]) for v in cast(list, python_val)]

        if hasattr(python_type, "__origin__") and get_origin(python_type) is dict:
            return {
                k: self._serialize_nebula_type(v, get_args(python_type)[1]) for k, v in cast(dict, python_val).items()
            }
        ...
        if inspect.isclass(python_type) and (
            issubclass(python_type, NebulaSchema)
            or issubclass(python_type, NebulaFile)
            or issubclass(python_type, NebulaDirectory)
            or issubclass(python_type, StructuredDataset)
        ):
            lv = TypeEngine.to_literal(NebulaContext.current_context(), python_val, python_type, None)
            # dataclasses_json package will extract the "path" from NebulaFile, NebulaDirectory, and write it to a
            # JSON which will be stored in IDL. The path here should always be a remote path, but sometimes the
            # path in NebulaFile and NebulaDirectory could be a local path. Therefore, reset the python value here,
            ...
            if issubclass(python_type, NebulaFile) or issubclass(python_type, NebulaDirectory):
            ...
            for v in dataclasses.fields(python_type):
                ...
                field_type = v.type
                python_val.__setattr__(v.name, self._serialize_nebula_type(val, field_type))
            return python_val

    def _deserialize_nebula_type(self, python_val: T, expected_python_type: Type) -> Optional[T]:
        ...
        from nebulakit.types.structured.structured_dataset import StructuredDataset, StructuredDatasetTransformerEngine

        # Handle Optional
        if get_origin(expected_python_type) is typing.Union and type(None) in get_args(expected_python_type):
            if python_val is None:
                return None
            return self._deserialize_nebula_type(python_val, get_args(expected_python_type)[0])

        if hasattr(expected_python_type, "__origin__") and expected_python_type.__origin__ is list:
            return [self._deserialize_nebula_type(v, expected_python_type.__args__[0]) for v in python_val]  # type: ignore

        if hasattr(expected_python_type, "__origin__") and expected_python_type.__origin__ is dict:
            return {k: self._deserialize_nebula_type(v, expected_python_type.__args__[1]) for k, v in python_val.items()}  # type: ignore

        if not dataclasses.is_dataclass(expected_python_type):
            return python_val

        if issubclass(expected_python_type, NebulaSchema):
            t = NebulaSchemaTransformer()
            return t.to_python_value(
                NebulaContext.current_context(),
                Literal(
                    scalar=Scalar(
                        schema=Schema(
                            cast(NebulaSchema, python_val).remote_path, t._get_schema_type(expected_python_type)
                        )
                    )
                ),
                expected_python_type,
            )
        elif issubclass(expected_python_type, NebulaFile):
            return NebulaFilePathTransformer().to_python_value(
                NebulaContext.current_context(),
                Literal(
                    scalar=Scalar(
                        blob=Blob(
                            ...
                            uri=cast(NebulaFile, python_val).path,
                        )
                    )
                ),
                expected_python_type,
            )
        elif issubclass(expected_python_type, NebulaDirectory):
            return NebulaDirToMultipartBlobTransformer().to_python_value(
                NebulaContext.current_context(),
                Literal(
                    scalar=Scalar(
                        blob=Blob(
                            ...
                            uri=cast(NebulaDirectory, python_val).path,
                        )
                    )
                ),
                expected_python_type,
            ...
            for f in dataclasses.fields(expected_python_type):
                ...
                if hasattr(f.type, "__origin__") and f.type.__origin__ is list:
                    value = [self._deserialize_nebula_type(v, f.type.__args__[0]) for v in value]
                elif hasattr(f.type, "__origin__") and f.type.__origin__ is dict:
                    value = {k: self._deserialize_nebula_type(v, f.type.__args__[1]) for k, v in value.items()}
                else:
                    value = self._deserialize_nebula_type(value, f.type)
    ...
    def to_python_value(self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[T]) -> T:
        if not dataclasses.is_dataclass(expected_python_type):
            raise TypeTransformerFailedError(
                ...
                "user defined datatypes in Nebulakit"
            )
        ...
        return self._fix_dataclass_int(expected_python_type, self._deserialize_nebula_type(dc, expected_python_type))

    # This ensures that calls with the same literal type returns the same dataclass. For example, `pynebula run``
...
class EnumTransformer(TypeTransformer[enum.Enum]):
    ...
    def get_literal_type(self, t: Type[T]) -> LiteralType:
        if is_annotated(t):
            raise ValueError(
                f"Nebulakit does not currently have support \
                    for NebulaAnnotations applied to enums. {t} cannot be \
                    parsed."
            )
    ...
    def to_literal(
        self, ctx: NebulaContext, python_val: enum.Enum, python_type: Type[T], expected: LiteralType
    ) -> Literal:
...
class TypeEngine(typing.Generic[T]):
    """
    Core Extensible TypeEngine of Nebulakit. This should be used to extend the capabilities of NebulaKits type system.
    ...
    def get_transformer(cls, python_type: Type) -> TypeTransformer[T]:
        """
        The TypeEngine hierarchy for nebulaKit. This method looksup and selects the type transformer. The algorithm is
        as follows
        ...
        if hasattr(python_type, "__origin__"):
            ...
            raise ValueError(f"Generic Type {python_type.__origin__} not supported currently in Nebulakit.")
        ...
        return NebulaPickleTransformer()
    ...
    def lazy_import_transformers(cls):
        ...
        from nebulakit.types.structured import (
        ...
        if is_imported("tensorflow"):
            from nebulakit.extras import tensorflow  # noqa: F401
        if is_imported("torch"):
            from nebulakit.extras import pytorch  # noqa: F401
        if is_imported("sklearn"):
            from nebulakit.extras import sklearn  # noqa: F401
        if is_imported("pandas"):
            try:
                from nebulakit.types import schema  # noqa: F401
            except ValueError:
        ...
        if is_imported("numpy"):
            from nebulakit.types import numpy  # noqa: F401
        if is_imported("PIL"):
            from nebulakit.types.file import image  # noqa: F401
    ...
    def to_literal_type(cls, python_type: Type) -> LiteralType:
        ...
        if is_annotated(python_type):
            for x in get_args(python_type)[1:]:
                if not isinstance(x, NebulaAnnotation):
                    continue
                if data is not None:
                    raise ValueError(
                        f"More than one NebulaAnnotation used within {python_type} typehint. Nebulakit requires a max of one."
                    )
                data = x.data
        if data is not None:
            idl_type_annotation = TypeAnnotationModel(annotations=data)
            res = LiteralType.from_nebula_idl(res.to_nebula_idl())
            res._annotation = idl_type_annotation
        return res

    @classmethod
    def to_literal(cls, ctx: NebulaContext, python_val: typing.Any, python_type: Type, expected: LiteralType) -> Literal:
        """
        Converts a python value of a given type and expected ``LiteralType`` into a resolved ``Literal`` value.
        """
        from nebulakit.core.promise import Promise, VoidPromise
        ...
        if isinstance(python_val, tuple):
            raise AssertionError(
                "Tuples are not a supported type for individual values in Nebula - got a tuple -"
    ...
    @classmethod
    def to_python_value(cls, ctx: NebulaContext, lv: Literal, expected_python_type: Type) -> typing.Any:
        """
    ...
    @classmethod
    def to_html(cls, ctx: NebulaContext, python_val: typing.Any, expected_python_type: Type[typing.Any]) -> str:
        transformer = cls.get_transformer(expected_python_type)
        if is_annotated(expected_python_type):
            expected_python_type, *annotate_args = get_args(expected_python_type)
            from nebulakit.deck.renderer import Renderable
    ...
    def named_tuple_to_variable_map(cls, t: typing.NamedTuple) -> _interface_models.VariableMap:
        """
        Converts a python-native ``NamedTuple`` to a nebula-specific VariableMap of named literals.
        """
        variables = {}
    ...
    def literal_map_to_kwargs(
        cls, ctx: NebulaContext, lm: LiteralMap, python_types: typing.Dict[str, type]
    ...
    def dict_to_literal_map(
        ...
        for k, v in d.items():
            ...
            try:
                ...
                raise user_exceptions.NebulaTypeException(type(v), python_type, received_value=v)
    ...
    def guess_python_types(
        cls, nebula_variable_dict: typing.Dict[str, _interface_models.Variable]
    ) -> typing.Dict[str, type]:
        """
        Transforms a dictionary of nebula-specific ``Variable`` objects to a dictionary of regular python values.
        """
        python_types = {}
        for k, v in nebula_variable_dict.items():
            python_types[k] = cls.guess_python_type(v.type)
        return python_types

    @classmethod
    def guess_python_type(cls, nebula_type: LiteralType) -> type:
        """
        Transforms a nebula-specific ``LiteralType`` to a regular python value.
        """
        for _, transformer in cls._REGISTRY.items():
            try:
                return transformer.guess_python_type(nebula_type)
            except ValueError:
                logger.debug(f"Skipping transformer {transformer.name} for {nebula_type}")

        # Because the dataclass transformer is handled explicitly in the get_transformer code, we have to handle it
        # separately here too.
        try:
            return cls._DATACLASS_TRANSFORMER.guess_python_type(literal_type=nebula_type)
        except ValueError:
            logger.debug(f"Skipping transformer {cls._DATACLASS_TRANSFORMER.name} for {nebula_type}")
        raise ValueError(f"No transformers could reverse Nebula literal type {nebula_type}")
...
class ListTransformer(TypeTransformer[T]):
    ...
    def get_literal_type(self, t: Type[T]) -> Optional[LiteralType]:
        """
        Only univariate Lists are supported in Nebula
        """
        try:
    ...
    def is_batchable(t: Type):
        ...
        It returns True only if the type is either List or Annotated(List) and the List subtype is NebulaPickle.
        """
        from nebulakit.types.pickle import NebulaPickle

        if is_annotated(t):
            return ListTransformer.is_batchable(get_args(t)[0])
        if get_origin(t) is list:
            subtype = get_args(t)[0]
            if subtype == NebulaPickle or (hasattr(subtype, "__origin__") and subtype.__origin__ == NebulaPickle):
                return True
        return False
    ...
    def to_literal(self, ctx: NebulaContext, python_val: T, python_type: Type[T], expected: LiteralType) -> Literal:
        ...
        if ListTransformer.is_batchable(python_type):
            from nebulakit.types.pickle.pickle import BatchSize, NebulaPickle
            ...
            if batch_size > 0:
                lit_list = [
                    TypeEngine.to_literal(ctx, python_val[i : i + batch_size], NebulaPickle, expected.collection_type)
    ...
    def to_python_value(self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[T]) -> typing.List[typing.Any]:  # type: ignore
        try:
            lits = lv.collection.literals
        except AttributeError:
            raise TypeTransformerFailedError(
                (
                    f"The expected python type is '{expected_python_type}' but the received Nebula literal value "
                    f"is not a collection (Nebula's representation of Python lists)."
                )
            )
        if self.is_batchable(expected_python_type):
            from nebulakit.types.pickle import NebulaPickle

            batch_list = [TypeEngine.to_python_value(ctx, batch, NebulaPickle) for batch in lits]
            if len(batch_list) > 0 and type(batch_list[0]) is list:
                # Make it have backward compatibility. The upstream task may use old version of Nebulakit that
...
def display_pickle_warning(python_type: str):
    ...
    logger.warning(
        f"Unsupported Type {python_type} found, Nebula will default to use PickleFile as the transport. "
        f"Pickle can only be used to send objects between the exact same version of Python, "
        f"and we strongly recommend to use python type that nebula support."
    )
...
def _type_essence(x: LiteralType) -> LiteralType:
    if x.metadata is not None or x.structure is not None or x.annotation is not None:
        x = LiteralType.from_nebula_idl(x.to_nebula_idl())
        x._metadata = None
        x._structure = None
        x._annotation = None
...
class UnionTransformer(TypeTransformer[T]):
    ...
    def to_python_value(self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[T]) -> Optional[typing.Any]:
        expected_python_type = get_underlying_type(expected_python_type)
...
class DictTransformer(TypeTransformer[dict]):
    ...
    def get_dict_types(t: Optional[Type[dict]]) -> typing.Tuple[Optional[type], Optional[type]]:
        ...
        if _origin is not None:
            if _origin is Annotated:
                raise ValueError(
                    f"Nebulakit does not currently have support \
                        for NebulaAnnotations applied to dicts. {t} cannot be \
                        parsed."
                )
    ...
    def dict_to_generic_literal(v: dict) -> Literal:
        """
        Creates a nebula-specific ``Literal`` value from a native python dictionary.
        """
    ...
    def get_literal_type(self, t: Type[dict]) -> LiteralType:
        """
        Transforms a native python dictionary to a nebula-specific ``LiteralType``
        """
    ...
    def to_literal(
        self, ctx: NebulaContext, python_val: typing.Any, python_type: Type[dict], expected: LiteralType
    ) -> Literal:
        ...
        for k, v in python_val.items():
            if type(k) != str:
                raise ValueError("Nebula MapType expects all keys to be strings")
    ...
    def to_python_value(self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[dict]) -> dict:
        if lv and lv.map and lv.map.literals is not None:
            tp = self.get_dict_types(expected_python_type)
            if tp is None or tp[0] is None:
                raise TypeError(
                    "TypeMismatch: Cannot convert to python dictionary from Nebula Literal Dictionary as the given "
                    "dictionary does not have sub-type hints or they do not match with the originating dictionary "
                    "source. Nebulakit does not currently support implicit conversions"
                )
            if tp[0] != str:
...
class TextIOTransformer(TypeTransformer[typing.TextIO]):
    ...
    def to_literal(
        self, ctx: NebulaContext, python_val: typing.TextIO, python_type: Type[typing.TextIO], expected: LiteralType
    ) -> Literal:
        raise NotImplementedError("Implement handle for TextIO")

    def to_python_value(
        self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[typing.TextIO]
    ) -> typing.TextIO:
...
class BinaryIOTransformer(TypeTransformer[typing.BinaryIO]):
    ...
    def to_literal(
        self, ctx: NebulaContext, python_val: typing.BinaryIO, python_type: Type[typing.BinaryIO], expected: LiteralType
    ) -> Literal:
        raise NotImplementedError("Implement handle for TextIO")

    def to_python_value(
        self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[typing.BinaryIO]
    ) -> typing.BinaryIO:
        local_path = ctx.file_access.get_random_local_path()
        ctx.file_access.get_data(lv.scalar.blob.uri, local_path, is_multipart=False)
...
def _register_default_type_transformers():
    ...
    # inner type is. Also unsupported are typing's Tuples. Even though you can look inside them, Nebula's type system
    ...
    # Also, it's not entirely true that Nebula IDL doesn't support tuples. We can always fake them as structs, but we'll
...
class LiteralsResolver(collections.UserDict):
    """
    LiteralsResolver is a helper class meant primarily for use with the NebulaRemote experience or any other situation
    ...
    def __init__(
        ...
        ctx: Optional[NebulaContext] = None,
    ):
        """
        :param literals: A Python map of strings to Nebula Literal models.
        :param variable_map: This map should be basically one side (either input or output) of the Nebula
    ...
    def get(self, attr: str, as_type: Optional[typing.Type] = None) -> typing.Any:  # type: ignore
        ...
        val = TypeEngine.to_python_value(
            self._ctx or NebulaContext.current_context(), self._literals[attr], cast(Type, as_type)
        )
