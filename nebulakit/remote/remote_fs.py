...
import fsspec
import requests
from nebulaidl.service.dataproxy_pb2 import CreateUploadLocationResponse
from fsspec.callbacks import NoOpCallback
from fsspec.implementations.http import HTTPFileSystem
from fsspec.utils import get_protocol

from nebulakit.loggers import logger
from nebulakit.tools.script_mode import hash_file
...
# at least not yet.
REMOTE_PLACEHOLDER = "nebula://data"

HashStructure = typing.Dict[str, typing.Tuple[bytes, int]]


class NebulaPathResolver:
    protocol = "nebula://"
    _nebula_path_to_remote_map: typing.Dict[str, str] = {}
    _lock = threading.Lock()

    @classmethod
    def resolve_remote_path(cls, nebula_uri: str) -> typing.Optional[str]:
        """
        Given a nebula uri, return the remote path if it exists or was created in current session, otherwise return None
        """
        with cls._lock:
            if nebula_uri in cls._nebula_path_to_remote_map:
                return cls._nebula_path_to_remote_map[nebula_uri]
            return None

    @classmethod
    def add_mapping(cls, nebula_uri: str, remote_path: str):
        """
        Thread safe method to dd a mapping from a nebula uri to a remote path
        """
        with cls._lock:
            cls._nebula_path_to_remote_map[nebula_uri] = remote_path


class HttpFileWriter(fsspec.spec.AbstractBufferedFile):
    def __init__(self, remote: NebulaRemote, filename: str, **kwargs):
    ...
    def _upload_chunk(self, final=False):
        ...
        try:
            ...
            NebulaPathResolver.add_mapping(self.path, res.native_url)
...
def get_nebula_fs(remote: NebulaRemote) -> typing.Type[NebulaFS]:
    class _NebulaFS(NebulaFS):
...
class NebulaFS(HTTPFileSystem):
    ...
    async def _get_file(self, rpath, lpath, **kwargs):
        """
        Don't do anything special. If it's a nebula url, the create a download link and write to lpath,
        otherwise default to parent.
        """
        raise NotImplementedError("NebulaFS currently doesn't support downloading files.")
    ...
    async def _put_file(
        ...
        **kwargs,
        ...
        NebulaPathResolver.add_mapping(rpath, resp.native_url)
    ...
    def extract_common(native_urls: typing.List[str]) -> str:
        ...
        this will return back 's3://my-s3-bucket/nebulasnacks/development/'
    ...
    async def _put(
        ...
        **kwargs,
        ...
        """
        cp file.txt nebula://data/...
        rpath gets ignored, so it doesn't matter what it is.
        """
        if rpath != REMOTE_PLACEHOLDER:
            logger.debug(f"NebulaFS doesn't yet support specifying full remote path, ignoring {rpath}")
    ...
    def exists(self, path, **kwargs):
        raise NotImplementedError("nebula file system currently can't check if a file exists.")
    ...
    def __str__(self):
        p = super().__str__()
        return f"NebulaFS({self._remote}): {p}"
