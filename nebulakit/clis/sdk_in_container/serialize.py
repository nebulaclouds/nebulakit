...
from nebulakit.clis.sdk_in_container import constants
from nebulakit.clis.sdk_in_container.constants import CTX_PACKAGES
from nebulakit.configuration import FastSerializationSettings, ImageConfig, SerializationSettings
from nebulakit.exceptions.scopes import system_entry_point
from nebulakit.interaction.click_types import key_value_callback
from nebulakit.tools.fast_registration import fast_package
from nebulakit.tools.repo import serialize_to_folder

CTX_IMAGE = "image"
CTX_LOCAL_SRC_ROOT = "local_source_root"
CTX_NEBULAKIT_VIRTUALENV_ROOT = "nebulakit_virtualenv_root"
...
def serialize_all(
    ...
    nebulakit_virtualenv_root: typing.Optional[str] = None,
    ...
    This function will write to the folder specified the following protobuf types ::
        nebulaidl.admin.launch_plan_pb2.LaunchPlan
        nebulaidl.admin.workflow_pb2.WorkflowSpec
        nebulaidl.admin.task_pb2.TaskSpec

    These can be inspected by calling (in the launch plan case) ::
        nebula-cli parse-proto -f filename.pb -p nebulaidl.admin.launch_plan_pb2.LaunchPlan

    See :py:class:`nebulakit.models.core.identifier.ResourceType` to match the trailing index in the file name with the
    entity type.
    ...
    :param nebulakit_virtualenv_root: The full path of the virtual env in the container.
    """
    ...
    serialization_settings = SerializationSettings(
        ...
        nebulakit_virtualenv_root=nebulakit_virtualenv_root,
...
@click.option(
    ...
    "Optional when running ``pynebula serialize`` in out-of-container-mode and your code lies outside of your working directory.",
)
@click.option(
    "--in-container-config-path",
    required=False,
    help="This is where the configuration for your task lives inside the container. "
    "The reason it needs to be a separate option is because this pynebula utility cannot know where the Dockerfile "
    "writes the config file to. Required for running ``pynebula serialize`` in out-of-container-mode",
)
@click.option(
    "--in-container-virtualenv-root",
    required=False,
    help="DEPRECATED: This flag is ignored! This is the root of the nebulakit virtual env in your container. "
    "The reason it needs to be a separate option is because this pynebula utility cannot know where nebulakit is "
    "installed inside your container. Required for running `pynebula serialize` in out of container mode when "
    "your container installs the nebulakit virtualenv outside of the default `/opt/venv`",
)
@click.option(
    "--env",
    "--envvars",
...
def serialize(
    ...
    ctx.obj[CTX_ENV] = env
    click.echo(f"Serializing Nebula elements with image {image_config}")

    if in_container_virtualenv_root:
        ctx.obj[CTX_NEBULAKIT_VIRTUALENV_ROOT] = in_container_virtualenv_root
        ...
        nebulakit_install_loc = os.path.abspath(nebulakit.__file__)
        ctx.obj[CTX_NEBULAKIT_VIRTUALENV_ROOT] = os.path.dirname(nebulakit_install_loc)
...
def workflows(ctx, folder=None):
    ...
    serialize_all(
        ...
        nebulakit_virtualenv_root=ctx.obj[CTX_NEBULAKIT_VIRTUALENV_ROOT],
