...
from nebulakit.tools.fast_registration import (
    FAST_FILEENDING,
    FAST_PREFIX,
    compute_digest,
    fast_package,
...
from nebulakit.tools.ignore import DockerIgnore, GitIgnore, IgnoreGroup, StandardIgnore
from tests.nebulakit.unit.tools.test_ignore import make_tree


@pytest.fixture
def nebula_project(tmp_path):
    tree = {
...
def test_package(nebula_project, tmp_path):
    archive_fname = fast_package(source=nebula_project, output_dir=tmp_path)
...
def test_package_with_symlink(nebula_project, tmp_path):
    archive_fname = fast_package(source=nebula_project / "src", output_dir=tmp_path, deref_symlinks=True)
...
def test_digest_ignore(nebula_project):
    ignore = IgnoreGroup(nebula_project, [GitIgnore, DockerIgnore, StandardIgnore])
    digest1 = compute_digest(nebula_project, ignore.is_ignored)

    change_file = nebula_project / "data" / "large.file"
    assert ignore.is_ignored(change_file)
    change_file.write_text("I don't matter")

    digest2 = compute_digest(nebula_project, ignore.is_ignored)
    assert digest1 == digest2


def test_digest_change(nebula_project):
    ignore = IgnoreGroup(nebula_project, [GitIgnore, DockerIgnore, StandardIgnore])
    digest1 = compute_digest(nebula_project, ignore.is_ignored)

    change_file = nebula_project / "src" / "workflows" / "hello_world.py"
