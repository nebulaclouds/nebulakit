...
def test_diabetes():
    ...
    nt = typing.NamedTuple("Outputs", model=NebulaFile[MODELSER_JOBLIB])

    @task(cache_version="1.0", cache=True, limits=Resources(mem="200Mi"))
    def fit(x: NebulaSchema[FEATURE_COLUMNS], y: NebulaSchema[CLASSES_COLUMNS], hyperparams: dict) -> nt:
        """
    ...
    @task(cache_version="1.0", cache=True, limits=Resources(mem="200Mi"))
    def predict(x: NebulaSchema[FEATURE_COLUMNS], model_ser: NebulaFile[MODELSER_JOBLIB]) -> NebulaSchema[CLASSES_COLUMNS]:
        """
    ...
    def diabetes_xgboost_model(
        dataset: NebulaFile[typing.TypeVar("csv")],
        # = "https://raw.githubusercontent.com/jbrownlee/Datasets/master/pima-indians-diabetes.data.csv",
        test_split_ratio: float = 0.33,
        seed: int = 7,
    ) -> typing.NamedTuple("Outputs", model=NebulaFile[MODELSER_JOBLIB], accuracy=float):
        """
