...
from nebulaidl.admin import description_entity_pb2

from nebulakit.models import common as _common_models


@dataclass
class Description(_common_models.NebulaIdlEntity):
    """
    ...
    @classmethod
    def from_nebula_idl(cls, pb2_object: description_entity_pb2.Description) -> "Description":
        return cls(
...
@dataclass
class SourceCode(_common_models.NebulaIdlEntity):
    """
    ...
    @classmethod
    def from_nebula_idl(cls, pb2_object: description_entity_pb2.SourceCode) -> "SourceCode":
        return cls(link=pb2_object.link) if pb2_object.link else None


@dataclass
class Documentation(_common_models.NebulaIdlEntity):
    """
    ...
    def to_nebula_idl(self):
        return description_entity_pb2.DescriptionEntity(
            ...
            long_description=self.long_description.to_nebula_idl() if self.long_description else None,
            source_code=self.source_code.to_nebula_idl() if self.source_code else None,
        )

    @classmethod
    def from_nebula_idl(cls, pb2_object: description_entity_pb2.DescriptionEntity) -> "Documentation":
        return cls(
            short_description=pb2_object.short_description,
            long_description=Description.from_nebula_idl(pb2_object.long_description)
            if pb2_object.long_description
            else None,
            source_code=SourceCode.from_nebula_idl(pb2_object.source_code) if pb2_object.source_code else None,
        )
