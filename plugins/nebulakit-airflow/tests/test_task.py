...
from nebulakitplugins.airflow.task import (
...
def test_xcom_push():
    ...
    with NebulaContextManager.with_context(ctx.with_execution_state(execution_state)) as child_ctx:
...
def test_airflow_container_task():
    ...
    assert t.task_resolver.loader_args(serialization_settings, t) == [
        "task-module",
        "nebulakitplugins.airflow.task",
        "task-name",
        "AirflowContainerTask",
        "task-config",
        '{"py/object": "nebulakitplugins.airflow.task.AirflowObj", "module": '
