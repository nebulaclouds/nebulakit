...
from nebulakit.core.base_task import kwtypes
...
from nebulakit.core.reference_entity import ReferenceEntity, ReferenceSpec, ReferenceTemplate, TaskReference
from nebulakit.core.task import reference_task, task
from nebulakit.core.testing import patch, task_mock
from nebulakit.core.workflow import reference_workflow, workflow
...
def test_ref():
    assert ref_t1.id.project == "nebulasnacks"
...
def test_ref_plain_two_outputs():
    ...
    with context_manager.NebulaContextManager.with_context(ctx.with_new_compilation_state()):
        xx, yy = r1(a="five", b=6)
...
def test_lps(resource_type):
    ...
    if resource_type == _identifier_model.ResourceType.LAUNCH_PLAN:
        ...
        assert wf_spec.template.nodes[0].workflow_node.launchplan_ref.name == "app.other.nebula_entity"
    else:
        assert wf_spec.template.nodes[0].task_node.reference_id.project == "proj"
        assert wf_spec.template.nodes[0].task_node.reference_id.name == "app.other.nebula_entity"
...
def test_lp_with_output():
    ...
    assert wf_spec.template.nodes[1].workflow_node.launchplan_ref.name == "app.other.nebula_entity"
