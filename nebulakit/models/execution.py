...
import nebulaidl
import nebulaidl.admin.cluster_assignment_pb2 as _cluster_assignment_pb2
import nebulaidl.admin.execution_pb2 as _execution_pb2
import nebulaidl.admin.node_execution_pb2 as _node_execution_pb2
import nebulaidl.admin.task_execution_pb2 as _task_execution_pb2

import nebulakit
from nebulakit.models import common as _common_models
from nebulakit.models import literals as _literals_models
from nebulakit.models import security
from nebulakit.models.core import execution as _core_execution
from nebulakit.models.core import identifier as _identifier
from nebulakit.models.node_execution import DynamicWorkflowNodeMetadata


class SystemMetadata(_common_models.NebulaIdlEntity):
    ...
    def to_nebula_idl(self) -> nebulaidl.admin.execution_pb2.SystemMetadata:
        return _execution_pb2.SystemMetadata(execution_cluster=self.execution_cluster)

    @classmethod
    def from_nebula_idl(cls, pb2_object: nebulaidl.admin.execution_pb2.SystemMetadata) -> SystemMetadata:
        return cls(
            execution_cluster=pb2_object.execution_cluster,
        )


class ExecutionMetadata(_common_models.NebulaIdlEntity):
    ...
    def to_nebula_idl(self):
        """
        :rtype: nebulaidl.admin.execution_pb2.ExecutionMetadata
        """
        p = _execution_pb2.ExecutionMetadata(
            mode=self.mode,
            principal=self.principal,
            nesting=self.nesting,
            parent_node_execution=self.parent_node_execution.to_nebula_idl()
            if self.parent_node_execution is not None
            else None,
            reference_execution=self.reference_execution.to_nebula_idl()
            if self.reference_execution is not None
            else None,
            system_metadata=self.system_metadata.to_nebula_idl() if self.system_metadata is not None else None,
        )
    ...
    def from_nebula_idl(cls, pb2_object):
        """
        :param nebulaidl.admin.execution_pb2.ExecutionMetadata pb2_object:
        ...
        return cls(
            ...
            parent_node_execution=_identifier.NodeExecutionIdentifier.from_nebula_idl(pb2_object.parent_node_execution)
            if pb2_object.HasField("parent_node_execution")
            else None,
            reference_execution=_identifier.WorkflowExecutionIdentifier.from_nebula_idl(pb2_object.reference_execution)
            if pb2_object.HasField("reference_execution")
            else None,
            system_metadata=SystemMetadata.from_nebula_idl(pb2_object.system_metadata)
            if pb2_object.HasField("system_metadata")
            else None,
        )


class ExecutionSpec(_common_models.NebulaIdlEntity):
    def __init__(
        self,
        launch_plan,
        metadata,
        ...
        """
        :param nebulakit.models.core.identifier.Identifier launch_plan: Launch plan unique identifier to execute
        ...
        :param nebulakit.models.common.Labels labels: Labels to apply to the execution.
        :param nebulakit.models.common.Annotations annotations: Annotations to apply to the execution
        :param nebulakit.models.common.AuthRole auth_role: The authorization method with which to execute the workflow.
        ...
        :param envs: nebulakit.models.common.Envs environment variables to set for this execution.
    ...
    def to_nebula_idl(self):
        """
        :rtype: nebulaidl.admin.execution_pb2.ExecutionSpec
        """
        return _execution_pb2.ExecutionSpec(
            launch_plan=self.launch_plan.to_nebula_idl(),
            metadata=self.metadata.to_nebula_idl(),
            notifications=self.notifications.to_nebula_idl() if self.notifications else None,
            disable_all=self.disable_all,  # type: ignore
            labels=self.labels.to_nebula_idl(),
            annotations=self.annotations.to_nebula_idl(),
            auth_role=self._auth_role.to_nebula_idl() if self.auth_role else None,
            raw_output_data_config=self._raw_output_data_config.to_nebula_idl()
            ...
            envs=self.envs.to_nebula_idl() if self.envs else None,
            tags=self.tags,
            cluster_assignment=self._cluster_assignment.to_nebula_idl() if self._cluster_assignment else None,
        )

    @classmethod
    def from_nebula_idl(cls, p):
        """
        :param nebulaidl.admin.execution_pb2.ExecutionSpec p:
        :return: ExecutionSpec
        """
        return cls(
            launch_plan=_identifier.Identifier.from_nebula_idl(p.launch_plan),
            metadata=ExecutionMetadata.from_nebula_idl(p.metadata),
            notifications=NotificationList.from_nebula_idl(p.notifications) if p.HasField("notifications") else None,
            disable_all=p.disable_all if p.HasField("disable_all") else None,
            labels=_common_models.Labels.from_nebula_idl(p.labels),
            annotations=_common_models.Annotations.from_nebula_idl(p.annotations),
            auth_role=_common_models.AuthRole.from_nebula_idl(p.auth_role),
            raw_output_data_config=_common_models.RawOutputDataConfig.from_nebula_idl(p.raw_output_data_config)
            if p.HasField("raw_output_data_config")
            else None,
            max_parallelism=p.max_parallelism,
            security_context=security.SecurityContext.from_nebula_idl(p.security_context)
            if p.security_context
            else None,
            overwrite_cache=p.overwrite_cache,
            envs=_common_models.Envs.from_nebula_idl(p.envs) if p.HasField("envs") else None,
            tags=p.tags,
            cluster_assignment=ClusterAssignment.from_nebula_idl(p.cluster_assignment)
            if p.HasField("cluster_assignment")
            else None,
        )


class ClusterAssignment(_common_models.NebulaIdlEntity):
    ...
    def to_nebula_idl(self):
        """
        :rtype: nebulaidl.admin._cluster_assignment_pb2.ClusterAssignment
        """
    ...
    def from_nebula_idl(cls, p):
        """
        :param nebulaidl.admin._cluster_assignment_pb2.ClusterAssignment p:
        :rtype: nebulaidl.admin.ClusterAssignment
        """
        return cls(cluster_pool=p.cluster_pool_name)


class LiteralMapBlob(_common_models.NebulaIdlEntity):
    def __init__(self, values=None, uri=None):
        """
        :param nebulakit.models.literals.LiteralMap values:
        :param Text uri:
        """
        self._values = values
        self._uri = uri
    ...
    def to_nebula_idl(self):
        """
        :rtype: nebulaidl.admin.execution_pb2.LiteralMapBlob
        """
        return _execution_pb2.LiteralMapBlob(
            values=self.values.to_nebula_idl() if self.values is not None else None,
            uri=self.uri,
        )

    @classmethod
    def from_nebula_idl(cls, pb):
        """
        :param nebulaidl.admin.execution_pb2.LiteralMapBlob pb:
        :rtype: LiteralMapBlob
        """
        values = None
        if pb.HasField("values"):
            values = LiteralMapBlob.from_nebula_idl(pb.values)
        return cls(values=values, uri=pb.uri if pb.HasField("uri") else None)


class Execution(_common_models.NebulaIdlEntity):
    ...
    def from_nebula_idl(cls, pb):
        """
        :param nebulaidl.admin.execution_pb2.Execution pb:
        :rtype: Execution
        """
        return cls(
            id=_identifier.WorkflowExecutionIdentifier.from_nebula_idl(pb.id),
            closure=ExecutionClosure.from_nebula_idl(pb.closure),
            spec=ExecutionSpec.from_nebula_idl(pb.spec),
        )


class AbortMetadata(_common_models.NebulaIdlEntity):
    ...
    def to_nebula_idl(self) -> nebulaidl.admin.execution_pb2.AbortMetadata:
        return _execution_pb2.AbortMetadata(cause=self.cause, principal=self.principal)

    @classmethod
    def from_nebula_idl(cls, pb2_object: nebulaidl.admin.execution_pb2.AbortMetadata) -> AbortMetadata:
        return cls(
...
class ExecutionClosure(_common_models.NebulaIdlEntity):
    def __init__(
        self,
        phase: int,
        started_at: datetime.datetime,
        duration: datetime.timedelta,
        error: typing.Optional[nebulakit.models.core.execution.ExecutionError] = None,
        ...
        """
        :param phase: From the nebulakit.models.core.execution.WorkflowExecutionPhase enum
        :param started_at:
    ...
    @property
    def error(self) -> nebulakit.models.core.execution.ExecutionError:
        return self._error

    @property
    def phase(self) -> int:
        """
        From the nebulakit.models.core.execution.WorkflowExecutionPhase enum
        """
        return self._phase
    ...
    def to_nebula_idl(self):
        """
        :rtype: nebulaidl.admin.execution_pb2.ExecutionClosure
        """
        obj = _execution_pb2.ExecutionClosure(
            phase=self.phase,
            error=self.error.to_nebula_idl() if self.error is not None else None,
            outputs=self.outputs.to_nebula_idl() if self.outputs is not None else None,
            abort_metadata=self.abort_metadata.to_nebula_idl() if self.abort_metadata is not None else None,
        )
    ...
    def from_nebula_idl(cls, pb2_object):
        """
        :param nebulaidl.admin.execution_pb2.ExecutionClosure pb2_object:
        :rtype: ExecutionClosure
        """
        error = None
        if pb2_object.HasField("error"):
            error = _core_execution.ExecutionError.from_nebula_idl(pb2_object.error)
        outputs = None
        if pb2_object.HasField("outputs"):
            outputs = LiteralMapBlob.from_nebula_idl(pb2_object.outputs)
        abort_metadata = None
        if pb2_object.HasField("abort_metadata"):
            abort_metadata = AbortMetadata.from_nebula_idl(pb2_object.abort_metadata)
        return cls(
            error=error,
            outputs=outputs,
...
class NotificationList(_common_models.NebulaIdlEntity):
    def __init__(self, notifications):
        """
        :param list[nebulakit.models.common.Notification] notifications: A simple list of notifications.
        """
        self._notifications = notifications
    ...
    def to_nebula_idl(self):
        """
        :rtype:  nebulaidl.admin.execution_pb2.NotificationList
        """
        return _execution_pb2.NotificationList(notifications=[n.to_nebula_idl() for n in self.notifications])

    @classmethod
    def from_nebula_idl(cls, pb2_object):
        """
        :param nebulaidl.admin.execution_pb2.NotificationList pb2_object:
        :rtype: NotificationList
        """
        return cls([_common_models.Notification.from_nebula_idl(p) for p in pb2_object.notifications])


class _CommonDataResponse(_common_models.NebulaIdlEntity):
    """
...
class WorkflowExecutionGetDataResponse(_CommonDataResponse):
    ...
    def from_nebula_idl(cls, pb2_object):
        ...
        """
        return cls(
            inputs=_common_models.UrlBlob.from_nebula_idl(pb2_object.inputs),
            outputs=_common_models.UrlBlob.from_nebula_idl(pb2_object.outputs),
            full_inputs=_literals_models.LiteralMap.from_nebula_idl(pb2_object.full_inputs),
            full_outputs=_literals_models.LiteralMap.from_nebula_idl(pb2_object.full_outputs),
        )
    ...
    def to_nebula_idl(self):
        ...
        return _execution_pb2.WorkflowExecutionGetDataResponse(
            inputs=self.inputs.to_nebula_idl(),
            outputs=self.outputs.to_nebula_idl(),
            full_inputs=self.full_inputs.to_nebula_idl(),
            full_outputs=self.full_outputs.to_nebula_idl(),
        )
...
class NodeExecutionGetDataResponse(_CommonDataResponse):
    ...
    def from_nebula_idl(cls, pb2_object):
        ...
        return cls(
            ...
            full_inputs=_literals_models.LiteralMap.from_nebula_idl(pb2_object.full_inputs),
            full_outputs=_literals_models.LiteralMap.from_nebula_idl(pb2_object.full_outputs),
            dynamic_workflow=DynamicWorkflowNodeMetadata.from_nebula_idl(pb2_object.dynamic_workflow)
    ...
    def to_nebula_idl(self):
        ...
        return _node_execution_pb2.NodeExecutionGetDataResponse(
            ...
            full_inputs=self.full_inputs.to_nebula_idl(),
            full_outputs=self.full_outputs.to_nebula_idl(),
            dynamic_workflow=self.dynamic_workflow.to_nebula_idl() if self.dynamic_workflow else None,
        )
