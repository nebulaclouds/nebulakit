...
class PKCEAuthenticator(Authenticator):
    ...
    caching. Otherwise, it will just receive the access token alone. Your NebulaCTL Helm config however should only
    ...
    def _initialize_auth_client(self):
        if not self._auth_client:
            ...
            self._auth_client = AuthorizationClient(
                ...
                # self._scopes refers to nebulakit.configuration.PlatformConfig (config.yaml)
...
class ClientCredentialsAuthenticator(Authenticator):
    ...
    def __init__(
        ...
        # Use scopes from `nebulakit.configuration.PlatformConfig` if passed
    ...
    def refresh_credentials(self):
        ...
        is meant for SDK use-cases of auth (like pynebula, or when users call SDK functions that require access to Admin,
...
class DeviceCodeAuthenticator(Authenticator):
    ...
    def __init__(
        ...
        # Input param: scopes refers to nebulakit.configuration.PlatformConfig (config.yaml)
        # cfg.scopes refers to PublicClientConfig scopes (can be defined in Helm deployments)
        ...
        if self._device_auth_endpoint is None:
            raise AuthenticationError(
                "Device Authentication is not available on the Nebula backend / authentication server"
            )
        self._session = session or requests.Session()
        super().__init__(
            endpoint=endpoint,
