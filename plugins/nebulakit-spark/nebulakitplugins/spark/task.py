...
from nebulakit import NebulaContextManager, PythonFunctionTask, logger
from nebulakit.configuration import DefaultImages, SerializationSettings
from nebulakit.core.context_manager import ExecutionParameters
from nebulakit.extend import ExecutionState, TaskPlugins
...
def new_spark_session(name: str, conf: Dict[str, str] = None):
    ...
    In cluster mode (running in hosted nebula, this will disregard the spark conf passed in)
    ...
    # We run in cluster-mode in Nebula.
    sess_builder = _pyspark.sql.SparkSession.builder.appName(f"NebulaSpark: {name}")
    if "NEBULA_INTERNAL_EXECUTION_ID" not in os.environ and conf is not None:
...
class PysparkFunctionTask(AsyncAgentExecutorMixin, PythonFunctionTask[Spark]):
        ...
        **kwargs,
        ...
        if isinstance(container_image, ImageSpec):
            if container_image.base_image is None:
                img = f"cr.nebula.org/nebulaclouds/nebulakit:spark-{DefaultImages.get_version_suffix()}"
    ...
    def get_custom(self, settings: SerializationSettings) -> Dict[str, Any]:
        ...
        return MessageToDict(job.to_nebula_idl())

    def pre_execute(self, user_params: ExecutionParameters) -> ExecutionParameters:
        import pyspark as _pyspark

        ctx = NebulaContextManager.current_context()
        sess_builder = _pyspark.sql.SparkSession.builder.appName(f"NebulaSpark: {user_params.execution_id}")
...
# Inject the Spark plugin into nebulakits dynamic plugin loading system
