...
from nebulakit.extras.tasks.shell import OutputLocation, RawShellTask, ShellTask, get_raw_shell_task
from nebulakit.types.directory import NebulaDirectory
from nebulakit.types.file import CSVFile, NebulaFile
...
def test_input_substitution_primitive():
    ...
    with pytest.raises(NebulaRecoverableException):
...
def test_input_substitution_files():
    t = ShellTask(
        ...
        script="""
            ...
            echo "Hello World {inputs.y} on  {inputs.j}"
            """,
        inputs=kwtypes(f=CSVFile, y=NebulaDirectory, j=datetime.datetime),
    )
...
def test_input_substitution_files_ctx():
    sec = nebulakit.current_context().secrets
...
def test_input_output_substitution_files():
    ...
    t = ShellTask(
        ...
        debug=True,
        script=script,
        inputs=kwtypes(f=NebulaFile),
        output_locs=[
            OutputLocation(var="y", var_type=NebulaFile, location="{inputs.f}.mod"),
        ],
    )
...
def test_input_single_output_substitution_files():
    ...
    t = ShellTask(
        ...
        script=script,
        inputs=kwtypes(f=CSVFile, y=NebulaDirectory, j=datetime.datetime),
        output_locs=[OutputLocation(var="z", var_type=NebulaFile, location="{inputs.f}.pyc")],
    )
...
def test_input_output_extra_and_missing_variables(script):
    ...
    t = ShellTask(
        ...
        inputs=kwtypes(f=CSVFile, y=NebulaDirectory, j=datetime.datetime),
        output_locs=[
            OutputLocation(var="x", var_type=NebulaDirectory, location="{inputs.y}"),
            OutputLocation(var="z", var_type=NebulaFile, location="{inputs.f}.pyc"),
        ],
    )
...
def test_reuse_variables_for_both_inputs_and_outputs():
    t = ShellTask(
        ...
        inputs=kwtypes(f=CSVFile, y=NebulaDirectory, j=datetime.datetime),
        output_locs=[
            OutputLocation(var="y", var_type=NebulaFile, location="{inputs.f}.pyc"),
        ],
    )
    if os.name == "nt":
        t._script = t._script.replace("cat", "type")
...
def test_can_use_complex_types_for_inputs_to_f_string_template():
    ...
    t = ShellTask(
        ...
        output_locs=[
            OutputLocation(var="x", var_type=NebulaFile, location="{inputs.input_args.in_file}.tmp"),
        ],
    )
    ...
    input_args = InputArgs(NebulaFile(path=test_csv))
