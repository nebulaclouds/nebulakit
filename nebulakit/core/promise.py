...
from nebulakit.core import context_manager as _nebula_context
from nebulakit.core import interface as nebula_interface
from nebulakit.core import type_engine
from nebulakit.core.context_manager import (
    BranchEvalMode,
    ExecutionParameters,
    ExecutionState,
    NebulaContext,
...
from nebulakit.core.type_engine import DictTransformer, ListTransformer, TypeEngine, TypeTransformerFailedError
from nebulakit.exceptions import user as _user_exceptions
from nebulakit.exceptions.user import NebulaPromiseAttributeResolveException
...
from nebulakit.models.literals import Primitive
from nebulakit.models.types import SimpleType


def translate_inputs_to_literals(
    ctx: NebulaContext,
    incoming_values: Dict[str, Any],
    nebula_interface_types: Dict[str, _interface_models.Variable],
    ...
    be converted into Nebula literals) or Promises (the literals in which would just get extracted).
    ...
    Here, in task_2, during execution we'd have a list of Promises. We have to make sure to give task2 a Nebula
    LiteralCollection (Nebula's name for list), not a Python list of Nebula literals.

    This helper function is used both when sorting out inputs to a task, as well as outputs of a function.

    :param ctx: Context needed in case a non-primitive literal needs to be translated to a Nebula literal (like a file)
    :param incoming_values: This is a map of your task's input or wf's output kwargs basically
    :param nebula_interface_types: One side of an :py:class:`nebulakit.models.interface.TypedInterface` basically.
    ...
    for k, v in incoming_values.items():
        if k not in nebula_interface_types:
            raise ValueError(f"Received unexpected keyword argument {k}")
        var = nebula_interface_types[k]
        t = native_types[k]
        try:
...
def resolve_attr_path_in_promise(p: Promise) -> Promise:
    ...
    This is for local execution only. The remote execution will be resolved in nebulapropeller.
    """
    ...
    for attr in p.attr_path:
        # If current value is Nebula literal collection (list) or map (dictionary), use [] to resolve
        if (
            ...
            if type(attr) == str and attr not in curr_val.value.literals:
                raise NebulaPromiseAttributeResolveException(
    ...
    if type(curr_val.value) is _literals_models.Scalar and type(curr_val.value.value) is _struct.Struct:
        ...
        # Reconstruct the resolved result to nebula literal (because the resolved result might not be struct)
        curr_val = TypeEngine.to_literal(NebulaContextManager.current_context(), new_st, type(new_st), literal_type)
...
class ComparisonExpression(object):
    ...
    def __init__(self, lhs: Union["Promise", Any], op: ComparisonOps, rhs: Union["Promise", Any]):
        ...
        if self._lhs is None:
            self._lhs = type_engine.TypeEngine.to_literal(NebulaContextManager.current_context(), lhs, type(lhs), None)
        if self._rhs is None:
            self._rhs = type_engine.TypeEngine.to_literal(NebulaContextManager.current_context(), rhs, type(rhs), None)
...
class Promise(object):
    ...
    def val(self) -> _literals_models.Literal:
        """
        If the promise is ready then this holds the actual evaluate value in Nebula's type system
        """
        return self._val
    ...
    def __bool__(self):
        raise ValueError(
            "Nebulakit does not support Unary expressions or performing truth value testing,"
            " This is a limitation in python. For Logical `and\\or` use `&\\|` (bitwise) instead"
        )
...
def create_native_named_tuple(
    ...
    if isinstance(promises, Promise):
        ...
        # only show the name of output key if it's user-defined (by default Nebula names these as "o<n>")
...
def create_task_output(
    ...
    class Output(collections.namedtuple(named_tuple_name, variables)):  # type: ignore
        ...
        def runs_before(self, other: Any):
            ...
            nebulakit.core.node.Node for more information. Local workflow execution in the manual ``create_node``
...
def binding_data_from_python_std(
    ctx: _nebula_context.NebulaContext,
...
class SupportsNodeCreation(Protocol):
    ...
    @property
    def python_interface(self) -> nebula_interface.Interface:
        ...
...
class HasNebulaInterface(Protocol):
    @property
    def name(self) -> str:
        ...
...
def create_and_link_node_from_remote(
    ...
    **kwargs,
    ...
    """
    This method is used to generate a node with bindings especially when using remote entities, like NebulaWorkflow,
    NebulaTask and NebulaLaunchplan.
    ...
    if ctx.compilation_state is None:
        raise _user_exceptions.NebulaAssertion("Cannot create node when not compiling...")
    ...
    for k in sorted(typed_interface.inputs):
        ...
        if k not in kwargs:
            ...
            raise _user_exceptions.NebulaAssertion("Missing input `{}` type `{}`".format(k, var.type))
        v = kwargs[k]
        # This check ensures that tuples are not passed into a function, as tuples are not supported by Nebula
    ...
    nebulakit_node = Node(
        ...
        nebula_entity=entity,
    )
    ctx.compilation_state.add_node(nebulakit_node)

    if len(typed_interface.outputs) == 0:
        return VoidPromise(entity.name, NodeOutput(node=nebulakit_node, var="placeholder"))
    ...
    for output_name, output_var_model in typed_interface.outputs.items():
        node_outputs.append(Promise(output_name, NodeOutput(node=nebulakit_node, var=output_name)))
...
def create_and_link_node(
    ...
    **kwargs,
    ...
    """
    This method is used to generate a node with bindings within a nebulakit workflow. this is useful to traverse the
    ...
    typed_interface = nebula_interface.transform_interface_to_typed_interface(interface)
            ...
            if var.type.union_type:
                for variant in var.type.union_type.variants:
                    if variant.simple == SimpleType.NONE:
                        ...
                        is_optional = True
            if not is_optional:
                from nebulakit.core.base_task import Task
                ...
                if isinstance(entity, Task) and _default is not None:
                    error_msg += (
                        ". Nebula workflow syntax is a domain-specific language (DSL) for building execution graphs which "
                        "supports a subset of Python’s semantics. When calling tasks, all kwargs have to be provided."
                    )

                raise _user_exceptions.NebulaAssertion(error_msg)
            else:
                continue
        v = kwargs[k]
        # This check ensures that tuples are not passed into a function, as tuples are not supported by Nebula
...
class LocallyExecutable(Protocol):
    def local_execute(self, ctx: NebulaContext, **kwargs) -> Union[Tuple[Promise], Promise, VoidPromise, None]:
        ...
...
def nebula_entity_call_handler(
    ...
    #. Workflow Execution Mode - when a workflow is being run locally. Even though workflows are functions
       ...
       nothing. Subsequent tasks will have to know how to unwrap these. If by chance a non-Nebula task uses a
