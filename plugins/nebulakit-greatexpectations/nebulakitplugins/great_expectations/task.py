...
from nebulakit import PythonInstanceTask
from nebulakit.core.context_manager import NebulaContext
from nebulakit.extend import Interface
...
class GreatExpectationsTask(PythonInstanceTask[BatchRequestConfig]):
    ...
    def _nebula_file(self, dataset: NebulaFile) -> str:
    ...
    def _nebula_schema(self, dataset: NebulaSchema) -> str:
        if not self._local_file_path:
            raise ValueError("local_file_path is missing!")
    ...
    def execute(self, **kwargs) -> Any:
        ...
        if not issubclass(datatype, (NebulaFile, NebulaSchema, str)):
            raise TypeError("'dataset' has to have NebulaFile/NebulaSchema/str datatype")
        ...
        # NebulaFile
        if issubclass(datatype, NebulaFile):
            dataset = self._nebula_file(dataset)

        # NebulaSchema
        # convert schema to parquet file
        if issubclass(datatype, NebulaSchema) and not is_runtime:
            dataset = self._nebula_schema(dataset)
        ...
        if self._batch_request_config and (self._batch_request_config.runtime_parameters or is_runtime):
            ...
            elif is_runtime and issubclass(datatype, NebulaSchema):
