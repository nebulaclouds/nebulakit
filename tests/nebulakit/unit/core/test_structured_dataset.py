...
from nebulakit.models.types import SchemaType, SimpleType, StructuredDatasetType
...
def test_register_renderers():
    ...
    assert renderers[str].to_html("nebula") == "hello nebula"
    ...
    with pytest.raises(NotImplementedError, match="Could not find a renderer for <class 'int'> in"):
        StructuredDatasetTransformerEngine().to_html(NebulaContextManager.current_context(), 3, int)
