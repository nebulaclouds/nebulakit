"""
This Plugin adds the capability of running distributed pytorch training to Nebula using backend plugins, natively on
...
import cloudpickle
from nebulaidl.plugins.kubeflow import common_pb2 as kubeflow_common
from nebulaidl.plugins.kubeflow import pytorch_pb2 as pytorch_task
from google.protobuf.json_format import MessageToDict

import nebulakit
from nebulakit import PythonFunctionTask, Resources
from nebulakit.configuration import SerializationSettings
from nebulakit.core.resources import convert_resources_to_resource_model
from nebulakit.exceptions.user import NebulaRecoverableException
from nebulakit.extend import IgnoreOutputs, TaskPlugins
from nebulakit.loggers import logger

from .error_handling import create_recoverable_error_file, is_recoverable_worker_error

TORCH_IMPORT_ERROR_MESSAGE = "PyTorch is not installed. Please install `nebulakitplugins-kfpytorch['elastic']`."
...
class PyTorchFunctionTask(PythonFunctionTask[PyTorch]):
    ...
    def _convert_replica_spec(
        ...
        return pytorch_task.DistributedPyTorchTrainingReplicaSpec(
            ...
            resources=resources.to_nebula_idl() if resources else None,
...
# Register the Pytorch Plugin into the nebulakit core plugin system
...
class ElasticWorkerResult(NamedTuple):
    ...
    Attributes:
        ...
        decks (list[nebulakit.Deck]): A list of nebulakit Deck objects created in the worker process.
    """

    return_value: Any
    decks: List[nebulakit.Deck]
...
def spawn_helper(
    ...
    Returns:
        ElasticWorkerResult: A named tuple containing the return value of the task function and a list of
            nebulakit Deck objects created in the worker process.
    """
    from nebulakit.bin.entrypoint import setup_execution
    ...
    with setup_execution(
        ...
        try:
            ...
            if isinstance(e, NebulaRecoverableException):
                create_recoverable_error_file()
            raise
        return ElasticWorkerResult(return_value=return_val, decks=nebulakit.current_context().decks)
...
class PytorchElasticFunctionTask(PythonFunctionTask[Elastic]):
    ...
    def _execute(self, **kwargs) -> Any:
        ...
        Raises:
            NebulaRecoverableException: If the first exception raised in the local worker group is or
                inherits from `NebulaRecoverableException`.
            RuntimeError: If the first exception raised in the local worker group is not and does not
                inherit from `NebulaRecoverableException`.
        ...
        config = LaunchConfig(
            run_id=nebulakit.current_context().execution_id.name,
        ...
        if self.task_config.start_method == "spawn":
            ...
            only args. Nebula only works with kwargs. Thus, we create a closure which already has
        ...
        try:
            ...
            if is_recoverable_worker_error(first_failure):
                raise NebulaRecoverableException(e.format_msg())
            else:
        ...
        if 0 in out:
            ...
            for deck in out[0].decks:
                if not isinstance(deck, nebulakit.deck.deck.TimeLineDeck):
    ...
    def get_custom(self, settings: SerializationSettings) -> Optional[Dict[str, Any]]:
        if self.task_config.nnodes == 1:
            ...
            from nebulaidl.plugins.kubeflow.pytorch_pb2 import ElasticConfig
...
# Register the PytorchElastic Plugin into the nebulakit core plugin system
