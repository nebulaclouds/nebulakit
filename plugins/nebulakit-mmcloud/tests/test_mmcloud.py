...
import grpc
import pytest
from nebulaidl.admin.agent_pb2 import PERMANENT_FAILURE, RUNNING, SUCCEEDED
from nebulakitplugins.mmcloud import MMCloudAgent, MMCloudConfig, MMCloudTask
from nebulakitplugins.mmcloud.utils import async_check_output, nebula_to_mmcloud_resources

from nebulakit import Resources, task
from nebulakit.configuration import DefaultImages, ImageConfig, SerializationSettings
...
def test_nebula_to_mmcloud_resources():
    B_IN_GIB = 1073741824
    success_cases = {
        ("0", "0", "0", "0"): (1, 1, 1, 1),
    ...
    for (req_cpu, req_mem, lim_cpu, lim_mem), (min_cpu, min_mem, max_cpu, max_mem) in success_cases.items():
        resources = nebula_to_mmcloud_resources(
