...
import nebulakit
from nebulakit import Deck, NebulaContextManager, task
    ...
    if expect_error:
        with pytest.raises(ValueError):
            ...
            def t_df(a: str) -> int:
                ...
                nebulakit.current_context().default_deck.append(TopFrameRenderer().to_html(df))
                return int(a)
...
@mock.patch("nebulakit.deck.deck.ipython_check")
...
def test_get_deck():
    ...
    ctx = NebulaContextManager.current_context()
    ctx.user_space_params._decks = [ctx.user_space_params.default_deck]
    ctx.user_space_params._decks[0] = nebulakit.Deck("test", html)
    _output_deck("test_task", ctx.user_space_params)
