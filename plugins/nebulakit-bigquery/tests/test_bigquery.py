...
from nebulakit import StructuredDataset, kwtypes, workflow
...
def test_serialization():
    bigquery_task = BigQueryTask(
        name="nebulakit.demo.bigquery_task.query",
        inputs=kwtypes(ds=str),
        task_config=BigQueryConfig(
            ProjectID="Nebula", Location="Asia", QueryJobConfig=QueryJobConfig(allow_large_results=True)
        ),
    ...
    s = Struct()
    s.update({"ProjectID": "Nebula", "Location": "Asia", "allowLargeResults": True})
...
def test_local_exec():
    bigquery_task = BigQueryTask(
        name="nebulakit.demo.bigquery_task.query2",
        inputs=kwtypes(ds=str),
        query_template=query_template,
        task_config=BigQueryConfig(ProjectID="Nebula", Location="Asia"),
        output_structured_dataset_type=StructuredDataset,
    )
