...
from nebulakit.core.type_engine import TypeEngine, TypeTransformer, get_batch_size
...
from nebulakit.types.file import FileExt, NebulaFile
...
@dataclass
class NebulaDirectory(DataClassJsonMixin, os.PathLike, typing.Generic[T]):
    ...
    Please first read through the comments on the :py:class:`nebulakit.types.file.NebulaFile` class as the
    implementation here is similar.

    One thing to note is that the ``os.PathLike`` type that comes with Python was used as a stand-in for ``NebulaFile``.
    That is, if a task's output signature is an ``os.PathLike``, Nebula takes that to mean ``NebulaFile``. There is no
    easy way to distinguish an ``os.PathLike`` where the user means a File and where the user means a Directory. As
    such, if you want to use a directory, you must declare all types as ``NebulaDirectory``. You'll still be able to
    return a string literal though instead of a full-fledged ``NebulaDirectory`` object assuming the str is a directory.

    **Converting from a Nebula literal value to a Python instance of NebulaDirectory**

    +-----------------------------+------------------------------------------------------------------------------------+
    | Type of Nebula IDL Literal   |    NebulaDirectory                                                                  |
    +=============+===============+====================================================================================+
    | Multipart   | uri matches   | NebulaDirectory object stores the original string                                   |
    ...
    |             |               |                                                                                    |
    |             +---------------+------------------------------------------------------------------------------------+
    |             | uri matches   | NebulaDirectory object just wraps the string                                        |
    |             | /local/path   |                                                                                    |
    ...
    **Converting from a Python value (NebulaDirectory, str, or pathlib.Path) to a Nebula literal**

    +-----------------------------------+------------------------------------------------------------------------------+
    | Type of Python value              | NebulaDirectory                                                               |
    +===================+===============+==============================================================================+
    ...
    | NebulaDirectory    +---------------+------------------------------------------------------------------------------+
    |                   | path matches  | Contents of file are uploaded to the Nebula blob store (S3, GCS, etc.), in    |
    ...
    As inputs ::
        ...
        def t1(in1: NebulaDirectory["svg"]):
            ...

    As outputs:

    The contents of this local directory will be uploaded to the Nebula store. ::

        return NebulaDirectory("/path/to/dir/")

        return NebulaDirectory["svg"]("/path/to/dir/", remote_path="s3://special/output/location")

    Similar to the NebulaFile example, if you give an already remote location, it will not be copied to Nebula's
    durable store, the uri will just be stored as is. ::

        return NebulaDirectory("s3://some/other/folder")
    ...
    The format [] bit is still there because in Nebula, directories are stored as Blob Types also, just like files, and
    ...
    def __init__(
        ...
        :param remote_directory: If the user wants to return something and also specify where it should be uploaded to.
            If set to False, then nebulakit will not upload the directory to the remote store.
        """
    ...
    @classmethod
    def new_remote(cls) -> NebulaDirectory:
        """
        Create a new NebulaDirectory object using the currently configured default remote in the context (i.e.
        ...
        If you want to write a whole folder, you can let your task return a NebulaDirectory object,
        and let nebulakit handle the uploading.
        """
        ...
        return NebulaDirectory(path=d)

    def __class_getitem__(cls, item: typing.Union[typing.Type, str]) -> typing.Type[NebulaDirectory]:
        if item is None:
            return cls
        ...
        class _SpecificFormatDirectoryClass(NebulaDirectory):
            # Get the type engine to see this as kind of a generic
            __origin__ = NebulaDirectory
    ...
    def remote_source(self) -> str:
        """
        If this is an input to a task, and the original path is s3://something, nebulakit will download the
    ...
    def new_file(self, name: typing.Optional[str] = None) -> NebulaFile:
        """
        ...
        return NebulaFile(path=new_path)

    def new_dir(self, name: typing.Optional[str] = None) -> NebulaDirectory:
        """
        ...
        new_path = self.sep.join([str(self.path).rstrip(self.sep), name])  # trim trailing sep if any and join
        return NebulaDirectory(path=new_path)
    ...
    def crawl(
        ...
        """
        Crawl returns a generator of all files prefixed by any sub-folders under the given "NebulaDirectory".
...
class NebulaDirToMultipartBlobTransformer(TypeTransformer[NebulaDirectory]):
    """
    This transformer handles conversion between the Python native NebulaDirectory class defined above, and the Nebula
    IDL literal/type of Multipart Blob. Please see the NebulaDirectory comments for additional information.
    ...
    def __init__(self):
        super().__init__(name="NebulaDirectory", t=NebulaDirectory)

    @staticmethod
    def get_format(t: typing.Type[NebulaDirectory]) -> str:
        return t.extension()
    ...
    def assert_type(self, t: typing.Type[NebulaDirectory], v: typing.Union[NebulaDirectory, os.PathLike, str]):
        if isinstance(v, NebulaDirectory) or isinstance(v, str) or isinstance(v, os.PathLike):
            """
            NOTE: we do not do a isdir check because the given path could be remote reference
            """
            return
        raise TypeError(
            f"No automatic conversion from {type(v)} declared type {t} to NebulaDirectory found."
            f" Use (NebulaDirectory, str, os.PathLike)"
        )

    def get_literal_type(self, t: typing.Type[NebulaDirectory]) -> LiteralType:
        return _type_models.LiteralType(blob=self._blob_type(format=NebulaDirToMultipartBlobTransformer.get_format(t)))

    def to_literal(
        self,
        ctx: NebulaContext,
        python_val: NebulaDirectory,
        python_type: typing.Type[NebulaDirectory],
        ...
        # There are two kinds of literals we handle, either an actual NebulaDirectory, or a string path to a directory.
        # Handle the NebulaDirectory case
        if isinstance(python_val, NebulaDirectory):
            ...
            # - If the path given is already a remote path, say https://www.google.com, uploading the Nebula
        ...
        # Handle the string case
            ...
            raise AssertionError(f"Expected NebulaDirectory or os.PathLike object, received {type(python_val)}")
    ...
    def to_python_value(
        self, ctx: NebulaContext, lv: Literal, expected_python_type: typing.Type[NebulaDirectory]
    ) -> NebulaDirectory:
        ...
        # For the remote case, return an NebulaDirectory object that can download
        ...
        fd = NebulaDirectory.__class_getitem__(expected_format)(local_folder, _downloader)
        fd._remote_source = uri
        return fd

    def guess_python_type(self, literal_type: LiteralType) -> typing.Type[NebulaDirectory[typing.Any]]:
        if (
            literal_type.blob is not None
            and literal_type.blob.dimensionality == _core_types.BlobType.BlobDimensionality.MULTIPART
        ):
            return NebulaDirectory.__class_getitem__(literal_type.blob.format)
        raise ValueError(f"Transformer {self} cannot reverse {literal_type}")


TypeEngine.register(NebulaDirToMultipartBlobTransformer())
