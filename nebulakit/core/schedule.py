"""
.. autoclass:: nebulakit.core.schedule.CronSchedule
   :noindex:
...
from nebulakit.models import schedule as _schedule_models


# Duplicates nebulakit.common.schedules.Schedule to avoid using the ExtendedSdkType metaclass.
class CronSchedule(_schedule_models.Schedule):
    ...
    This uses standard `cron format <https://docs.nebula.org/en/latest/concepts/schedules.html#cron-expression-table>`__
    ...
    def __init__(
        ...
        :param str schedule: This takes a cron alias (see ``_VALID_CRON_ALIASES``) or a croniter parseable schedule.
          Only one of this or ``cron_expression`` can be set, not both. This uses standard `cron format <https://docs.nebula.org/en/latest/concepts/schedules.html#cron-expression-table>`_
        ...
        :param str kickoff_time_input_arg: This is a convenient argument to use when your code needs to know what time
          ...
          that until Nebula has an atomic clock, there could be a few seconds here and there. That is, if your run is
...
class FixedRate(_schedule_models.Schedule):
    ...
    def _translate_duration(duration: datetime.timedelta):
        ...
        :rtype: nebulakit.models.schedule.Schedule.FixedRate
        """
