...
import grpc
from nebulaidl.admin.agent_pb2 import CreateTaskResponse, DeleteTaskResponse, GetTaskResponse, Resource
from nebulakitplugins.mmcloud.utils import async_check_output, mmcloud_status_to_nebula_state

from nebulakit import current_context
...
class MMCloudAgent(AgentBase):
    ...
    async def async_create(
        ...
        """
        Submit Nebula task as MMCloud job to the OpCenter, and return the job UID for the task.
        """
        submit_command = [
            "float",
            "submit",
        ...
        # We do not use container.resources because NebulaPropeller will impose limits that should not apply to MMCloud
        ...
        try:
            ...
            with NamedTemporaryFile(mode="w") as job_file:
                ...
                logger.info(f"Attempting to submit Nebula task {task_id} as MMCloud job")
                ...
                try:
                    ...
                    logger.exception(
                        f"Failed to submit Nebula task {task_id} as MMCloud job\n"
                        f"[stdout] {e.stdout.decode()}\n"
                        f"[stderr] {e.stderr.decode()}\n"
                    )
                    raise
                except (UnicodeError, json.JSONDecodeError):
                    logger.exception(f"Failed to decode submit response for Nebula task: {task_id}")
                    raise
                except KeyError:
                    logger.exception(f"Failed to obtain MMCloud job id for Nebula task: {task_id}")
                    raise

                logger.info(f"Submitted Nebula task {task_id} as MMCloud job {job_id}")
    ...
    async def async_get(self, context: grpc.ServicerContext, resource_meta: bytes) -> GetTaskResponse:
        ...
        task_state = mmcloud_status_to_nebula_state(job_status)
