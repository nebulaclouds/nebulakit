...
from nebulakit.clis.sdk_in_container.constants import CTX_CONFIG_FILE
from nebulakit.configuration import Config, ImageConfig, get_config_file
from nebulakit.loggers import cli_logger
from nebulakit.remote.remote import NebulaRemote

NEBULA_REMOTE_INSTANCE_KEY = "nebula_remote"
...
def get_and_save_remote_with_click_context(
    ...
    NB: This function will by default mutate the click Context.obj dictionary, adding a remote key with value
        of the created NebulaRemote object.
    ...
    :return: NebulaRemote instance
    """
    if ctx.obj.get(NEBULA_REMOTE_INSTANCE_KEY) is not None:
        return ctx.obj[NEBULA_REMOTE_INSTANCE_KEY]
    cfg_file_location = ctx.obj.get(CTX_CONFIG_FILE)
    r = get_remote(cfg_file_location, project, domain, data_upload_location)
    if save:
        ctx.obj[NEBULA_REMOTE_INSTANCE_KEY] = r
    return r
...
def patch_image_config(config_file: Optional[str], image_config: ImageConfig) -> ImageConfig:
    ...
    # * The default nebulakit images, which are already supplied by the base run_level_params.
    ...
    #   override all).
    #
    # However, the run_level_params already contains both the default nebulakit images (lowest priority), as well
