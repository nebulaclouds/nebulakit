# Nebulakit FlyIn Plugin

> Flyin = Nebula Interactive = Develop Nebula with the speed of flying

FlyIn plugin provides users' favorite interface to develop and debug a nebula task interactively. We support vscode, jupyter (WIP), and neovim (WIP).
...
```bash
pip install nebulakitplugins-flyin
```
...
## User Guide
1. Build the image with Dockerfile.dev `docker build --push . -f Dockerfile.dev -t localhost:30000/nebulakit:dev --build-arg PYTHON_VERSION=3.8`
2. Run the decorated task on the remote. For example: `pynebula run --remote --image localhost:30000/nebulakit:dev [PYTHONFILE] [WORKFLOW|TASK] [ARGS]...`
...
from nebulakitplugins.flyin import vscode, VscodeConfig, DEFAULT_CODE_SERVER_EXTENSIONS

@task(
    container_image="localhost:30000/nebulakit-vscode:0.0.2",
    environment={"NEBULA_SDK_LOGGING_LEVEL": "20"}
)
@vscode(
