...
import grpc
import requests
from nebulaidl.service.auth_pb2 import OAuth2MetadataRequest, PublicClientAuthConfigRequest
from nebulaidl.service.auth_pb2_grpc import AuthMetadataServiceStub

from nebulakit.clients.auth.authenticator import (
    Authenticator,
    ClientConfig,
    ClientConfigStore,
...
from nebulakit.clients.grpc_utils.auth_interceptor import AuthUnaryInterceptor
from nebulakit.clients.grpc_utils.default_metadata_interceptor import DefaultMetadataInterceptor
from nebulakit.clients.grpc_utils.wrap_exception_interceptor import RetryExceptionWrapperInterceptor
from nebulakit.configuration import AuthType, PlatformConfig


class RemoteClientConfigStore(ClientConfigStore):
    """
    This class implements the ClientConfigStore that is served by the Nebula Server, that implements AuthMetadataService
    """
...
def upgrade_channel_to_proxy_authenticated(cfg: PlatformConfig, in_channel: grpc.Channel) -> grpc.Channel:
    ...
    channel that uses Interceptor to perform authentication with a proxy infront of Nebula
...
def wrap_exceptions_channel(cfg: PlatformConfig, in_channel: grpc.Channel) -> grpc.Channel:
    ...
    exceptions and raise Exception from the Family nebulakit.exceptions

    .. note:: This channel should be usually the outermost channel. This channel will raise a NebulaException
...
def upgrade_session_to_proxy_authenticated(cfg: PlatformConfig, session: requests.Session) -> requests.Session:
    ...
    perform authentication with a proxy infront of Nebula
