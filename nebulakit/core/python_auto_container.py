...
from nebulakit.core.base_task import PythonTask, TaskMetadata, TaskResolverMixin
from nebulakit.core.context_manager import NebulaContextManager
from nebulakit.core.pod_template import PodTemplate
from nebulakit.core.resources import Resources, ResourceSpec
from nebulakit.core.tracked_abc import NebulaTrackedABC
from nebulakit.core.tracker import TrackedInstance, extract_task_module
from nebulakit.core.utils import _get_container_definition, _serialize_pod_spec, timeit
from nebulakit.extras.accelerators import BaseAccelerator
from nebulakit.image_spec.image_spec import ImageBuildEngine, ImageSpec
from nebulakit.loggers import logger
from nebulakit.models import task as _task_model
from nebulakit.models.security import Secret, SecurityContext

T = TypeVar("T")
_PRIMARY_CONTAINER_NAME_FIELD = "primary_container_name"


class PythonAutoContainerTask(PythonTask[T], ABC, metaclass=NebulaTrackedABC):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if secret_requests:
            for s in secret_requests:
                if not isinstance(s, Secret):
                    raise AssertionError(f"Secret {s} should be of type nebulakit.Secret, received {type(s)}")
        ...
        compilation_state = NebulaContextManager.current_context().compilation_state
    ...
    def get_default_command(self, settings: SerializationSettings) -> List[str]:
        """
        Returns the default pynebula-execute command used to run this on hosted Nebula platforms.
        """
        container_args = [
            "pynebula-execute",
            "--inputs",
    ...
    def set_command_fn(self, get_command_fn: Optional[Callable[[SerializationSettings], List[str]]] = None):
        """
        By default, the task will run on the Nebula platform using the pynebula-execute command.
        However, it can be useful to update the command with which the task is serialized for specific cases like
        running map tasks ("pynebula-map-execute") or for fast-executed tasks.
        """
    ...
    def get_command(self, settings: SerializationSettings) -> List[str]:
        ...
        registered on a hosted Nebula platform.
        """
    ...
    def get_extended_resources(self, settings: SerializationSettings) -> Optional[tasks_pb2.ExtendedResources]:
        ...
        return tasks_pb2.ExtendedResources(gpu_accelerator=self.accelerator.to_nebula_idl())
