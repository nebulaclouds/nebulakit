...
class TensorFlowRecordFileTransformer(TypeTransformer[TFRecordFile]):
    ...
    def to_literal(
        self, ctx: NebulaContext, python_val: TFRecordFile, python_type: Type[TFRecordFile], expected: LiteralType
    ) -> Literal:
        meta = BlobMetadata(
    ...
    def to_python_value(
        self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[TFRecordFile]
    ) -> TFRecordDatasetV2:
...
class TensorFlowRecordsDirTransformer(TypeTransformer[TFRecordsDirectory]):
    ...
    def to_python_value(
        self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[TFRecordsDirectory]
    ) -> TFRecordDatasetV2:
        uri, metadata = extract_metadata_and_uri(lv, expected_python_type)
        local_dir = ctx.file_access.get_random_local_directory()
