...
import nebulakit
from nebulakit import NebulaContextManager
from nebulakit.bin.entrypoint import get_one_of
from nebulakit.deck.renderer import TopFrameRenderer
...
def mlflow_autolog(fn=None, *, framework=mlflow.sklearn, experiment_name: typing.Optional[str] = None):
    ...
    One benefit of doing so is that the mlflow metrics are then rendered inline using NebulaDecks and can be viewed
    in jupyter notebook, as well as in hosted Nebula environment:
    ...
    When the task is called in a Nebula backend, the decorator starts a new MLFlow run using the Nebula execution name
    ...
    :param experiment_name: The MLFlow experiment name. If not provided, uses the Nebula execution name.
    """
    ...
    def wrapper(*args, **kwargs):
        ...
        if not ctx.execution_state.is_local_execution():
            experiment = f"{get_one_of('NEBULA_INTERNAL_EXECUTION_WORKFLOW', '_F_WF')}" or experiment_name
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            if run is not None:
                ...
                if figure:
                    nebulakit.Deck("mlflow metrics", figure.to_html())
                params = get_run_params(client, run_id)
                if params is not None:
                    nebulakit.Deck("mlflow params", TopFrameRenderer(max_rows=10).to_html(params))
        return out
