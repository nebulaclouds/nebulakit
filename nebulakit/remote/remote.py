"""
This module provides the ``NebulaRemote`` object, which is the end-user's main starting point for interacting
with a Nebula backend in an interactive and programmatic way. This of this experience as kind of like the web UI
...
import fsspec
import requests
from nebulaidl.admin.signal_pb2 import Signal, SignalListRequest, SignalSetRequest
from nebulaidl.core import literals_pb2 as literals_pb2

from nebulakit.clients.friendly import SynchronousNebulaClient
from nebulakit.clients.helpers import iterate_node_executions, iterate_task_executions
from nebulakit.configuration import Config, FastSerializationSettings, ImageConfig, SerializationSettings
from nebulakit.core import constants, utils
from nebulakit.core.base_task import PythonTask
...
from nebulakit.core.python_auto_container import PythonAutoContainerTask
from nebulakit.core.reference_entity import ReferenceSpec
from nebulakit.core.type_engine import LiteralsResolver, TypeEngine
from nebulakit.core.workflow import WorkflowBase, WorkflowFailurePolicy
...
from nebulakit.exceptions.user import (
    ...
    NebulaValueException,
)
from nebulakit.loggers import remote_logger
from nebulakit.models import common as common_models
from nebulakit.models import filters as filter_models
from nebulakit.models import launch_plan as launch_plan_models
from nebulakit.models import literals as literal_models
from nebulakit.models import task as task_models
from nebulakit.models import types as type_models
from nebulakit.models.admin import common as admin_common_models
from nebulakit.models.admin import workflow as admin_workflow_models
from nebulakit.models.admin.common import Sort
from nebulakit.models.core import workflow as workflow_model
from nebulakit.models.core.identifier import Identifier, ResourceType, SignalIdentifier, WorkflowExecutionIdentifier
from nebulakit.models.core.workflow import NodeMetadata
from nebulakit.models.execution import (
    ClusterAssignment,
    ExecutionMetadata,
    ExecutionSpec,
...
from nebulakit.models.launch_plan import LaunchPlanState
from nebulakit.models.literals import Literal, LiteralMap
from nebulakit.remote.backfill import create_backfill_workflow
from nebulakit.remote.data import download_literal
from nebulakit.remote.entities import NebulaLaunchPlan, NebulaNode, NebulaTask, NebulaTaskNode, NebulaWorkflow
...
from nebulakit.remote.remote_fs import get_nebula_fs
from nebulakit.tools.fast_registration import fast_package
from nebulakit.tools.interactive import ipython_check
from nebulakit.tools.script_mode import compress_scripts, hash_file
from nebulakit.tools.translator import (
    NebulaControlPlaneEntity,
    NebulaLocalEntity,
    Options,
    get_serializable,
...
def _get_latest_version(list_entities_method: typing.Callable, project: str, domain: str, name: str):
    ...
    if not admin_entity:
        raise user_exceptions.NebulaEntityNotExistException("Named entity {} not found".format(named_entity))
    return admin_entity.id.version


def _get_entity_identifier(
    list_entities_method: typing.Callable,
    resource_type: int,  # from nebulakit.models.core.identifier.ResourceType
    project: str,
    domain: str,
    name: str,
...
def _get_git_repo_url(source_path):
    ...
    try:
        ...
        if url.startswith("git@"):
            # url format: git@github.com:nebulakit/nebulakit.git
            prefix_len, suffix_len = len("git@"), len(".git")
            return url[prefix_len:-suffix_len].replace(":", "/")
        elif url.startswith("https://"):
            # url format: https://github.com/nebulakit/nebulakit
            prefix_len = len("https://")
            return url[prefix_len:]
        elif url.startswith("http://"):
            # url format: http://github.com/nebulakit/nebulakit
            prefix_len = len("http://")
            return url[prefix_len:]
        else:
...
class NebulaRemote(object):
    """Main entrypoint for programmatically accessing a Nebula remote backend.

    The term 'remote' is synonymous with 'backend' or 'deployment' and refers to a hosted instance of the
    Nebula platform, which comes with a Nebula Admin server on some known URI.
    """
    ...
    def __init__(
        ...
        data_upload_location: str = "nebula://my-s3-bucket/",
        **kwargs,
    ):
        """Initialize a NebulaRemote object.

        :type kwargs: All arguments that can be passed to create the SynchronousNebulaClient. These are usually grpc
            parameters, if you want to customize credentials, ssl handling etc.
        :param default_project: default project to use when fetching or executing nebula entities.
        :param default_domain: default domain to use when fetching or executing nebula entities.
        ...
        if config is None or config.platform is None or config.platform.endpoint is None:
            raise user_exceptions.NebulaAssertion("Nebula endpoint should be provided.")

        if data_upload_location is None:
            data_upload_location = NebulaContext.current_context().file_access.raw_output_prefix
        self._kwargs = kwargs
        ...
        fsspec.register_implementation("nebula", get_nebula_fs(remote=self), clobber=True)
        ...
        self._ctx = NebulaContextManager.current_context().with_file_access(self._file_access).build()

    @property
    def context(self) -> NebulaContext:
        return self._ctx

    @property
    def client(self) -> SynchronousNebulaClient:
        """Return a SynchronousNebulaClient for additional operations."""
        if not self._client_initialized:
            self._client = SynchronousNebulaClient(self.config.platform, **self._kwargs)
            self._client_initialized = True
        return self._client

    @property
    def default_project(self) -> str:
        """Default project to use when fetching or executing nebula entities."""
    ...
    def get(
        self, nebula_uri: typing.Optional[str] = None
    ) -> typing.Optional[typing.Union[LiteralsResolver, Literal, HTML, bytes]]:
        """
        General function that works with nebula tiny urls. This can return outputs (in the form of LiteralsResolver, or
        ...
        """
        if nebula_uri is None:
            raise user_exceptions.NebulaUserException("nebula_uri cannot be empty")
        ctx = self._ctx or NebulaContextManager.current_context()
        try:
            data_response = self.client.get_data(nebula_uri)

            if data_response.HasField("literal_map"):
                lm = LiteralMap.from_nebula_idl(data_response.literal_map)
                return LiteralsResolver(lm.literals)
            elif data_response.HasField("literal"):
                return Literal.from_nebula_idl(data_response.literal)
            elif data_response.HasField("pre_signed_urls"):
                if len(data_response.pre_signed_urls.signed_url) == 0:
                    raise ValueError(f"Nebula url {nebula_uri} resolved to empty download link")
                ...
                if ipython_check():
                    ...
                    remote_logger.debug(f"IPython found, returning HTML from {nebula_uri}")
                    with fs.open(d, "rb") as r:
                        html = HTML(str(r.read()))
                        return html
                # If not return bytes
                else:
                    remote_logger.debug(f"IPython not found, returning HTML as bytes from {nebula_uri}")
                    return fs.open(d, "rb").read()

        except user_exceptions.NebulaUserException as e:
            remote_logger.info(f"Error from Nebula backend when trying to fetch data: {e.__cause__}")

        remote_logger.info(f"Nothing found from {nebula_uri}")

    def remote_context(self):
        """Context manager with remote-specific configuration."""
        return NebulaContextManager.with_context(
            NebulaContextManager.current_context().with_file_access(self.file_access)
        )
    ...
    def fetch_task_lazy(
        ...
        """
        if name is None:
            raise user_exceptions.NebulaAssertion("the 'name' argument must be specified.")
    ...
    def fetch_task(self, project: str = None, domain: str = None, name: str = None, version: str = None) -> NebulaTask:
        """Fetch a task entity from nebula admin.
        ...
        :returns: :class:`~nebulakit.remote.tasks.task.NebulaTask`

        :raises: NebulaAssertion if name is None
        """
        if name is None:
            raise user_exceptions.NebulaAssertion("the 'name' argument must be specified.")
        ...
        nebula_task = NebulaTask.promote_from_model(admin_task.closure.compiled_task.template)
        nebula_task.template._id = task_id
        return nebula_task

    def fetch_workflow_lazy(
        self, project: str = None, domain: str = None, name: str = None, version: str = None
    ) -> LazyEntity[NebulaWorkflow]:
        """
    ...
    def fetch_workflow(
        ...
        """
        Fetch a workflow entity from nebula admin.
        :param project: fetch entity from this project. If None, uses the default_project attribute.
        :param domain: fetch entity from this domain. If None, uses the default_domain attribute.
        :param name: fetch entity with matching name.
        :param version: fetch entity with matching version. If None, gets the latest version of the entity.
        ...
        for wf_template in wf_templates:
            for node in NebulaWorkflow.get_non_system_nodes(wf_template.nodes):
        ...
        return NebulaWorkflow.promote_from_closure(compiled_wf, node_launch_plans)

    def fetch_launch_plan(
        self, project: str = None, domain: str = None, name: str = None, version: str = None
    ) -> NebulaLaunchPlan:
        """Fetch a launchplan entity from nebula admin.

        :param project: fetch entity from this project. If None, uses the default_project attribute.
        :param domain: fetch entity from this domain. If None, uses the default_domain attribute.
        :param name: fetch entity with matching name.
        :param version: fetch entity with matching version. If None, gets the latest version of the entity.
        :returns: :class:`~nebulakit.remote.launch_plan.NebulaLaunchPlan`
        ...
        nebula_launch_plan = NebulaLaunchPlan.promote_from_model(launch_plan_id, admin_launch_plan.spec)

        wf_id = nebula_launch_plan.workflow_id
        workflow = self.fetch_workflow(wf_id.project, wf_id.domain, wf_id.name, wf_id.version)
        nebula_launch_plan._interface = workflow.interface
        nebula_launch_plan._nebula_workflow = workflow

        return nebula_launch_plan

    def fetch_execution(self, project: str = None, domain: str = None, name: str = None) -> NebulaWorkflowExecution:
        """Fetch a workflow execution entity from nebula admin.

        :param project: fetch entity from this project. If None, uses the default_project attribute.
        :param domain: fetch entity from this domain. If None, uses the default_domain attribute.
        :param name: fetch entity with matching name.
        :returns: :class:`~nebulakit.remote.workflow_execution.NebulaWorkflowExecution`

        :raises: NebulaAssertion if name is None
        """
        if name is None:
            raise user_exceptions.NebulaAssertion("the 'name' argument must be specified.")
        execution = NebulaWorkflowExecution.promote_from_model(
    ...
    def list_signals(
        ...
        req = SignalListRequest(workflow_execution_id=wf_exec_id.to_nebula_idl(), limit=limit, filters=filters)
    ...
    def set_signal(
        ...
        :param value: This is either a Literal or a Python value which NebulaRemote will invoke the TypeEngine to
        ...
        :param literal_type: Provide a Nebula literal type to help with conversion if the value you provided
            is not a Literal
        """
        wf_exec_id = WorkflowExecutionIdentifier(
            project=project or self.default_project, domain=domain or self.default_domain, name=execution_name
        ...
        req = SignalSetRequest(id=SignalIdentifier(signal_id, wf_exec_id).to_nebula_idl(), value=lit.to_nebula_idl())
    ...
    def recent_executions(
    ...
    ) -> typing.List[NebulaWorkflowExecution]:
        ...
        return [NebulaWorkflowExecution.promote_from_model(e) for e in exec_models]
    ...
    def list_tasks_by_version(
        ...
        return [NebulaTask.promote_from_model(t.closure.compiled_task.template) for t in t_models]
    ...
    def raw_register(
        self,
        cp_entity: NebulaControlPlaneEntity,
        settings: SerializationSettings,
        version: str,
        create_default_launchplan: bool = True,
        options: Options = None,
        og_entity: NebulaLocalEntity = None,
    ) -> typing.Optional[Identifier]:
        """
        Raw register method, can be used to register control plane entities. Usually if you have a Nebula Entity like a
        WorkflowBase, Task, LaunchPlan then use other methods. This should be used only if you have already serialized entities

        :param cp_entity: The controlplane "serializable" version of a nebula entity. This is in the form that NebulaAdmin
            understands.
        ...
        :param og_entity: Pass in the original workflow (nebulakit type) if create_default_launchplan is true
        :return: Identifier of the created entity
        """
        if isinstance(cp_entity, RemoteEntity):
            if isinstance(cp_entity, (NebulaWorkflow, NebulaTask)):
        ...
        if isinstance(cp_entity, task_models.TaskSpec):
            if isinstance(cp_entity, NebulaTask):
        ...
        if isinstance(cp_entity, admin_workflow_models.WorkflowSpec):
            ...
            if create_default_launchplan:
                if not og_entity:
                    raise user_exceptions.NebulaValueException(
                        "To create default launch plan, please pass in the original nebulakit workflow `og_entity`"
                    )
    ...
    def _serialize_and_register(
        ...
        """
        This method serializes and register the given Nebula entity
    ...
    def fast_package(self, root: os.PathLike, deref_symlinks: bool = True, output: str = None) -> (bytes, str):
        ...
        # Upload zip file to Admin using NebulaRemote.
    ...
    def _version_from_hash(
        ...
        *additional_context: str,
        ...
        but we don't have to use it when registering with the Nebula backend.
        ...
        :return:
        """
        from nebulakit import __version__
    ...
    ) -> typing.Union[NebulaWorkflow, NebulaTask]:
        """
    ...
    def register_launch_plan(
        ...
        :param version:
        :param project: Optionally provide a project, if not already provided in nebularemote constructor or a separate one
        :param domain: Optionally provide a domain, if not already provided in NebulaRemote constructor or a separate one
        :param options:
        :return:
        """
    ...
    def _execute(
        self,
        entity: typing.Union[NebulaTask, NebulaWorkflow, NebulaLaunchPlan],
    ...
    ) -> NebulaWorkflowExecution:
        """Common method for execution across all entities.

        :param nebula_id: entity identifier
        :param inputs: dictionary mapping argument names to values
        :param project: project on which to execute the entity referenced by nebula_id
        :param domain: domain on which to execute the entity referenced by nebula_id
        ...
        with self.remote_context() as ctx:
            input_nebula_type_map = entity.interface.inputs

            for k, v in inputs.items():
                if input_nebula_type_map.get(k) is None:
                    raise user_exceptions.NebulaValueException(
                ...
                if isinstance(v, Literal):
                    ...
                    if k not in type_hints:
                        try:
                            type_hints[k] = TypeEngine.guess_python_type(input_nebula_type_map[k].type)
                        except ValueError:
                            remote_logger.debug(f"Could not guess type for {input_nebula_type_map[k].type}, skipping...")
        ...
        try:
            # Currently, this will only execute the nebula entity referenced by
            # nebula_id in the same project and domain. However, it is possible to execute it in a different project
            # and domain, which is specified in the first two arguments of client.create_execution. This is useful
            # in the case that I want to use a nebula entity from e.g. project "A" but actually execute the entity on a
        ...
        except user_exceptions.NebulaEntityAlreadyExistsException:
        ...
        execution = NebulaWorkflowExecution.promote_from_model(self.client.get_execution(exec_id))
    ...
    def _resolve_identifier_kwargs(
        ...
        auto-generated version, and ultimately the entity project/domain if entity is a remote nebula entity.
        """
        ...
        if not (ident.project and ident.domain and ident.name):
            raise ValueError(
                ...
                f" Specify them in the execute method or when intializing NebulaRemote"
            )
        return ident

    def execute(
        self,
        entity: typing.Union[NebulaTask, NebulaLaunchPlan, NebulaWorkflow, PythonTask, WorkflowBase, LaunchPlan],
        inputs: typing.Dict[str, typing.Any],
        project: str = None,
        domain: str = None,
        name: str = None,
        ...
        - ``Nebula{Task, Workflow, LaunchPlan}`` remote module objects.
        ...
        :param type_hints: Python types to be passed to the TypeEngine so that it knows how to properly convert the
          input values for the execution into Nebula literals. If missing, will default to first guessing the type
          using the type engine, and then to ``type(v)``. Providing the correct Python types is particularly important
          if the inputs are containers like lists or maps, or if the Python type is one of the more complex Nebula
        ...
        .. note:
            ...
            The ``name`` and ``version`` arguments do not apply to ``NebulaTask``, ``NebulaLaunchPlan``, and
            ``NebulaWorkflow`` entity inputs. These values are determined by referencing the entity identifier values.
        """
        if entity.python_interface:
            type_hints = type_hints or entity.python_interface.inputs
        if isinstance(entity, NebulaTask) or isinstance(entity, NebulaLaunchPlan):
        ...
        if isinstance(entity, NebulaWorkflow):
    ...
    # Nebula Remote Entities
    # ---------------------

    def execute_remote_task_lp(
        self,
        entity: typing.Union[NebulaTask, NebulaLaunchPlan],
        inputs: typing.Dict[str, typing.Any],
        project: str = None,
        domain: str = None,
        execution_name: typing.Optional[str] = None,
        ...
        """Execute a NebulaTask, or NebulaLaunchplan.
    ...
    def execute_local_task(
        ...
        :param tags: Tags to set for the execution.
        :param cluster_pool: Specify cluster pool on which newly created execution should be placed.
        :return: NebulaWorkflowExecution object.
        """
        resolved_identifiers = self._resolve_identifier_kwargs(entity, project, domain, name, version)
        resolved_identifiers_dict = asdict(resolved_identifiers)
        try:
            nebula_task: NebulaTask = self.fetch_task(**resolved_identifiers_dict)
        except NebulaEntityNotExistException:
            ...
            nebula_task: NebulaTask = self.register_task(entity, ss)
    ...
    def execute_local_workflow(
        ...
        try:
            ...
            remote_logger.info("Registering workflow because it wasn't found in Nebula Admin.")
            if not image_config:
        ...
        try:
            nebula_lp = self.fetch_launch_plan(**resolved_identifiers_dict)
        except NebulaEntityNotExistException:
            remote_logger.info("Try to register default launch plan because it wasn't found in Nebula Admin!")
    ...
    def execute_local_launch_plan(
        ...
        domain = resolved_identifiers.domain
        try:
            nebula_launchplan: NebulaLaunchPlan = self.fetch_launch_plan(**resolved_identifiers_dict)
        except NebulaEntityNotExistException:
            nebula_launchplan: NebulaLaunchPlan = self.register_launch_plan(
                entity,
                version=version,
                project=project,
                domain=domain,
    ...
    def wait(
        self,
        execution: NebulaWorkflowExecution,
        ...
        raise user_exceptions.NebulaTimeout(f"Execution {self} did not complete before timeout.")
    ...
    def sync(
        self,
        execution: NebulaWorkflowExecution,
        entity_definition: typing.Union[NebulaWorkflow, NebulaTask] = None,
        ...
        """
        if not isinstance(execution, NebulaWorkflowExecution):
    ...
    def sync_execution(
        ...
        """
        Sync a NebulaWorkflowExecution object with its corresponding remote state.
        """
        ...
        if sync_nodes:
            underlying_node_executions = [
                NebulaNodeExecution.promote_from_model(n) for n in iterate_node_executions(self.client, execution.id)
            ]

        # This condition is only true for single-task executions
        if execution.spec.launch_plan.resource_type == ResourceType.TASK:
            nebula_entity = self.fetch_task(lp_id.project, lp_id.domain, lp_id.name, lp_id.version)
            node_interface = nebula_entity.interface
            if sync_nodes:
                ...
                node_mapping = (
                    {
                        task_node_exec[0].id.node_id: NebulaNode(
                            id=nebula_entity.id,
                            upstream_nodes=[],
                            bindings=[],
                            metadata=NodeMetadata(name=""),
                            task_node=NebulaTaskNode(nebula_entity),
                        )
                    }
        ...
        # This is the default case, an execution of a normal workflow through a launch plan
            ...
            node_interface = fetched_lp.nebula_workflow.interface
            execution._nebula_workflow = fetched_lp.nebula_workflow
            node_mapping = fetched_lp.nebula_workflow._node_map
    ...
    def sync_node_execution(
        self,
        execution: NebulaNodeExecution,
        node_mapping: typing.Dict[str, NebulaNode],
    ) -> NebulaNodeExecution:
        """
        Get data backing a node execution. These NebulaNodeExecution objects should've come from Admin with the model
        ...
        if not execution.metadata.is_parent_node and execution.closure.workflow_node_metadata:
            ...
            execution._interface = launched_exec._nebula_workflow.interface
            return execution
        ...
        if execution.metadata.is_parent_node:
            ...
            if node_execution_get_data_response.dynamic_workflow is not None:
                ...
                node_launch_plans = {}
                # TODO: Inspect branch nodes for launch plans
                for node in NebulaWorkflow.get_non_system_nodes(compiled_wf.primary.template.nodes):
                    if (
                ...
                dynamic_nebula_wf = NebulaWorkflow.promote_from_closure(compiled_wf, node_launch_plans)
                execution._underlying_node_executions = [
                    self.sync_node_execution(NebulaNodeExecution.promote_from_model(cne), dynamic_nebula_wf._node_map)
                ...
                execution._interface = dynamic_nebula_wf.interface

            # Handle the case where it's a static subworkflow
            elif isinstance(execution._node.nebula_entity, NebulaWorkflow):
                sub_nebula_workflow = execution._node.nebula_entity
                sub_node_mapping = {n.id: n for n in sub_nebula_workflow.nebula_nodes}
                execution._underlying_node_executions = [
                    self.sync_node_execution(NebulaNodeExecution.promote_from_model(cne), sub_node_mapping)
                    for cne in child_node_executions
                ]
                execution._interface = sub_nebula_workflow.interface
        ...
        # This is the plain ol' task execution case
            ...
            execution._task_executions = [
                self.sync_task_execution(
                    NebulaTaskExecution.promote_from_model(t), node_mapping[node_id].task_node.nebula_task
                )
                for t in iterate_task_executions(self.client, execution.id)
            ]
            execution._interface = execution._node.nebula_entity.interface
    ...
    def sync_task_execution(
        self, execution: NebulaTaskExecution, entity_definition: typing.Optional[NebulaTask] = None
    ) -> NebulaTaskExecution:
        """Sync a NebulaTaskExecution object with its corresponding remote state."""
    ...
    def terminate(self, execution: NebulaWorkflowExecution, cause: str):
    ...
    def _assign_inputs_and_outputs(
        self,
        execution: typing.Union[NebulaWorkflowExecution, NebulaNodeExecution, NebulaTaskExecution],
        execution_data,
    ...
    def _get_input_literal_map(self, execution_data: ExecutionDataResponse) -> literal_models.LiteralMap:
        ...
        if bool(execution_data.full_inputs.literals):
            ...
            with self.remote_context() as ctx:
                ...
                return literal_models.LiteralMap.from_nebula_idl(
    ...
    def generate_console_url(
        self,
        entity: typing.Union[
            NebulaWorkflowExecution, NebulaNodeExecution, NebulaTaskExecution, NebulaWorkflow, NebulaTask, NebulaLaunchPlan
        ],
    ):
        """
        Generate a Nebulaconsole URL for the given Nebula remote endpoint.
        This will automatically determine if this is an execution or an entity and change the type automatically
        """
        if isinstance(entity, (NebulaWorkflowExecution, NebulaNodeExecution, NebulaTaskExecution)):
        ...
        if not isinstance(entity, (NebulaWorkflow, NebulaTask, NebulaLaunchPlan)):
    ...
    def launch_backfill(
    ...
    ) -> typing.Optional[NebulaWorkflowExecution, NebulaWorkflow, WorkflowBase]:
        """
        ...
        :param launchplan: str launchplan name in the nebula backend
        ...
        :return: In case of dry-run, return WorkflowBase, else if no_execute return NebulaWorkflow else in the default
            case return a NebulaWorkflowExecution
        """
    ...
    def download(
        ...
        specified, then all file like objects will be recursively downloaded (e.g. NebulaFile/Dir (blob),
