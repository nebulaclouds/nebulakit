...
import nebulakit
from nebulakit import StructuredDataset, kwtypes, map_task, task, workflow
...
from nebulakit.types.file import NebulaFile, PythonNotebook
...
def test_nebula_types():
    @task
    def create_file() -> NebulaFile:
        tmp_file = tempfile.mktemp()
        with open(tmp_file, "w") as f:
            f.write("abc")
        return NebulaFile(path=tmp_file)

    @task
    def create_dir() -> NebulaDirectory:
        tmp_dir = tempfile.mkdtemp()
        with open(os.path.join(tmp_dir, "file.txt"), "w") as f:
            f.write("abc")
        return NebulaDirectory(path=tmp_dir)
    ...
    nb_types = NotebookTask(
        name="test",
        notebook_path=_get_nb_path(nb_name, abs=False),
        inputs=kwtypes(ff=NebulaFile, fd=NebulaDirectory, sd=StructuredDataset),
