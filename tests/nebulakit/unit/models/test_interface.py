...
from nebulakit.models import interface, types
from tests.nebulakit.common.parameterizers import LIST_OF_ALL_LITERAL_TYPES
...
def test_variable_type(literal_type):
    ...
    assert var == interface.Variable.from_nebula_idl(var.to_nebula_idl())
...
def test_typed_interface(literal_type):
    ...
    pb = typed_interface.to_nebula_idl()
    deserialized_typed_interface = interface.TypedInterface.from_nebula_idl(pb)
...
def test_parameter():
    ...
    obj2 = interface.Parameter.from_nebula_idl(obj.to_nebula_idl())
    assert obj == obj2
    assert obj2.var == v
...
def test_parameter_map():
    ...
    obj2 = interface.ParameterMap.from_nebula_idl(obj.to_nebula_idl())
    assert obj == obj2
...
def test_variable_map():
    ...
    obj2 = interface.VariableMap.from_nebula_idl(obj.to_nebula_idl())
    assert obj == obj2
