"""
Logic for serializing a basemodel to a literalmap that can be passed between nebula tasks.

The serialization process is as follows:

1. Serialize the basemodel to json, replacing all nebula types with unique placeholder strings
2. Serialize the nebula types to separate literals and store them in the nebula object store (a singleton object)
3. Return a literal map with the json and the nebula object store represented as a literalmap {placeholder: nebula type}
...
OBJECTS_KEY = "Serialized Nebula Objects"

SerializedBaseModel = Annotated[str, "A pydantic BaseModel that has been serialized with placeholders for Nebula types."]
...
class BaseModelNebulaObjectStore:
    """
    This class is an intermediate store for python objects that are being serialized/deserialized.

    On serialization of a basemodel, nebula objects are serialized and stored in this object store.
    """
    ...
    def register_python_object(self, python_object: object) -> LiteralObjID:
        ...
        serialized_item = serialize_to_nebula_literal(python_object)
...
def serialize_basemodel(basemodel: pydantic.BaseModel) -> literals.Literal:
    ...
    The BaseModel is first serialized into a JSON format, where all Nebula types are replaced with unique placeholder strings.
    The Nebula Types are serialized into separate Nebula literals
    """
    store = BaseModelNebulaObjectStore()
    basemodel_literal = serialize_basemodel_to_literal(basemodel, store)
    basemodel_literalmap = literals.LiteralMap(
        {
            BASEMODEL_JSON_KEY: basemodel_literal,  # json with nebula types replaced with placeholders
            OBJECTS_KEY: store.to_literal(),  # nebula type-engine serialized types
        }
    )
...
def serialize_basemodel_to_literal(
    ...
    nebulaobject_store: BaseModelNebulaObjectStore,
) -> literals.Literal:
    """
    Serialize a pydantic BaseModel to json and protobuf, separating out the Nebula types into a separate store.
    On deserialization, the store is used to reconstruct the Nebula types.
    """

    def encoder(obj: Any) -> Union[str, commons.LiteralObjID]:
        if isinstance(obj, commons.PYDANTIC_SUPPORTED_NEBULA_TYPES):
            return nebulaobject_store.register_python_object(obj)
...
def serialize_to_nebula_literal(python_obj: object) -> literals.Literal:
    """
    Use the Nebula TypeEngine to serialize a python object to a Nebula Literal.
    """
...
def make_literal_from_json(json: str) -> literals.Literal:
    """
    Converts the json representation of a pydantic BaseModel to a Nebula Literal.
    """
