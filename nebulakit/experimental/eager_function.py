...
from nebulakit import Deck, Secret, current_context
from nebulakit.configuration import DataConfig, PlatformConfig, S3Config
from nebulakit.core.base_task import PythonTask
from nebulakit.core.context_manager import ExecutionState, NebulaContext, NebulaContextManager
from nebulakit.core.python_function_task import PythonFunctionTask
from nebulakit.core.task import task
from nebulakit.core.workflow import WorkflowBase
...
NEBULA_SANDBOX_INTERNAL_ENDPOINT = "nebula-sandbox-grpc.nebula:8089"
NEBULA_SANDBOX_MINIO_ENDPOINT = "http://nebula-sandbox-minio.nebula:9000"

NODE_HTML_TEMPLATE = """
<style>
    #nebula-frame-container > div.active {{font-family: Open sans;}}
</style>

<style>
    #nebula-frame-container div.input-output {{
...
class EagerException(Exception):
    ...
    This exception should be used in an :py:func:`@eager <nebulakit.experimental.eager>` workflow function to
    catch exceptions that are raised by tasks or subworkflows.

    .. code-block:: python

        from nebulakit import task
        from nebulakit.experimental import eager, EagerException
...
class AsyncEntity:
    """A wrapper around a Nebula entity (task, workflow, launch plan) that allows it to be executed asynchronously."""

    def __init__(
        self,
        entity,
        remote: Optional[NebulaRemote],
        ctx: NebulaContext,
    ...
    async def __call__(self, **kwargs):
        ...
        msg = f"Running nebula {type(self.entity)} {entity_name} on remote cluster: {url}"
...
class AsyncNode:
    ...
    @property
    def url(self) -> str:
        # make sure that internal nebula sandbox endpoint is replaced with localhost endpoint when rendering the urls
        # for nebula decks
        endpoint_root = NEBULA_SANDBOX_INTERNAL_ENDPOINT.replace("http://", "")
...
def eager(
    _fn=None,
    *,
    remote: Optional[NebulaRemote] = None,
    ...
    **kwargs,
    ...
    :param remote: A :py:class:`~nebulakit.remote.NebulaRemote` object to use for executing Nebula entities.
    ...
    :param local_entrypoint: If True, the eager workflow will can be executed locally but use the provided
        :py:func:`~nebulakit.remote.NebulaRemote` object to create task/workflow executions. This is useful for local
        testing against a remote Nebula cluster.
    :param kwargs: keyword-arguments forwarded to :py:func:`~nebulakit.task`.

    This type of workflow will execute all nebula entities within it eagerly, meaning that all python constructs can be
    used inside of an ``@eager``-decorated function. This is because eager workflows use a
    :py:class:`~nebulakit.remote.remote.NebulaRemote` object to kick off executions when a nebula entity needs to produce a
    value.
    ...
    Unlike :py:func:`dynamic workflows <nebulakit.dynamic>`, eager workflows are not compiled into a workflow spec, but
    uses python's `async <https://docs.python.org/3/library/asyncio.html>`__ capabilities to execute nebula entities.
    ...
    .. important::
       ...
       :py:class:`~nebulakit.remote.remote.NebulaRemote` using the ``client_credentials`` authentication, which is
       configured via :py:class:`~nebulakit.configuration.PlatformConfig`.
       ...
       .. code-block:: python
            ...
            @eager(
                remote=NebulaRemote(config=Config.auto(config_file="config.yaml")),
       ...
       Where ``config.yaml`` contains is a nebulactl-compatible config file.
       For more details, see `here <https://docs.nebula.org/projects/nebulactl/en/latest/#configuration>`__.

       When using a sandbox cluster started with ``nebulactl demo start``, however, the ``client_secret_group``
       and ``client_secret_key`` are not needed, :

       .. code-block:: python

            @eager(remote=NebulaRemote(config=Config.for_sandbox()))
            async def eager_workflow(x: int) -> int:
                ...
    ...
    .. important::
       ...
       workflow runtime will be local, but all task/subworkflow invocations will occur on the specified Nebula cluster.
...
def _prepare_remote(
    ...
    client_secret_key: Optional[str] = None,
    local_entrypoint: bool = False,
) -> Optional[NebulaRemote]:
    """Prepare NebulaRemote object for accessing Nebula cluster in a task running on the same cluster."""
    ...
    # Handle the case where this the task is running in a Nebula cluster and needs to access the cluster itself
    # via NebulaRemote.
    if remote.config.platform.endpoint.startswith("localhost"):
        # replace sandbox endpoints with internal dns, since localhost won't exist within the Nebula cluster
        return _internal_demo_remote(remote)
    return _internal_remote(remote, client_secret_group, client_secret_key)


def _internal_demo_remote(remote: NebulaRemote) -> NebulaRemote:
    """Derives a NebulaRemote object from a sandbox yaml configuration, modifying parts to make it work internally."""
    # replace sandbox endpoints with internal dns, since localhost won't exist within the Nebula cluster
    return NebulaRemote(
        config=remote.config.with_params(
            platform=PlatformConfig(
                endpoint=NEBULA_SANDBOX_INTERNAL_ENDPOINT,
                insecure=True,
                auth_mode="Pkce",
            ...
            data_config=DataConfig(
                s3=S3Config(
                    endpoint=NEBULA_SANDBOX_MINIO_ENDPOINT,
...
def _internal_remote(
    ...
    """Derives a NebulaRemote object from a yaml configuration file, modifying parts to make it work internally."""
    ...
    # get the raw output prefix from the context that's set from the pynebula-execute entrypoint
    # (see nebulakit/bin/entrypoint.py)
