...
from nebulakit.core import mock_stats
from nebulakit.core.context_manager import ExecutionParameters, NebulaContext, NebulaContextManager, SecretsManager
...
def test_levels():
    ...
    b = ctx.new_builder()
    b.nebula_client = SampleTestClass(value=1)
    with NebulaContextManager.with_context(b) as outer:
        assert outer.nebula_client.value == 1
        b = outer.new_builder()
        b.nebula_client = SampleTestClass(value=2)
        with NebulaContextManager.with_context(b) as ctx:
            assert ctx.nebula_client.value == 2

        with NebulaContextManager.with_context(outer.with_new_compilation_state()) as ctx:
            assert ctx.nebula_client.value == 1
...
def test_validate_image(mock_image):
    ...
    assert ic
    assert ic.default_image == Image(name="default", fqn="cr.nebula.org/nebulaclouds/nebulakit", tag="py3.9-latest")
...
def test_secrets_manager_file(tmpdir: py.path.local):
    ...
    os.environ["NEBULA_SECRETS_DEFAULT_DIR"] = tmp
    sec = SecretsManager()
    ...
    del os.environ["NEBULA_SECRETS_DEFAULT_DIR"]
...
def test_serialization_settings_transport():
    ...
    serialization_settings = SerializationSettings(
        ...
        nebulakit_virtualenv_root="/opt/venv/blah",
