...
from nebulaidl.plugins.sagemaker import parameter_ranges_pb2 as _pb2_params
from nebulakitplugins.awssagemaker.training import SagemakerBuiltinAlgorithmsTask, SagemakerCustomTrainingTask
from google.protobuf import json_format
from google.protobuf.json_format import MessageToDict

from nebulakit import NebulaContext
from nebulakit.configuration import SerializationSettings
from nebulakit.extend import DictTransformer, PythonTask, TypeEngine, TypeTransformer
...
# %%
# HPO Task allows ParameterRangeOneOf and HyperparameterTuningJobConfig as inputs. In nebulakit this is possible
...
class HPOTuningJobConfigTransformer(TypeTransformer[_hpo_job_model.HyperparameterTuningJobConfig]):
    ...
    def to_literal(
        ...
        d = MessageToDict(python_val.to_nebula_idl())
        return DictTransformer.dict_to_generic_literal(d)

    def to_python_value(
        self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[_hpo_job_model.HyperparameterTuningJobConfig]
        ...
        if lv and lv.scalar and lv.scalar.generic is not None:
            ...
            return _hpo_job_model.HyperparameterTuningJobConfig.from_nebula_idl(o)
        return None
...
class ParameterRangesTransformer(TypeTransformer[_params.ParameterRangeOneOf]):
    ...
    def to_python_value(
        self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[_params.ParameterRangeOneOf]
    ) -> _params.ParameterRangeOneOf:
        if lv and lv.scalar and lv.scalar.generic is not None:
            d = json.loads(json_format.MessageToJson(lv.scalar.generic))
            o = _pb2_params.ParameterRangeOneOf()
            o = json_format.ParseDict(d, o)
            return _params.ParameterRangeOneOf.from_nebula_idl(o)
        return None
