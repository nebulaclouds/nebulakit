...
from nebulakit.core.condition import ConditionalSection, conditional
...
from nebulakit.core.docstring import Docstring
from nebulakit.core.interface import (
    Interface,
...
from nebulakit.core.promise import (
    NodeOutput,
    Promise,
    VoidPromise,
    ...
    create_task_output,
    extract_obj_name,
    nebula_entity_call_handler,
    translate_inputs_to_literals,
)
from nebulakit.core.python_auto_container import PythonAutoContainerTask
from nebulakit.core.reference_entity import ReferenceEntity, WorkflowReference
from nebulakit.core.tracker import extract_task_module
from nebulakit.core.type_engine import TypeEngine
from nebulakit.exceptions import scopes as exception_scopes
from nebulakit.exceptions.user import NebulaValidationException, NebulaValueException
...
from nebulakit.models.documentation import Description, Documentation
...
class WorkflowMetadata(object):
    ...
    def __post_init__(self):
        if (
            ...
            raise NebulaValidationException(f"Failure policy {self.on_failure} not acceptable")

    def to_nebula_model(self):
...
class WorkflowMetadataDefaults(object):
    ...
    def __post_init__(self):
        ...
        if self.interruptible is not True and self.interruptible is not False:
            raise NebulaValidationException(f"Interruptible must be boolean, {self.interruptible} invalid")
...
def get_promise(binding_data: _literal_models.BindingData, outputs_cache: Dict[Node, Dict[str, Promise]]) -> Promise:
    ...
    raise NebulaValidationException("Binding type unrecognized.")
...
class WorkflowBase(object):
    def __init__(
        ...
        **kwargs,
        ...
        NebulaEntities.entities.append(self)
    ...
    def __call__(self, *args, **kwargs) -> Union[Tuple[Promise], Promise, VoidPromise, Tuple, Coroutine, None]:
        ...
        self.compile()
        try:
            return nebula_entity_call_handler(self, *args, **input_kwargs)
    ...
    def local_execute(self, ctx: NebulaContext, **kwargs) -> Union[Tuple[Promise], Promise, VoidPromise, None]:
        ...
        # holding Nebula literal values. Even in a wf, a user can call a sub-workflow with a Python native value.
        literal_map = translate_inputs_to_literals(
            ctx,
            incoming_values=kwargs,
            nebula_interface_types=self.interface.inputs,
        ...
        if len(self.python_interface.outputs) == 0:
            raise NebulaValueException(function_outputs, "Interface output should've been VoidPromise or None.")
        ...
        wf_outputs_as_literal_dict = translate_inputs_to_literals(
            ctx,
            wf_outputs_as_map,
            nebula_interface_types=self.interface.outputs,
...
class ImperativeWorkflow(WorkflowBase):
    ...
    .. literalinclude:: ../../../tests/nebulakit/unit/core/test_imperative.py
    ...
    def __init__(
        ...
        # is an error that Admin would return at compile time anyways, but this allows nebulakit to raise
        # the error earlier.
    ...
    def execute(self, **kwargs):
        ...
        """
        if not self.ready():
            raise NebulaValidationException(f"Workflow not ready, wf is currently {self}")
        ...
        for node in self.compilation_state.nodes:
            ...
            if len(entity.python_interface.outputs) == 0:
                raise NebulaValueException(results, "Interface output should've been VoidPromise or None.")
            ...
            if len(expected_output_names) == 1:
                ...
                if len(results) != len(expected_output_names):
                    raise NebulaValueException(results, f"Different lengths {results} {expected_output_names}")
    ...
    def create_conditional(self, name: str) -> ConditionalSection:
        ...
        NebulaContextManager.with_context(ctx.with_compilation_state(self.compilation_state))
    ...
    def add_entity(self, entity: Union[PythonTask, LaunchPlan, WorkflowBase], **kwargs) -> Node:
        ...
        if ctx.compilation_state is not None:
            raise Exception("Can't already be compiling")
        with NebulaContextManager.with_context(ctx.with_compilation_state(self.compilation_state)) as ctx:
    ...
    def add_workflow_input(self, input_name: str, python_type: Type) -> Promise:
        ...
        if input_name in self._inputs:
            raise NebulaValidationException(f"Input {input_name} has already been specified for wf {self.name}.")
    ...
    def add_workflow_output(
        ...
        if output_name in self._python_interface.outputs:
            raise NebulaValidationException(f"Output {output_name} already exists in workflow {self.name}")
        ...
        if python_type is None:
            ...
            python_type = promise.ref.node.nebula_entity.python_interface.outputs[promise.var]
            logger.debug(f"Inferring python type for wf output {output_name} from Promise provided {python_type}")

        nebula_type = TypeEngine.to_literal_type(python_type=python_type)
        ...
        with NebulaContextManager.with_context(ctx.with_compilation_state(self.compilation_state)) as ctx:
            b, _ = binding_from_python_std(
                ctx, output_name, expected_literal_type=nebula_type, t_value=p, t_value_type=python_type
            )
...
class PythonFunctionWorkflow(WorkflowBase, ClassStorageTaskResolver):
    """
    Please read :std:ref:`nebula:divedeep-workflows` first for a high-level understanding of what workflows are in Nebula.
    This Python object represents a workflow  defined by a function and decorated with the
    :py:func:`@workflow <nebulakit.workflow>` decorator. Please see notes on that object for additional information.
    """
    ...
    def compile(self, **kwargs):
        ...
        with NebulaContextManager.with_context(
            ...
            for n in comp_ctx.compilation_state.nodes:
                if isinstance(n.nebula_entity, PythonAutoContainerTask) and n.nebula_entity.task_resolver == self:
                    logger.debug(f"WF {self.name} saving task {n.nebula_entity.name}")
                    self.add(n.nebula_entity)
...
def workflow(
    ...
    """
    This decorator declares a function to be a Nebula workflow. Workflows are declarative entities that construct a DAG
    ...
    Nebula.
    That is, workflows should not call non-Nebula entities since they are only run once (again, this is with respect to
    the platform, local runs notwithstanding).

    Example:

    .. literalinclude:: ../../../tests/nebulakit/unit/core/test_workflows.py
    ...
    actually not. When nebulakit scans the workflow function, the objects being passed around between the tasks are not
    ...
    :param failure_policy: Use the options in nebulakit.WorkflowFailurePolicy
...
class ReferenceWorkflow(ReferenceEntity, PythonFunctionWorkflow):  # type: ignore
    """
    A reference workflow is a pointer to a workflow that already exists on your Nebula installation. This
...
def reference_workflow(
    ...
    .. literalinclude:: ../../../tests/nebulakit/unit/core/test_references.py
       :pyobject: ref_wf1
    """
