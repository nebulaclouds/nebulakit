...
from nebulaidl.admin.agent_pb2 import PERMANENT_FAILURE, RETRYABLE_FAILURE, RUNNING, SUCCEEDED, State
from kubernetes.utils.quantity import parse_quantity

from nebulakit.core.resources import Resources

MMCLOUD_STATUS_TO_NEBULA_STATE = {
    "Submitted": RUNNING,
...
def mmcloud_status_to_nebula_state(status: str) -> State:
    """
    Map MMCloud status to Nebula state.
    """
    return MMCLOUD_STATUS_TO_NEBULA_STATE[status]


def nebula_to_mmcloud_resources(
    requests: Optional[Resources] = None,
    limits: Optional[Resources] = None,
) -> Tuple[int, int, int, int]:
    """
    Map Nebula (K8s) resources to MMCloud resources.
    """
    B_IN_GIB = 1073741824
