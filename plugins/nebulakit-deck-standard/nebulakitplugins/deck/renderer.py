...
class ImageRenderer:
    """Converts a NebulaFile or PIL.Image.Image object to an HTML string with the image data
    represented as a base64-encoded string.
    """

    def to_html(self, image_src: Union[NebulaFile, "PIL.Image.Image"]) -> str:
        img = self._get_image_object(image_src)
        return self._image_to_html_string(img)

    @staticmethod
    def _get_image_object(image_src: Union[NebulaFile, "PIL.Image.Image"]) -> "PIL.Image.Image":
        if isinstance(image_src, NebulaFile):
