# Nebulakit Identity Aware Proxy
...
This nebulakit plugin allows users to generate ID tokens via an external command for use with Nebula deployments protected with IAP. A step by step guide to protect a Nebula deployment with IAP is provided as well.

**Disclaimer: Do not choose this deployment path with the goal of *a* Nebula deployment configured with authentication on GCP. The deployment is more involved than the standard Nebula GCP deployment. Follow this guide if your organization has a security policy that requires the use of GCP Identity Aware Proxy.**

## Configuring the token generation CLI provided by this plugin

1. Install this plugin via `pip install nebulakitplugins-identity-aware-proxy`.

    Verify the installation with `nebula-iap --help`.
...
2. Create OAuth 2.0 credentials for both the token generation CLI and for IAP.
    ...
    2. Follow the instructions to [activate IAP](https://cloud.google.com/iap/docs/enabling-kubernetes-howto#enabling_iap) in your project and cluster. In the process you will create web application type OAuth credentials for IAP (similar as done above for the desktop application type credentials). Again, note the client id and client secret. Don't proceed with the instructions to create the Kubernetes secret for these credentials and the backend config yet, this is done in the deployment guide below. Stop when you have the client id and secret.
        ...
        Note: In case you have an existing [Nebula deployment with auth configured](https://docs.nebula.org/en/latest/deployment/configuration/auth_setup.html#apply-oidc-configuration), you likely already have web application type OAuth credentials. You can reuse those credentials for Nebula's IAP.
...
4. Test the token generation CLI:
    ...
    ```console
    nebula-iap generate-user-id-token \
...
5. Configure proxy authorization with this CLI in `~/.nebula/config.yaml`:

    ```yaml
    admin:
      endpoint: dns:///<fill in your nebula domain >.com
      insecure: false
      insecureSkipVerify: true
      authType: Pkce
      proxyCommand: ["nebula-iap", "generate-user-id-token", "--desktop_client_id", ...]  # Add this line
    ```

    This configures the Nebula clients to send `"proxy-authorization"` headers with the token generated by the CLI with every request in order to pass the GCP Identity Aware Proxy.
...
## Configuring your Nebula deployment to use IAP

### Introduction

To protect your Nebula deployment with IAP, we have to deploy it with a GCE ingress (instead of the Nginx ingress used by the default Nebula deployment).

Nebulaadmin has a gRPC endpoint. The gRPC protocol requires the use of http2. When using http2 between a GCP load balancer (created by the GCE ingress) and a backend in GKE, the use of TLS is required ([see documentation](https://cloud.google.com/kubernetes-engine/docs/how-to/ingress-http2)):
...
Nebula is then deployed behind the Istio ingress gateway and does not need to be configured to use TLS itself.
...
4. [Install Cert Manager](https://cert-manager.io/docs/installation/helm/) to [create and rotate](https://cert-manager.io/docs/configuration/selfsigned/) a self-signed certificate for the istio ingress (pod):
    ...
    * `helm install cert-manager jetstack/cert-manager --namespace cert-manager --create-namespace --set installCRDs=true`
    ...
    This additional self-managed encryption is also required to use http2 and in extension gRPC. To repeat, we mainly add this self-signed certificate in order to be able to expose a gRPC service (nebulaadmin) via a GCP load balancer, less for the additional encryption.
...
6. Deploy the GCE ingress that will route traffic to the istio ingress gateway:
    ...
    * Create a DNS record for your Nebula domain to route traffic to this static IP address.
    * Create a GCP managed certificate (please fill in your domain):
        ...
        metadata:
          name: nebula-managed-certificate
          namespace: istio-system
        spec:
          domains:
            - < fill in your domain >
    ...
    * Create the ingress (please fill in the name of the static IP):
        ...
        metadata:
          annotations:
            ...
            networking.gke.io/managed-certificates: nebula-managed-certificate
...
7. Connect nebulaadmin and nebulaconsole to the istio ingress gateway:
    ...
    Next, we configure the Istio service mesh to route requests from the Istio ingress gateway to nebulaadmin and nebulaconsole.

    In istio, this is configured using a so-called `VirtualService` object.

    Please fill in your nebula domain in the following manifest and apply it to the cluster:
    ...
    In this `VirtualService`, the routing rules for nebulaadmin and nebulaconsole are configured which in Nebula's default deployment are configured in the Nginx ingress.
...
8. Test your Nebula deployment with IAP by e.g. executing this python script:
    ...
    A browser window should open and ask you to login with your Google account. You should then see confirmation that you *"Successfully logged into accounts.google.com"* (this was for the IAP), finally followd by confirmation that you *"Successfully logged into 'your nebula domain'"* (this was for Nebula itself).



9. At this point your Nebula deployment should be successfully protected by a GCP identity aware proxy using a zero trust model.
    ...
    You could also open the nebula console in an incognito browser window and verify that you are asked to login with your Google account.

    Finally, you could also comment out the `proxyCommand` line in your `~/.nebula/config.yaml` and verify that you are no longer able to access your Nebula deployment behind IAP.

10. The double login observed above is due to the fact that the Nebula clients send `"proxy-authorization"` headers generated by the CLI provided by this plugin with every request in order to make it past IAP. They still also send the regular `"authorization"` header issued by nebulaadmin itself.

      Since the refresh token for Nebula and the one for IAP by default don't have the same lifespan, you likely won't notice this double login  again. However, since your deployment is already protected by IAP, the ID token (issued by nebulaadmin) in the `"authorization"` header mostly serves to identify users. Therefore, you can consider to increase the lifespan of the refresh token issued by nebulaadmin to e.g. 7 days by setting `configmap.adminServer.auth.appAuth.selfAuthServer.refreshTokenLifespan` to e.g. `168h0m0s` in your Nebula helm values file. This way, your users should barely notice the double login.
