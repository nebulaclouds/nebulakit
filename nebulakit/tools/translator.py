...
from nebulakit import PythonFunctionTask, SourceCode
from nebulakit.configuration import SerializationSettings
from nebulakit.core import constants as _common_constants
from nebulakit.core.array_node_map_task import ArrayNodeMapTask
from nebulakit.core.base_task import PythonTask
from nebulakit.core.condition import BranchNode
from nebulakit.core.container_task import ContainerTask
from nebulakit.core.gate import Gate
from nebulakit.core.launch_plan import LaunchPlan, ReferenceLaunchPlan
from nebulakit.core.map_task import MapPythonTask
from nebulakit.core.node import Node
from nebulakit.core.python_auto_container import PythonAutoContainerTask
from nebulakit.core.reference_entity import ReferenceEntity, ReferenceSpec, ReferenceTemplate
from nebulakit.core.task import ReferenceTask
from nebulakit.core.utils import _dnsify
from nebulakit.core.workflow import ReferenceWorkflow, WorkflowBase
from nebulakit.models import common as _common_models
from nebulakit.models import common as common_models
from nebulakit.models import interface as interface_models
...
from nebulakit.models.core import workflow as _core_wf
from nebulakit.models.core import workflow as workflow_model
from nebulakit.models.core.workflow import ApproveCondition
from nebulakit.models.core.workflow import ArrayNode as ArrayNodeModel
from nebulakit.models.core.workflow import BranchNode as BranchNodeModel
from nebulakit.models.core.workflow import GateNode, SignalCondition, SleepCondition, TaskNodeOverrides
from nebulakit.models.task import TaskSpec, TaskTemplate
from nebulakit.core.utils import ClassDecorator

NebulaLocalEntity = Union[
    PythonTask,
    BranchNode,
    Node,
    LaunchPlan,
...
NebulaControlPlaneEntity = Union[
    TaskSpec,
...
class Options(object):
    ...
    in a Nebula backend, and also when registering launch plans.
...
def get_serializable_workflow(
    ...
    for n in entity.nodes:
        ...
        if isinstance(n.nebula_entity, WorkflowBase):
            # We are currently not supporting reference workflows since these will
            # require a network call to nebulaadmin to populate the WorkflowTemplate
            # object
            if isinstance(n.nebula_entity, ReferenceWorkflow):
                raise Exception(
                    "Reference sub-workflows are currently unsupported. Use reference launch plans instead."
                )
            sub_wf_spec = get_serializable(entity_mapping, settings, n.nebula_entity, options)
        ...
        from nebulakit.remote import NebulaWorkflow

        if isinstance(n.nebula_entity, NebulaWorkflow):
            for swf in n.nebula_entity.nebula_sub_workflows:
                sub_wf = get_serializable(entity_mapping, settings, swf, options)
                sub_wfs.append(sub_wf.template)
            main_wf = get_serializable(entity_mapping, settings, n.nebula_entity, options)
            sub_wfs.append(main_wf.template)

        if isinstance(n.nebula_entity, BranchNode):
            if_else: workflow_model.IfElseBlock = n.nebula_entity._ifelse_block
            ...
            for leaf_node in leaf_nodes:
                if isinstance(leaf_node.nebula_entity, WorkflowBase):
                    sub_wf_spec = get_serializable(entity_mapping, settings, leaf_node.nebula_entity, options)
                    sub_wfs.append(sub_wf_spec.template)
                    sub_wfs.extend(sub_wf_spec.sub_workflows)
                elif isinstance(leaf_node.nebula_entity, NebulaWorkflow):
                    get_serializable(entity_mapping, settings, leaf_node.nebula_entity, options)
                    sub_wfs.append(leaf_node.nebula_entity)
                    sub_wfs.extend([s for s in leaf_node.nebula_entity.sub_workflows.values()])
    ...
    wf_t = workflow_model.WorkflowTemplate(
        id=wf_id,
        metadata=entity.workflow_metadata.to_nebula_model(),
        metadata_defaults=entity.workflow_metadata_defaults.to_nebula_model(),
        interface=entity.interface,
...
def get_serializable_node(
    ...
    if entity.nebula_entity is None:
        raise Exception(f"Node {entity.id} has no nebula entity")
    ...
    if isinstance(entity.nebula_entity, ReferenceEntity):
        ref_spec = get_serializable(entity_mapping, settings, entity.nebula_entity, options=options)
        ...
        if ref_template.resource_type == _identifier_model.ResourceType.TASK:
            ...
            raise Exception(
                f"Unexpected resource type for reference entity {entity.nebula_entity}: {ref_template.resource_type}"
            )
        return node_model

    from nebulakit.remote import NebulaLaunchPlan, NebulaTask, NebulaWorkflow

    if isinstance(entity.nebula_entity, ArrayNodeMapTask):
        node_model = workflow_model.Node(
            id=_dnsify(entity.id),
            metadata=entity.metadata,
            inputs=entity.bindings,
    ...
    elif isinstance(entity.nebula_entity, PythonTask):
        task_spec = get_serializable(entity_mapping, settings, entity.nebula_entity, options=options)
        node_model = workflow_model.Node(
            id=_dnsify(entity.id),
            metadata=entity.metadata,
            inputs=entity.bindings,
    ...
    elif isinstance(entity.nebula_entity, WorkflowBase):
        wf_spec = get_serializable(entity_mapping, settings, entity.nebula_entity, options=options)
        node_model = workflow_model.Node(
            id=_dnsify(entity.id),
            metadata=entity.metadata,
            inputs=entity.bindings,
    ...
    elif isinstance(entity.nebula_entity, BranchNode):
        node_model = workflow_model.Node(
            id=_dnsify(entity.id),
            metadata=entity.metadata,
            inputs=entity.bindings,
            upstream_node_ids=[n.id for n in upstream_nodes],
            output_aliases=[],
            branch_node=get_serializable(entity_mapping, settings, entity.nebula_entity, options=options),
        )

    elif isinstance(entity.nebula_entity, LaunchPlan):
        lp_spec = get_serializable(entity_mapping, settings, entity.nebula_entity, options=options)

        # Node's inputs should not contain the data which is fixed input
        node_input = []
        for b in entity.bindings:
            if b.var not in entity.nebula_entity.fixed_inputs.literals:
                node_input.append(b)
    ...
    elif isinstance(entity.nebula_entity, Gate):
        if entity.nebula_entity.sleep_duration:
            gn = GateNode(sleep=SleepCondition(duration=entity.nebula_entity.sleep_duration))
        elif entity.nebula_entity.input_type:
            output_name = list(entity.nebula_entity.python_interface.outputs.keys())[0]  # should be o0
            gn = GateNode(
                signal=SignalCondition(
                    entity.nebula_entity.name, type=entity.nebula_entity.literal_type, output_variable_name=output_name
                )
            )
        else:
            gn = GateNode(approve=ApproveCondition(entity.nebula_entity.name))
        node_model = workflow_model.Node(
            id=_dnsify(entity.id),
            metadata=entity.metadata,
            inputs=entity.bindings,
    ...
    elif isinstance(entity.nebula_entity, NebulaTask):
        # Recursive call doesn't do anything except put the entity on the map.
        get_serializable(entity_mapping, settings, entity.nebula_entity, options=options)
        node_model = workflow_model.Node(
            id=_dnsify(entity.id),
            metadata=entity.metadata,
            inputs=entity.bindings,
            ...
            output_aliases=[],
            task_node=workflow_model.TaskNode(
                reference_id=entity.nebula_entity.id,
                overrides=TaskNodeOverrides(resources=entity._resources, extended_resources=entity._extended_resources),
            ),
        )
    elif isinstance(entity.nebula_entity, NebulaWorkflow):
        wf_spec = get_serializable(entity_mapping, settings, entity.nebula_entity, options=options)
        for sub_wf in entity.nebula_entity.nebula_sub_workflows:
    ...
    elif isinstance(entity.nebula_entity, NebulaLaunchPlan):
        # Recursive call doesn't do anything except put the entity on the map.
        get_serializable(entity_mapping, settings, entity.nebula_entity, options=options)
        # Node's inputs should not contain the data which is fixed input
        node_input = []
        ...
        node_model = workflow_model.Node(
            ...
            upstream_node_ids=[n.id for n in upstream_nodes],
            output_aliases=[],
            workflow_node=workflow_model.WorkflowNode(launchplan_ref=entity.nebula_entity.id),
        )
    else:
        raise Exception(f"Node contained non-serializable entity {entity._nebula_entity}")
...
def get_serializable_array_node(
    ...
    # TODO Add support for other nebula entities
    entity = node.nebula_entity
...
def get_serializable_nebula_workflow(
    entity: "NebulaWorkflow", settings: SerializationSettings
...
def get_serializable_nebula_task(entity: "NebulaTask", settings: SerializationSettings) -> NebulaControlPlaneEntity:
    """
    TODO replace with deep copy
    """
    entity.id._project = settings.project
...
def get_serializable(
    ...
    """
    The nebulakit authoring code produces objects representing Nebula entities (tasks, workflows, etc.). In order to
    register these, they need to be converted into objects that Nebula Admin understands (the IDL objects basically, but
    ...
    :param entity: The local nebula entity to try to convert (along with its dependencies)
    ...
    elif isinstance(entity, NebulaTask) or isinstance(entity, NebulaWorkflow):
        if entity.should_register:
            if isinstance(entity, NebulaTask):
                cp_entity = get_serializable_nebula_task(entity, settings)
            else:
                if entity.should_register:
                    ...
                    for t in entity.nebula_tasks:
                        get_serializable(entity_mapping, settings, t, options)
                cp_entity = get_serializable_nebula_workflow(entity, settings)
        else:
            cp_entity = entity

    elif isinstance(entity, NebulaLaunchPlan):
        cp_entity = entity
        ...
        if entity.docs and entity.docs.long_description:
            if entity.docs.long_description.value:
                if sys.getsizeof(entity.docs.long_description.value) > 16 * 1024 * 1024:
                    raise ValueError(
                        "Long Description of the nebula entity exceeds the 16KB size limit. Please specify the uri in the long description instead."
                    )
