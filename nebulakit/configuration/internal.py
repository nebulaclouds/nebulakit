...
from nebulakit.configuration.file import ConfigEntry, ConfigFile, LegacyConfigEntry, YamlConfigEntry
...
class Credentials(object):
    ...
    """
    This command is executed to return a token for authorization with a proxy in front of Nebula using an external process.
    """

    CLIENT_ID = ConfigEntry(LegacyConfigEntry(SECTION, "client_id"), YamlConfigEntry("admin.clientId"))
    """
    This is the public identifier for the app which handles authorization for a Nebula deployment.
    More details here: https://www.oauth.com/oauth2-servers/client-registration/client-id-secret/.
    """

    CLIENT_CREDENTIALS_SECRET = ConfigEntry(LegacyConfigEntry(SECTION, "client_secret"))
    """
    Used for basic auth, which is automatically called during pynebula. This will allow the Nebula engine to read the
...
class Secrets(object):
    ...
    NEBULA_SECRETS_ENV_PREFIX variable
    """
...
class StatsD(object):
    SECTION = "secrets"
    # StatsD Config flags should ideally be controlled at the platform level and not through nebulakit's config file.
