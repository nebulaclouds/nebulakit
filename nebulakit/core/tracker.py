...
class InstanceTrackingMeta(type):
    """
    Please see the original class :py:class`nebulakit.common.mixins.registerable._InstanceTracker` also and also look
    at the tests in the ``tests/nebulakit/unit/core/tracker/test_tracking/`` folder to see how it's used.
    ...
    def _find_instance_module():
        ...
        while frame:
            if frame.f_code.co_name == "<module>" and "__name__" in frame.f_globals:
                ...
                # This is useful in cases that use NebulaRemote to load tasks/workflows that are defined
                # in the same file as where NebulaRemote is being invoked to register and execute Nebula
                # entities. One such case is with the `eager` decorator in the nebulakit.experimental module.
...
class TrackedInstance(metaclass=InstanceTrackingMeta):
    ...
    * Keep track of naming for non-function ``PythonAutoContainerTasks``.  That is, things like the
      :py:class:`nebulakit.extras.sqlite3.task.SQLite3Task` task.
    * Task resolvers, because task resolvers are instances of :py:class:`nebulakit.core.python_auto_container.TaskResolverMixin`
    ...
    def find_lhs(self) -> str:
        ...
        if self._instantiated_in is None or self._instantiated_in == "":
            raise _system_exceptions.NebulaSystemException(f"Object {self} does not have an _instantiated in")
        ...
        raise _system_exceptions.NebulaSystemException(f"Error looking for LHS in {self._instantiated_in}")
...
def get_full_module_path(mod: ModuleType, mod_name: str) -> str:
    if FeatureFlags.NEBULA_PYTHON_PACKAGE_ROOT != ".":
        package_root = (
            FeatureFlags.NEBULA_PYTHON_PACKAGE_ROOT if FeatureFlags.NEBULA_PYTHON_PACKAGE_ROOT != "auto" else None
        )
