...
from nebulakit import NebulaContextManager, logger
from nebulakit.configuration import SerializationSettings
from nebulakit.core.base_task import PythonTask, TaskResolverMixin
...
from nebulakit.extend.backend.base_agent import AsyncAgentExecutorMixin


@dataclass
class AirflowObj(object):
    """
    This class is used to store the Airflow task configuration. It is serialized and stored in the Nebula task config.
...
class AirflowContainerTask(PythonAutoContainerTask[AirflowObj]):
    ...
    These tasks don't have async method to get the job status, so cannot be used in the Nebula agent. We run these tasks in a container.
    """
...
class AirflowTask(AsyncAgentExecutorMixin, PythonTask[AirflowObj]):
    """
    This python task is used to wrap an Airflow task. It is used to run an Airflow task in Nebula agent.
...
def _get_airflow_instance(airflow_obj: AirflowObj) -> typing.Union[BaseOperator, BaseSensorOperator, BaseTrigger]:
    ...
    # airflow task instead of the Nebula task.
...
def _nebula_operator(*args, **kwargs):
    """
    This function is called by the Airflow operator to create a new task. We intercept this call and return a Nebula
    task instead.
    """
    cls = args[0]
    try:
        if NebulaContextManager.current_context().user_space_params.get_original_task:
...
def _nebula_xcom_push(*args, **kwargs):
    ...
    NebulaContextManager.current_context().user_space_params.xcom_data = args[1:]


params = NebulaContextManager.current_context().user_space_params
params.builder().add_attr("GET_ORIGINAL_TASK", False).add_attr("XCOM_DATA", {}).build()

# Monkey patch the Airflow operator. Instead of creating an airflow task, it returns a Nebula task.
BaseOperator.__new__ = _nebula_operator
