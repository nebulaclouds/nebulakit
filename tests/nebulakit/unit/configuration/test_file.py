...
from nebulakit.configuration import ConfigEntry, get_config_file, set_if_exists
from nebulakit.configuration.file import LegacyConfigEntry, _exists
from nebulakit.configuration.internal import Platform
...
def test_config_entry_envvar():
    ...
    os.environ["NEBULA_TEST_OP1"] = "xyz"
    assert c.read() == "xyz"
...
def test_config_entry_file():
    ...
    assert c.read(cfg) == "fakenebula.com"
...
def test_config_entry_precedence():
    ...
    old_environ = dict(os.environ)
    os.environ["NEBULA_PLATFORM_URL"] = "xyz"
    cfg = get_config_file(os.path.join(os.path.dirname(os.path.realpath(__file__)), "configs/good.config"))
...
@mock.patch("nebulakit.configuration.file.LegacyConfigEntry.read_from_file")
def test_env_var_bool_transformer(mock_file_read):
    mock_file_read.return_value = None
    test_env_var = "NEBULA_MADEUP_TEST_VAR_ABC123"
