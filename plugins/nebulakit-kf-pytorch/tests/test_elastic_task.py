...
from nebulakitplugins.kfpytorch.task import Elastic
...
def test_execution_params(
    ...
    if monkeypatch_exec_id_env_var:
        monkeypatch.setenv("NEBULA_INTERNAL_EXECUTION_ID", target_exec_id)
...
def test_deck(start_method: str) -> None:
    ...
    def train():
        ...
        ctx = nebulakit.current_context()
        deck = nebulakit.Deck("test-deck", f"Hello Nebula Deck viewer from worker process {os.environ.get('RANK')}")
    ...
    assert "Hello Nebula Deck viewer from worker process 0" in test_deck.html
...
def test_recoverable_error(recoverable: bool, start_method: str) -> None:
    ...
    class CustomRecoverableException(NebulaRecoverableException):
        pass
