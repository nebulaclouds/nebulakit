...
class Deck:
    ...
    Nebula context saves a list of deck objects, and we use renderers in those decks to render
    ...
    .. code-block:: python
        ...
        @task()
        def t1() -> str:
            md_text = '#Hello Nebula##Hello Nebula###Hello Nebula'
            m = MarkdownRenderer()
            s = BoxRenderer("sepal_length")
            deck = nebulakit.Deck("demo", s.to_html(iris_df))
            deck.append(m.to_html(md_text))
            default_deck = nebulakit.current_context().default_deck
    ...
    def __init__(self, name: str, html: Optional[str] = ""):
        self._name = name
        self._html = html
        NebulaContextManager.current_context().user_space_params.decks.append(self)
...
class TimeLineDeck(Deck):
    ...
    def html(self) -> str:
        try:
            from nebulakitplugins.deck.renderer import GanttChartRenderer, TableRenderer
        except ImportError:
            warning_info = "Plugin 'nebulakit-deck-standard' is not installed. To display time line, install the plugin in the image."
...
def _output_deck(task_name: str, new_user_params: ExecutionParameters):
    ...
    try:
        ...
        logger.info(f"{task_name} task creates nebula deck html to file://{local_path}")
        ...
        logger.error(f"Failed to write nebula deck html with error {e}.")
