...
from nebulaidl.core import security_pb2 as _sec

from nebulakit.models import common as _common


@dataclass
class Secret(_common.NebulaIdlEntity):
    """
    ...
    def to_nebula_idl(self) -> _sec.Secret:
        return _sec.Secret(
            group=self.group,
    ...
    @classmethod
    def from_nebula_idl(cls, pb2_object: _sec.Secret) -> "Secret":
        return cls(
...
@dataclass
class OAuth2Client(_common.NebulaIdlEntity):
    client_id: str
    client_secret: str

    def to_nebula_idl(self) -> _sec.OAuth2Client:
    ...
    @classmethod
    def from_nebula_idl(cls, pb2_object: _sec.OAuth2Client) -> "OAuth2Client":
        return cls(
...
class Identity(_common.NebulaIdlEntity):
    ...
    def to_nebula_idl(self) -> _sec.Identity:
        return _sec.Identity(
            ...
            oauth2_client=self.oauth2_client.to_nebula_idl() if self.oauth2_client else None,
        )

    @classmethod
    def from_nebula_idl(cls, pb2_object: _sec.Identity) -> "Identity":
        return cls(
            ...
            oauth2_client=OAuth2Client.from_nebula_idl(pb2_object.oauth2_client)
...
@dataclass
class OAuth2TokenRequest(_common.NebulaIdlEntity):
    class Type(Enum):
    ...
    def to_nebula_idl(self) -> _sec.OAuth2TokenRequest:
        return _sec.OAuth2TokenRequest(
            name=self.name,
            type=self.type_,
            token_endpoint=self.token_endpoint,
            idp_discovery_endpoint=self.idp_discovery_endpoint,
            client=self.client.to_nebula_idl() if self.client else None,
        )

    @classmethod
    def from_nebula_idl(cls, pb2_object: _sec.OAuth2TokenRequest) -> "OAuth2TokenRequest":
        return cls(
            ...
            client=OAuth2Client.from_nebula_idl(pb2_object.client) if pb2_object.HasField("client") else None,
        )


@dataclass
class SecurityContext(_common.NebulaIdlEntity):
    """
    This is a higher level wrapper object that for the most part users shouldn't have to worry about. You should
    be able to just use :py:class:`nebulakit.Secret` instead.
    """
    ...
    def to_nebula_idl(self) -> _sec.SecurityContext:
        if self.run_as is None and self.secrets is None and self.tokens is None:
            return None
        return _sec.SecurityContext(
            run_as=self.run_as.to_nebula_idl() if self.run_as else None,
            secrets=[s.to_nebula_idl() for s in self.secrets] if self.secrets else None,
            tokens=[t.to_nebula_idl() for t in self.tokens] if self.tokens else None,
        )

    @classmethod
    def from_nebula_idl(cls, pb2_object: _sec.SecurityContext) -> "SecurityContext":
        return cls(
            run_as=Identity.from_nebula_idl(pb2_object.run_as)
            if pb2_object.run_as and pb2_object.run_as.ByteSize() > 0
            else None,
            secrets=[Secret.from_nebula_idl(s) for s in pb2_object.secrets] if pb2_object.secrets else None,
            tokens=[OAuth2TokenRequest.from_nebula_idl(t) for t in pb2_object.tokens] if pb2_object.tokens else None,
        )
