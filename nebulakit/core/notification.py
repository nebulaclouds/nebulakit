...
the Nebula platform to send emails when a workflow run reaches certain stages (fails or succeeds, etc.).

.. note::

    Notifications require some setup and configuration on the Nebula platform side. Please contact your Nebula platform
    admins to get this feature enabled. See :std:ref:`cookbook:setting up workflow notifications`

Each notification type takes a list of :py:class:`nebulakit.models.core.execution.WorkflowExecutionPhase` and a list of
...
.. autoclass:: nebulakit.core.notification.Notification
...
# Duplicates nebulakit.common.notifications.Notification to avoid using the ExtendedSdkType metaclass.
class Notification(_common_model.Notification):
    ...
    def __init__(
        ...
        :param list[int] phases: A required list of phases for which to fire the event.  Events can only be fired for
            terminal phases.  Phases should be as defined in: nebulakit.models.core.execution.WorkflowExecutionPhase
        """
...
class Email(Notification):
    ...
    def __init__(self, phases: List[int], recipients_email: List[str]):
        """
        :param list[int] phases: A required list of phases for which to fire the event.  Events can only be fired for
            terminal phases. Phases should be as defined in: :py:class:`nebulakit.models.core.execution.WorkflowExecutionPhase`
        :param list[str] recipients_email: A required non-empty list of recipients for the notification.
        """
