...
from nebulakit import BlobType, NebulaContext, NebulaContextManager, Literal, LiteralType, StructuredDataset
...
class DirParamType(click.ParamType):
    ...
    def convert(
        ...
        # set remote_directory to false if running pynebula run locally. This makes sure that the original
        # directory is used and not a random one.
        remote_directory = None if getattr(ctx.obj, "is_remote", False) else False
        if p.exists() and p.is_dir():
            return NebulaDirectory(path=value, remote_directory=remote_directory)
...
class FileParamType(click.ParamType):
    ...
    def convert(
        ...
        return NebulaFile(path=value, remote_path=remote_path)
...
class PickleParamType(click.ParamType):
    ...
    def convert(
        ...
        if os.path.isfile(value):
            return NebulaFile(path=value, remote_path=remote_path)
        uri = NebulaContextManager.current_context().file_access.get_random_local_path()
        with open(uri, "w+b") as outfile:
            cloudpickle.dump(value, outfile)
        return NebulaFile(path=str(pathlib.Path(uri).resolve()), remote_path=remote_path)
...
def literal_type_to_click_type(lt: LiteralType, python_type: typing.Type) -> click.ParamType:
    """
    Converts a Nebula LiteralType given a python_type to a click.ParamType
    """
    if lt.simple:
        ...
        raise NotImplementedError(f"Type {lt.simple} is not supported in pynebula run")
    ...
    if lt.blob:
        if lt.blob.dimensionality == BlobType.BlobDimensionality.SINGLE:
            if lt.blob.format == NebulaPickleTransformer.PYTHON_PICKLE_FORMAT:
...
class NebulaLiteralConverter(object):
    name = "literal_type"
    ...
    def convert(
        ...
        """
        Convert the value to a Nebula Literal or a python native type. This is used by click to convert the input.
        """
        try:
            ...
            if self._python_type is datetime.date:
                ...
                value = value.date()
            lit = TypeEngine.to_literal(self._nebula_ctx, value, self._python_type, self._literal_type)

            if not self._is_remote:
                # If this is used for remote execution then we need to convert it back to a python native type
                # for NebulaRemote to use it. This maybe a double conversion penalty!
                return TypeEngine.to_python_value(self._nebula_ctx, lit, self._python_type)
            return lit
