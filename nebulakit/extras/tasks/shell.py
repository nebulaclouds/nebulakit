...
from nebulakit.core.python_function_task import PythonInstanceTask
...
class _PythonFStringInterpolizer:
    ...
    class _Formatter(string.Formatter):
        def format_field(self, value, format_spec):
            ...
            the type. Handles NebulaFile and NebulaDirectory specially.
            Downloads and returns the downloaded filepath.
            """
            if isinstance(value, NebulaFile):
                value.download()
                return value.path
            if isinstance(value, NebulaDirectory):
                value.download()
                return value.path
...
class ShellTask(PythonInstanceTask[T]):
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs: Other arguments that can be passed to
                :py:class:`~nebulakit.core.python_function_task.PythonInstanceTask`
        """
        ...
        if task_config is not None:
            ...
            if not fully_qualified_class_name == "nebulakitplugins.pod.task.Pod":
    ...
    def _validate_output_locs(self) -> typing.Dict[str, typing.Type]:
        ...
        for v in self._output_locs:
            ...
            if not issubclass(v.var_type, NebulaFile) and not issubclass(v.var_type, NebulaDirectory):
                raise ValueError(
                    "Currently only outputs of type NebulaFile/NebulaDirectory and their derived types are supported"
                )
    ...
    def execute(self, **kwargs) -> typing.Any:
        ...
        if returncode != 0:
            ...
            logger.error(error)
            # raise NebulaRecoverableException so that it's classified as user error and will be retried
            raise NebulaRecoverableException(error)

        final_outputs = []
        for v in self._output_locs:
            if issubclass(v.var_type, NebulaFile):
                final_outputs.append(NebulaFile(outputs[v.var]))
            if issubclass(v.var_type, NebulaDirectory):
                final_outputs.append(NebulaDirectory(outputs[v.var]))
...
def get_raw_shell_task(name: str) -> RawShellTask:
    return RawShellTask(
        name=name,
        debug=True,
        inputs=nebulakit.kwtypes(env=typing.Dict[str, str], script_args=str, script_file=str),
        output_locs=[
            OutputLocation(
                var="out",
                var_type=NebulaDirectory,
