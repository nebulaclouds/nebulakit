...
from nebulakit import LaunchPlan, Resources, map_task
from nebulakit.configuration import Image, ImageConfig
from nebulakit.core.map_task import MapPythonTask, MapTaskResolver
...
def serialization_settings():
    ...
    return nebulakit.configuration.SerializationSettings(
        project="project",
        domain="domain",
        version="version",
        env=None,
...
def test_serialization(serialization_settings):
    ...
    assert task_spec.template.container.args == [
        ...
        "task-module",
        "tests.nebulakit.unit.core.test_map_task",
        "task-name",
        "t1",
    ]
...
def test_serialization_workflow_def(serialization_settings):
    ...
    nebula_entities = list(serialized_control_plane_entities.keys())

    tasks_seen = []
    for entity in nebula_entities:
...
def test_inputs_outputs_length():
    ...
    assert m.name == "tests.nebulakit.unit.core.test_map_task.map_many_inputs_24c08b3a2f9c2e389ad9fc6a03482cf9"
    ...
    assert m.name == "tests.nebulakit.unit.core.test_map_task.map_many_inputs_697aa7389996041183cf6cfd102be4f7"
    ...
    assert m.name == "tests.nebulakit.unit.core.test_map_task.map_many_inputs_cc18607da7494024a402a5fa4b3ea5c6"
    ...
    assert m.name == "tests.nebulakit.unit.core.test_map_task.map_many_inputs_52fe80b04781ea77ef6f025f4b49abef"
...
def test_map_task_override(serialization_settings):
    ...
    assert wf.nodes[0].nebula_entity.run_task.container_image == "random:image"
