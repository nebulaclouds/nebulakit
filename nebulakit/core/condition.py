...
from nebulakit.models.core import condition as _core_cond
from nebulakit.models.core import workflow as _core_wf
from nebulakit.models.literals import Binding, BindingData, Literal, RetryStrategy
from nebulakit.models.types import Error
...
class ConditionalSection:
    ...
    def __init__(self, name: str):
        ...
        NebulaContextManager.push_context(ctx.enter_conditional_section().build())
    ...
    def end_branch(self) -> Optional[Union[Condition, Promise, Tuple[Promise], VoidPromise]]:
        ...
        if self._last_case:
            ...
            NebulaContextManager.current_context().compilation_state.add_node(n)  # type: ignore
    ...
    def __init__(
        ...
        if expr is not None:
            ...
            if isinstance(expr, Promise):
                raise AssertionError(
                    "Nebulakit does not support unary expressions of the form `if_(x) - where x is an"
            ...
            if not (isinstance(expr, ConjunctionExpression) or isinstance(expr, ComparisonExpression)):
                raise AssertionError(
                    "Nebulakit only supports Comparison (<,<=,>,>=,==,!=) or Conjunction (&/|) "
...
def conditional(name: str) -> ConditionalSection:
    ...
    if ctx.compilation_state:
        ...
        if ctx.execution_state.is_local_execution():
            ...
            from nebulakit.core.context_manager import BranchEvalMode
