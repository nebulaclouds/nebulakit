...
from nebulakit import dynamic
from nebulakit.configuration import FastSerializationSettings, Image, ImageConfig, SerializationSettings
...
from nebulakit.tools.translator import gather_dependent_entities, get_serializable
...
ft = NebulaTask.promote_from_model(t1_spec.template)
...
def test_calling_lp():
    ...
    remote_lp = NebulaLaunchPlan.promote_from_model(lp_model.id, lp_model.spec)
    # To pretend that we've fetched this launch plan from Admin, also fill in the Nebula interface, which isn't
    # part of the IDL object but is something NebulaRemote does
...
def test_dynamic():
    ...
    with context_manager.NebulaContextManager.with_context(
        ...
        with context_manager.NebulaContextManager.with_context(
            ...
            assert not args.startswith("pynebula-fast-execute")
...
def test_calling_wf():
    ...
    task_templates, wf_specs, lp_specs = gather_dependent_entities(serialized)
    fwf = NebulaWorkflow.promote_from_model(
        wf_spec.template, tasks={k: NebulaTask.promote_from_model(t) for k, t in task_templates.items()}
    )
    ...
    fwf_p1 = NebulaWorkflow.promote_from_model(
        wf_spec.template,
        sub_workflows=subwf_templates,
        tasks={k: NebulaTask.promote_from_model(t) for k, t in task_templates_p1.items()},
    )
