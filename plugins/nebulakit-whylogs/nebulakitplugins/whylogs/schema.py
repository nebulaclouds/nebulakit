...
from nebulakit import BlobType, NebulaContext
...
class WhylogsDatasetProfileTransformer(TypeTransformer[DatasetProfileView]):
    ...
    def to_python_value(self, ctx: NebulaContext, lv: Literal, expected_python_type: Type[DatasetProfileView]) -> T:
        local_dir = ctx.file_access.get_random_local_path()
        ctx.file_access.download(lv.scalar.blob.uri, local_dir)
        return DatasetProfileView.read(local_dir)

    def to_html(
        self, ctx: NebulaContext, python_val: DatasetProfileView, expected_python_type: Type[DatasetProfileView]
    ) -> str:
